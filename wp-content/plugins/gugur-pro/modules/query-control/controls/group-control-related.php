...
class Group_Control_Related extends Group_Control_Query {
	...
	protected function init_fields_by_name( $name ) {
		...
		$fields['post_type']['options']['related'] = __( 'Related', 'gugur-pro' );
		...
		$related_fallback = [
			...
			'options' => [
				'fallback_none' => __( 'None', 'gugur-pro' ),
				'fallback_by_id' => __( 'Manual Selection', 'gugur-pro' ),
				'fallback_recent' => __( 'Recent Posts', 'gugur-pro' ),
			],
			'default' => 'fallback_none',
			'description' => __( 'Displayed if no relevant results are found. Manual selection display order is random', 'gugur-pro' ),
			'condition' => [
				'post_type' => 'related',
			],
			'separator' => 'before',
		...
		$fields = \gugur\Utils::array_inject( $fields, 'include_term_ids', [ 'related_taxonomies' => $related_taxonomies ] );
		$fields = \gugur\Utils::array_inject( $fields, 'offset', [ 'related_fallback' => $related_fallback ] );
		$fields = \gugur\Utils::array_inject( $fields, 'related_fallback', [ 'fallback_ids' => $fallback_ids ] );
