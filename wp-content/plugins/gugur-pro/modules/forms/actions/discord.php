...
class Discord extends Action_Base {
	...
	public function register_settings_section( $widget ) {
		...
		$widget->add_control(
			...
			[
				...
				'label_block' => true,
				'separator' => 'before',
				'description' => __( 'Enter the webhook URL that will receive the form\'s submitted data.', 'gugur-pro' ) . ' ' . sprintf( '<a href="%s" target="_blank">%s</a>.', 'https://support.discordapp.com/hc/en-us/articles/228383668-Intro-to-Webhooks', __( 'Click here for Instructions', 'gugur-pro' ) ),
				'render_type' => 'none',
			]
		);
	...
	public function run( $record, $ajax_handler ) {
		...
		$embeds = [
			'title' => isset( $settings['discord_title'] ) ? $settings['discord_title'] : __( 'A new Submission', 'gugur-pro' ),
			'description' => isset( $settings['discord_content'] ) ? $settings['discord_content'] : __( 'A new Form Submission has been received', 'gugur-pro' ),
			'author' => [
				'name'     => isset( $settings['discord_username'] ) ? $settings['discord_username'] : __( 'gugur Forms', 'gugur-pro' ),
				'url'      => $page_url,
		...
		if ( ! empty( $settings['discord_ts'] ) && 'yes' === $settings['discord_ts'] ) {
			...
			$embeds['footer'] = [
				'text' => sprintf( __( 'Powered by %s', 'gugur-pro' ), 'gugur' ),
				'icon_url' => is_ssl() ? gugur_ASSETS_URL . 'images/logo-icon.png' : null,
			];
		}
		...
		$webhook_data = apply_filters( 'gugur_pro/forms/discord/webhook_args', $webhook_data );
