...
class Slack extends Action_Base {
	...
	public function register_settings_section( $widget ) {
		...
		$widget->add_control(
			...
			[
				...
				'label_block' => true,
				'separator' => 'before',
				'description' => __( 'Enter the webhook URL that will receive the form\'s submitted data.', 'gugur-pro' ) . ' ' . sprintf( '<a href="%s" target="_blank">%s</a>.', 'https://slack.com/apps/A0F7XDUAZ-incoming-webhooks/', __( 'Click here for Instructions', 'gugur-pro' ) ),
				'render_type' => 'none',
	...
	public function run( $record, $ajax_handler ) {
		...
		$attachment = [
			'text' => __( 'A new Form Submission has been received', 'gugur-pro' ),
			'title' => __( 'A new Submission', 'gugur-pro' ),
		...
		if ( ! empty( $settings['slack_add_ts'] ) && 'yes' === $settings['slack_add_ts'] ) {
			$attachment = array_merge( $attachment, [
				'footer' => sprintf( __( 'Powered by %s', 'gugur-pro' ), 'gugur' ),
				'footer_icon' => is_ssl() ? gugur_ASSETS_URL . 'images/logo-icon.png' : null,
				'ts' => time(),
			] );
		}
		...
		$webhook_data = apply_filters( 'gugur_pro/forms/slack/webhook_args', $webhook_data );
