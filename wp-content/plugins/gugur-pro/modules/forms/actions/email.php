...
class Email extends Action_Base {
	...
	public function register_settings_section( $widget ) {
		...
		$widget->add_control(
			...
			[
				'label' => __( 'To', 'gugur-pro' ),
				'type' => Controls_Manager::TEXT,
				'default' => get_option( 'admin_email' ),
				'placeholder' => get_option( 'admin_email' ),
				'label_block' => true,
				'title' => __( 'Separate emails with commas', 'gugur-pro' ),
				'render_type' => 'none',
			]
		);

		/* translators: %s: Site title. */
		$default_message = sprintf( __( 'New message from "%s"', 'gugur-pro' ), get_option( 'blogname' ) );

		$widget->add_control(
			$this->get_control_id( 'email_subject' ),
			[
				'label' => __( 'Subject', 'gugur-pro' ),
				'type' => Controls_Manager::TEXT,
		...
		$widget->add_control(
			...
			[
				...
				'description' => sprintf( __( 'By default, all form fields are sent via %s shortcode. To customize sent fields, copy the shortcode that appears inside each field and paste it above.', 'gugur-pro' ), '<code>[all-fields]</code>' ),
				'label_block' => true,
				'render_type' => 'none',
			]
		);
		...
		$widget->add_control(
			...
			[
				...
				'options' => [
					...
					'page_url' => __( 'Page URL', 'gugur-pro' ),
					'user_agent' => __( 'User Agent', 'gugur-pro' ),
					'remote_ip' => __( 'Remote IP', 'gugur-pro' ),
					'credit' => __( 'Credit', 'gugur-pro' ),
				],
				'render_type' => 'none',
			]
		);
	...
	public function run( $record, $ajax_handler ) {
		...
		$fields = [
			...
			/* translators: %s: Site title. */
			'email_subject' => sprintf( __( 'New message from "%s"', 'gugur-pro' ), get_bloginfo( 'name' ) ),
		 ...
		 */
		$headers = apply_filters( 'gugur_pro/forms/wp_mail_headers', $headers );
		 ...
		 */
		$fields['email_content'] = apply_filters( 'gugur_pro/forms/wp_mail_message', $fields['email_content'] );
		 ...
		 */
		do_action( 'gugur_pro/forms/mail_sent', $settings, $record );
