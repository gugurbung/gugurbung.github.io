...
use gugurPro\Modules\Forms\Classes\Mailchimp_Handler;
use gugur\Settings;
...
class Mailchimp extends Integration_Base {
	...
	public function register_settings_section( $widget ) {
		...
		$widget->add_control(
			...
			[
				...
				'description' => __( 'Add comma separated tags', 'gugur-pro' ),
				'type' => Controls_Manager::TEXT,
				'render_type' => 'none',
				'condition' => [
					'mailchimp_list!' => '',
		...
		$widget->add_control(
			...
			[
				'label' => __( 'Double Opt-In', 'gugur-pro' ),
	...
	public function register_admin_fields( Settings $settings ) {
		$settings->add_section( Settings::TAB_INTEGRATIONS, 'mailchimp', [
			'callback' => function() {
				echo '<hr><h2>' . esc_html__( 'MailChimp', 'gugur-pro' ) . '</h2>';
			},
			'fields' => [
				self::OPTION_NAME_API_KEY => [
					...
					'field_args' => [
						'type' => 'text',
						'desc' => sprintf( __( 'To integrate with our forms you need an <a href="%s" target="_blank">API Key</a>.', 'gugur-pro' ), 'https://kb.mailchimp.com/integrations/api-integrations/about-api-keys' ),
					],
				],
				'validate_api_data' => [
					'field_args' => [
						'type' => 'raw_html',
						'html' => sprintf( '<button data-action="%s" data-nonce="%s" class="button gugur-button-spinner" id="gugur_pro_mailchimp_api_key_button">%s</button>', self::OPTION_NAME_API_KEY . '_validate', wp_create_nonce( self::OPTION_NAME_API_KEY ), __( 'Validate API Key', 'gugur-pro' ) ),
					],
				],
			],
		] );
	...
	public function __construct() {
		if ( is_admin() ) {
			add_action( 'gugur/admin/after_create_settings/' . Settings::PAGE_ID, [ $this, 'register_admin_fields' ], 14 );
		}
