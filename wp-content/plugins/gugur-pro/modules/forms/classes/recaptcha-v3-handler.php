...
class Recaptcha_V3_Handler extends Recaptcha_Handler {
	...
	const OPTION_NAME_V3_SITE_KEY = 'gugur_pro_recaptcha_v3_site_key';
	const OPTION_NAME_V3_SECRET_KEY = 'gugur_pro_recaptcha_v3_secret_key';
	const OPTION_NAME_RECAPTCHA_THRESHOLD = 'gugur_pro_recaptcha_v3_threshold';
	const V3 = 'v3';
	...
	public static function get_setup_message() {
		return __( 'To use reCAPTCHA V3, you need to add the API Key and complete the setup process in Dashboard > gugur > Settings > Integrations > reCAPTCHA V3.', 'gugur-pro' );
	}

	public function register_admin_fields( Settings $settings ) {
		$settings->add_section( Settings::TAB_INTEGRATIONS, 'recaptcha_v3', [
			'label' => __( 'reCAPTCHA V3', 'gugur-pro' ),
			'callback' => function() {
				echo sprintf( __( '<a href="%s" target="_blank">reCAPTCHA V3</a> is a free service by Google that protects your website from spam and abuse. It does this while letting your valid users pass through with ease.', 'gugur-pro' ), 'https://www.google.com/recaptcha/intro/v3.html' );
			},
			'fields' => [
				...
				'pro_recaptcha_v3_threshold' => [
					'label' => __( 'Score Threshold', 'gugur-pro' ),
					'field_args' => [
						'attributes' => [
							'min' => 0,
							'max' => 1,
						...
						'std' => 0.5,
						'type' => 'number',
						'desc' => __( 'Score threshold should be a value between 0 and 1, default: 0.5', 'gugur-pro' ),
					],
				],
			],
		] );
	...
	protected function add_error( $ajax_handler, $field, $message ) {
		...
		$ajax_handler->add_error_message( __( 'reCAPTCHA V3 validation failed, suspected as abusive usage', 'gugur-pro' ) );
	}
	...
	public function add_field_type( $field_types ) {
		$field_types['recaptcha_v3'] = __( 'reCAPTCHA V3', 'gugur-pro' );
	...
	public function __construct() {
		parent::__construct();
		add_filter( 'gugur_pro/forms/render/item/' . self::get_recaptcha_name(), [ $this, 'filter_recaptcha_item' ], 10, 3 );
	}
}
