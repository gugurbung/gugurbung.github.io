...
class Recaptcha_Handler {
	...
	const OPTION_NAME_SITE_KEY = 'gugur_pro_recaptcha_site_key';

	const OPTION_NAME_SECRET_KEY = 'gugur_pro_recaptcha_secret_key';

	const OPTION_NAME_RECAPTCHA_THRESHOLD = 'gugur_pro_recaptcha_threshold';
	...
	public static function get_setup_message() {
		return __( 'To use reCAPTCHA, you need to add the API Key and complete the setup process in Dashboard > gugur > Settings > Integrations > reCAPTCHA.', 'gugur-pro' );
	}

	public function register_admin_fields( Settings $settings ) {
		$settings->add_section( Settings::TAB_INTEGRATIONS, static::get_recaptcha_name(), [
			'label' => __( 'reCAPTCHA', 'gugur-pro' ),
			'callback' => function () {
				echo sprintf( __( '<a href="%s" target="_blank">reCAPTCHA</a> is a free service by Google that protects your website from spam and abuse. It does this while letting your valid users pass through with ease.', 'gugur-pro' ), 'https://www.google.com/recaptcha/' );
			},
			'fields' => [
				'pro_recaptcha_site_key' => [
					'label' => __( 'Site Key', 'gugur-pro' ),
					'field_args' => [
						'type' => 'text',
					],
				],
				'pro_recaptcha_secret_key' => [
					'label' => __( 'Secret Key', 'gugur-pro' ),
					'field_args' => [
						'type' => 'text',
					],
				],
	...
	protected static function get_script_name() {
		return 'gugur-' . static::get_recaptcha_name() . '-api';
	}

	public function register_scripts() {
		$script_name = static::get_script_name();
		$src = 'https://www.google.com/recaptcha/api.js?render=explicit';
		wp_register_script( $script_name, $src, [], gugur_PRO_VERSION, true );
	}

	public function enqueue_scripts() {
		if ( Plugin::gugur()->preview->is_preview_mode() ) {
			return;
		}
		$script_name = static::get_script_name();
	...
	public function validation( $record, $ajax_handler ) {
		...
		if ( empty( $_POST['g-recaptcha-response'] ) ) {
			$ajax_handler->add_error( $field['id'], __( 'The Captcha field cannot be blank. Please enter a value.', 'gugur-pro' ) );

			return;
		}

		$recaptcha_errors = [
			'missing-input-secret' => __( 'The secret parameter is missing.', 'gugur-pro' ),
			'invalid-input-secret' => __( 'The secret parameter is invalid or malformed.', 'gugur-pro' ),
			'missing-input-response' => __( 'The response parameter is missing.', 'gugur-pro' ),
			'invalid-input-response' => __( 'The response parameter is invalid or malformed.', 'gugur-pro' ),
		];
		...
		if ( 200 !== (int) $response_code ) {
			...
			$ajax_handler->add_error( $field['id'], sprintf( __( 'Can not connect to the reCAPTCHA server (%d).', 'gugur-pro' ), $response_code ) );
		...
		if ( ! $this->validate_result( $result, $field ) ) {
			$message = __( 'Invalid Form - reCAPTCHA validation failed', 'gugur-pro' );
	...
	public function render_field( $item, $item_index, $widget ) {
		$recaptcha_html = '<div class="gugur-field" id="form-field-' . $item['custom_id'] . '">';
		...
		if ( static::is_enabled() ) {
			...
			$recaptcha_html .= '<div class="gugur-alert gugur-alert-info">';
	...
	public function add_field_type( $field_types ) {
		$field_types['recaptcha'] = __( 'reCAPTCHA', 'gugur-pro' );
	...
	public function __construct() {
		...
		add_action( 'gugur_pro/forms/render_field/' . static::get_recaptcha_name(), [ $this, 'render_field' ], 10, 3 );
		add_filter( 'gugur_pro/forms/render/item', [ $this, 'filter_field_item' ] );
		...
		if ( static::is_enabled() ) {
			...
			add_action( 'gugur/preview/enqueue_scripts', [ $this, 'enqueue_scripts' ] );
		}
