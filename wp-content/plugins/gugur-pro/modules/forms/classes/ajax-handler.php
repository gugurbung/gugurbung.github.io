...
class Ajax_Handler {
	...
	public static function is_form_submitted() {
		return wp_doing_ajax() && isset( $_POST['action'] ) && 'gugur_pro_forms_send_form' === $_POST['action'];
	}
	...
	public function ajax_send_form() {
		...
		$gugur = Plugin::gugur();
		$document = $gugur->documents->get( $post_id );
		...
		if ( ! empty( $form['templateID'] ) ) {
			$template = $gugur->documents->get( $form['templateID'] );
