...
class Price_Table extends Base_Widget {
	...
	protected function _register_controls() {
		...
		$this->add_control(
			'heading_tag',
			[
				'label' => __( 'Heading Tag', 'gugur-pro' ),
			...
			<# if ( settings.features_list ) { #>
				...
				<# _.each( settings.features_list, function( item, index ) {
					...
					var featureKey = view.getRepeaterSettingKey( 'item_text', 'features_list', index ),
						migrated = gugur.helpers.isIconMigrated( item, 'selected_item_icon' );
						...
						<li class="gugur-repeater-item-{{ item._id }}">
							<div class="gugur-price-table__feature-inner">
								<# if ( item.item_icon  || item.selected_item_icon ) {
									iconsHTML[ index ] = gugur.helpers.renderIcon( view, item.selected_item_icon, { 'aria-hidden': 'true' }, 'i', 'object' );
