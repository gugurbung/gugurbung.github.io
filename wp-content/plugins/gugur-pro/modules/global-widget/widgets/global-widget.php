<?php
namespace gugurPro\Modules\GlobalWidget\Widgets;
...
class Global_Widget extends Base_Widget {
	...
	public function __construct( $data = [], $args = null ) {
		if ( $data ) {
			$template_data = Plugin::gugur()->templates_manager->get_template_data( [
				'source' => 'local',
			...
			$original_widget_type = Plugin::gugur()->widgets_manager->get_widget_types( $template_data['content'][0]['widgetType'] );
			if ( ! $original_widget_type ) {
				throw new \Exception( 'Original Widget Type not found.' );
			}

			if ( ! empty( $data['previewSettings'] ) && ( is_preview() || Plugin::gugur()->preview->is_preview_mode() ) ) {
	...
	public function get_unique_selector() {
		return '.gugur-global-' . $this->get_data( 'templateID' );
	}
	...
	protected function _add_render_attributes() {
		...
		$this->add_render_attribute( '_wrapper', [
			'class' => [
				'gugur-global-' . $this->get_data( 'templateID' ),
				'gugur-widget-' . $original_widget_type,
			],
		] );
	}
	...
	private function set_preview_settings( $raw_data ) {
		...
		$ajax = Plugin::gugur()->common->get_component( 'ajax' );
