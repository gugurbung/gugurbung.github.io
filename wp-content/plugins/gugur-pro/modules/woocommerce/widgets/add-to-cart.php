...
class Add_To_Cart extends Widget_Button {
	...
	public function get_title() {
		return __( 'Custom Add To Cart', 'gugur-pro' );
	}
	...
	protected function _register_controls() {
		...
		$this->add_control(
			'show_quantity',
			[
				'label' => __( 'Show Quantity', 'gugur-pro' ),
				'type' => Controls_Manager::SWITCHER,
				'label_off' => __( 'Hide', 'gugur-pro' ),
				'label_on' => __( 'Show', 'gugur-pro' ),
				'description' => __( 'Please note that switching on this option will disable some of the design controls.', 'gugur-pro' ),
			]
		);
		...
		$this->update_control(
			'text',
			[
				'default' => __( 'Add to Cart', 'gugur-pro' ),
				'placeholder' => __( 'Add to Cart', 'gugur-pro' ),
			]
		);
	...
	private function render_ajax_button( $product ) {
		...
		if ( $product ) {
			...
			$settings['text'] = __( 'Please set a valid product', 'gugur-pro' );
	...
	private function render_form_button( $product ) {
		if ( ! $product && current_user_can( 'manage_options' ) ) {
			echo __( 'Please set a valid product', 'gugur-pro' );
		...
		$form = ob_get_clean();
		$form = str_replace( 'single_add_to_cart_button', 'single_add_to_cart_button gugur-button', $form );
		echo $form;
