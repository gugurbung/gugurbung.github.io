	...
	public static function get_preview_as_options() {
		...
		foreach ( $post_types as $post_type => $label ) {
			...
			if ( $post_type_object->has_archive ) {
				$post_type_archives[ 'post_type_archive/' . $post_type ] = sprintf( __( '%s Archive', 'gugur-pro' ), $post_type_object->label );
			}
			...
			foreach ( $post_type_taxonomies as $slug => $object ) {
				$taxonomies[ 'taxonomy/' . $slug ] = sprintf( __( '%s Archive', 'gugur-pro' ), $object->label );
			}
		}

		$options = [
			'archive/recent_posts' => __( 'Recent Posts', 'gugur-pro' ),
			'archive/date' => __( 'Date Archive', 'gugur-pro' ),
			'archive/author' => __( 'Author Archive', 'gugur-pro' ),
