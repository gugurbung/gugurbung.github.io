...
class Module extends Module_Base {
	...
	public function localize_settings( $settings ) {
		...
		$settings = array_replace_recursive( $settings, [
			'i18n' => [
				...
				'conditions_title' => sprintf( __( 'Where Do You Want to Display Your %s?', 'gugur-pro' ), $document->get_post_type_title() ),
				'conditions_description' => sprintf( __( 'Set the conditions that determine where your %s is used throughout your site.', 'gugur-pro' ), $document->get_post_type_title() ) . '<br>' . __( 'For example, choose \'Entire Site\' to display the template across your site.', 'gugur-pro' ),
			...
			<div class="gugur-form-field__select__wrapper">
				<select id="gugur-new-template__form__post-type" class="gugur-form-field__select" name="<?php echo Single::REMOTE_CATEGORY_META_KEY; ?>">
					...
					foreach ( $post_types as $post_type => $label ) {
						...
						// New: Core >=2.7.0
						$is_base_page = class_exists( '\gugur\Core\DocumentTypes\PageBase' ) && $doc_class instanceof \gugur\Core\DocumentTypes\PageBase;
	...
	public function admin_menu() {
		add_submenu_page( Source_Local::ADMIN_MENU_SLUG, '', __( 'Theme Builder', 'gugur-pro' ), 'publish_posts', $this->get_admin_templates_url( true ) );
	}
