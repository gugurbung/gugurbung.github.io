<?php
namespace gugurPro\Modules\AssetsManager\AssetTypes\Fonts;

use gugur\Core\Files\CSS\Base;
use gugurPro\Modules\AssetsManager\Classes;
use gugurPro\Modules\AssetsManager\AssetTypes\Fonts_Manager;
use gugurPro\Plugin;
...
class Custom_Fonts extends Classes\Font_Base {
	...
	const FONT_META_KEY = 'gugur_font_files';
	const FONT_FACE_META_KEY = 'gugur_font_face';

	public function get_name() {
		return __( 'Custom Fonts', 'gugur-pro' );
	}
	...
	public function add_meta_box() {
		add_meta_box(
			'gugur-font-' . $this->get_type() . 'metabox',
			__( 'Manage Your Font Files', 'gugur-pro' ),
			[ $this, 'render_metabox' ],
			Fonts_Manager::CPT,
			'normal',
			'default'
	...
	public function render_metabox( $post ) {
		...
		$fields = [
			...
			[
				...
				'field_type' => 'html',
				'label' => false,
				'raw_html' => sprintf( '<div class="inline-preview">%s</div>', __( 'gugur Is Making the Web Beautiful!!!', 'gugur-pro' ) ),
			],
			[
				'id' => 'toolbar',
				'field_type' => 'toolbar',
		...
		foreach ( $this->get_file_types() as $type => $mine ) {
			$fields[] = [
				...
				'label' => sprintf( __( '%s File', 'gugur-pro' ), strtoupper( $type ) ),
				/* translators: %s: Font file format. */
				'box_title' => sprintf( __( 'Upload font .%s file', 'gugur-pro' ), $type ),
				/* translators: %s: Font file format. */
				'box_action' => sprintf( __( 'Select .%s file', 'gugur-pro' ), $type ),
				'preview_anchor' => 'none',
		...
		$repeater = [
			...
			'id' => 'font_face',
			'label' => false,
			'add_label' => __( 'Add Font Variation', 'gugur-pro' ),
			'toggle_title' => __( 'Edit', 'gugur-pro' ),
			'remove_title' => __( 'Delete', 'gugur-pro' ),
			'field_type' => 'repeater',
			'row_label' => [
				'default' => 'Settings',
	...
	public function upload_mimes( $mine_types ) {
		if ( current_user_can( Fonts_Manager::CAPABILITY ) && $this->is_gugur_font_upload() ) {
			foreach ( $this->get_file_types() as $type => $mine ) {
	...
	public function wp_handle_upload_prefilter( $file ) {
		...
		/**
		 * @var \gugur\Core\Files\Assets\Svg\Svg_Handler $svg_handler;
		 */
		$svg_handler = Plugin::gugur()->assets_manager->get_asset( 'svg-handler' );

		if ( $svg_handler::svg_sanitizer_can_run() && ! $svg_handler->sanitize_svg( $file['tmp_name'] ) ) {
			$file['error'] = __( 'Invalid SVG Format, file not uploaded for security reasons', 'gugur-pro' );
		}

		return $file;
	}

	private function is_gugur_font_upload() {
		return isset( $_POST['uploadTypeCaller'] ) && 'gugur-admin-font-upload' === $_POST['uploadTypeCaller']; // phpcs:ignore
	}
	...
	public function get_font_face_from_data( $font_family, $data ) {
		...
		$font_face .= "\tfont-display: " . apply_filters( 'gugur_pro/custom_fonts/font_display', 'auto', $font_family, $data ) . ';' . PHP_EOL;
	...
	public function handle_panel_request( array $data ) {
		...
		if ( empty( $font_face ) ) {
			...
			$error_message = sprintf( __( 'Font %s was not found.', 'gugur-pro' ), $font_family );
	...
	private function get_font_style_options() {
		return [
			...
			'italic' => __( 'Italic', 'gugur-pro' ),
			'oblique' => __( 'Oblique', 'gugur-pro' ),
		];
	}
	...
	private function get_file_type_description( $file_type ) {
		$descriptions = [
			'eot' => __( 'Embedded OpenType, Used by IE6-IE9 Browsers', 'gugur-pro' ),
			'woff2' => __( 'The Web Open Font Format 2, Used by Super Modern Browsers', 'gugur-pro' ),
			'woff' => __( 'The Web Open Font Format, Used by Modern Browsers', 'gugur-pro' ),
			'ttf' => __( 'TrueType Fonts, Used for better supporting Safari, Android, iOS', 'gugur-pro' ),
			'svg' => __( 'SVG fonts allow SVG to be used as glyphs when displaying text, Used by Legacy iOS', 'gugur-pro' ),
		];
