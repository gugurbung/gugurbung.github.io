...
use gugurPro\Modules\AssetsManager\Classes\Font_Base;
use gugur\Settings;
...
class Typekit_Fonts extends Font_Base {
	...
	const TYPEKIT_FONTS_OPTION_NAME = 'gugur_typekit-data';
	...
	private function get_typekit_kit_id() {
		return get_option( 'gugur_' . self::TYPEKIT_KIT_ID_OPTION_NAME, false );
	}

	public function get_name() {
		return __( 'TypeKit Web Fonts by Adobe', 'gugur-pro' );
	}
	...
	private function fetch_typekit_data() {
		...
		if ( 200 !== $response_code ) {
			switch ( $response_code ) {
				case 404:
					$this->error = __( 'Project not found.', 'gugur-pro' );
					break;
				default:
		...
		if ( ! $response_body ) {
			$this->error = __( 'No project data was returned.', 'gugur-pro' );
	...
	public function handle_panel_request( array $data ) {
		...
		if ( ! $typekit_fonts || ! is_array( $typekit_fonts ) ) {
			throw new \Exception( __( 'Error with TypeKit fonts', 'gugur-pro' ) );
		}

		if ( ! in_array( $font_family, array_keys( $typekit_fonts ) ) ) {
			throw new \Exception( __( 'Font missing in Project', 'gugur-pro' ) );
		}
	...
	public function register_admin_fields( Settings $settings ) {
		...
		$button_label = __( 'Get Project ID', 'gugur-pro' );
		$found_label = '<span class="gugur-pro-typekit-count">{{count}}</span> ' . __( 'Fonts Families Found in project. Please note that typekit takes a few minutes to sync once you publish or update a project.', 'gugur-pro' );
		if ( $fonts && is_array( $fonts ) ) {
			$button_label = __( 'Sync Project', 'gugur-pro' );
		}
		$settings->add_section( Settings::TAB_INTEGRATIONS, 'typekit', [
			'callback' => function() {
				echo '<hr><h2>' . esc_html__( 'TypeKit Web Fonts by Adobe', 'gugur-pro' ) . '</h2>';
				esc_html_e( 'TypeKit partners with the world’s leading type foundries to bring thousands of beautiful fonts to designers every day.', 'gugur-pro' );
			},
			'fields' => [
				self::TYPEKIT_KIT_ID_OPTION_NAME => [
					'label' => __( 'Project ID', 'gugur-pro' ),
					'field_args' => [
						'type' => 'text',
						'desc' => sprintf( __( 'Enter Your <a href="%s" target="_blank">TypeKit Project ID</a>.', 'gugur-pro' ), 'https://fonts.adobe.com/typekit' ),
					],
					'setting_args' => [
				...
				'validate_api_data' => [
					'field_args' => [
						'type' => 'raw_html',
						'html' => sprintf( '<button data-found="%s" data-action="%s" data-nonce="%s" class="button gugur-button-spinner" id="gugur_pro_typekit_validate_button">%s</button><br><p><span class="gugur-pro-typekit-data hidden"></span></p>',
	...
	public function integrations_admin_ajax_handler() {
		...
		$fonts = [];
		try {
			update_option( 'gugur_' . self::TYPEKIT_KIT_ID_OPTION_NAME, sanitize_text_field( $_POST['kit_id'] ) );
	...
	protected function actions() {
		...
		add_action( 'gugur/fonts/print_font_links/' . $this->get_type(), [ $this, 'print_font_link' ] );
		add_action( 'wp_ajax_gugur_pro_admin_fetch_fonts', [ $this, 'integrations_admin_ajax_handler' ] );
	}
}
