...
class Custom_Icons extends  Assets_Base {
	...
	const META_KEY = 'gugur_custom_icon_set_config';
	const OPTION_NAME = 'gugur_custom_icon_sets_config';

	public $current_post_id = 0;

	public function get_name() {
		return __( 'Custom Icons', 'gugur-pro' );
	}
	...
	public function add_meta_box() {
		add_meta_box(
			'gugur-custom-icons-metabox',
			__( 'Icon Set', 'gugur-pro' ),
			[ $this, 'render_metabox' ],
			Icons_Manager::CPT,
			'normal',
			'default'
	...
	public function render_metabox( $post ) {
		...
		$fields = [
			[
				...
				'tag' => 'div',
				'attributes' => [
					'class' => 'gugur-custom-icons-metabox',
				],
			],
			[
				'id' => 'zip_upload',
				...
				'label' => false,
				'sub-label' => __( 'Your Fontello, IcoMoon or Fontastic .zip file', 'gugur-pro' ),
			],
			[
				'id' => 'close_div',
				'field_type' => 'html_tag',
	...
	public static function get_supported_icon_sets() {
		...
		return array_merge( apply_filters( 'gugur_pro/icons_manager/custom_icons/additional_supported_types', [] ), $icon_sets );
	}
	...
	private function upload() {
		...
		if ( 'zip' !== $ext ) {
			...
			return new \WP_Error( 'unsupported_file', __( 'Only zip files are allowed', 'gugur-pro' ) );
		}
	...
	private function upload_and_extract_zip() {
		...
		if ( count( $source_files ) === 0 ) {
			return new \WP_Error( 'incompatible_archive', esc_html__( 'Incompatible archive', 'gugur-pro' ) );
		}
	...
	public function custom_icons_upload_handler( $data ) {
		...
		return new \WP_Error( 'unsupported_zip_format', __( 'The zip file provided is not supported!', 'gugur-pro' ) );
	}
	...
	public function handle_delete_icon_set( $post_id ) {
		...
		$icon_set_dir = get_post_meta( $post_id, '_gugur_icon_set_path', true );
	...
	public function add_custom_icon_templates( $current_screen ) {
		if ( 'gugur_icons' !== $current_screen->id || 'post' !== $current_screen->base ) {
			return;
		}
		Plugin::gugur()->common->add_template( __DIR__ . '/templates.php' );
	}
	...
	protected function actions() {
		...
		add_filter( 'gugur/icons_manager/additional_tabs', [ $this, 'register_icon_libraries_control' ] );
		add_filter( 'gugur/editor/localize_settings', [ $this, 'add_custom_icons_url' ] );
