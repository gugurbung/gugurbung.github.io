...
jQuery(function () {
	gugurProAdmin.assetsManager.fontManager.init();
	gugurProAdmin.roleManager.advancedRoleManager.init();
});
...
module.exports = function () {
	...
	self.init = function () {
		jQuery(document).on('change', '.gugur-widget-template-select', function () {
			var $this = jQuery(this),
			    templateID = $this.val(),
			    $editButton = $this.parents('p').find('.gugur-edit-template'),
			    type = $this.find('[value="' + templateID + '"]').data('type');

			if ('page' !== type) {
				// 'widget' is editable only from gugur page
				$editButton.hide();

				return;
			}

			var editUrl = gugurProConfig.i18n.home_url + '?p=' + templateID + '&gugur';
...
module.exports = function (key, fieldID) {
	...
	self.cacheElements = function () {
		this.cache = {
			$button: jQuery('#gugur_pro_' + key + '_button'),
			$apiKeyField: jQuery('#gugur_pro_' + key),
			$apiUrlField: jQuery('#gugur_pro_' + fieldID)
		};
	};
...
module.exports = function () {
	...
	self.selectors = {
		editPageClass: 'post-type-gugur_font',
		title: '#title',
		...
		customFontsMetaBox: '#gugur-font-custommetabox',
		closeHandle: 'button.handlediv',
		toolbar: '.gugur-field-toolbar',
		inlinePreview: '.inline-preview',
		fileUrlInput: '.gugur-field-file input[type="text"]'
	};
	...
	self.updateRowLabel = function (event, $table) {
		...
		return self.renderTemplate(self.fontLabelTemplate, {
			...
			preview: '<span style="' + previewStyle.styleAttribute + '">gugur is making the web beautiful</span>',
			toolbar: toolbarHtml
		});
	};
...
module.exports = {
	...
	selectors: {
		uploadBtnClass: 'gugur-upload-btn',
		clearBtnClass: 'gugur-upload-clear-btn',
		uploadBtn: '.gugur-upload-btn',
		clearBtn: '.gugur-upload-clear-btn',
		inputURLField: '.gugur-field-file input[type="text"]'
	},
	...
	setUploadParams: function setUploadParams(ext, name) {
		...
		uploader.param('uploadTypeCaller', 'gugur-admin-font-upload');
...
module.exports = {
	selectors: {
		...
		repeaterField: '.gugur-field-repeater'
	},
...
module.exports = function () {
	...
	self.cacheElements = function () {
		this.cache = {
			$button: jQuery('#gugur_pro_typekit_validate_button'),
			$kitIdField: jQuery('#gugur_typekit-kit-id'),
			$dataLabelSpan: jQuery('.gugur-pro-typekit-data')
		};
	};
	self.bindEvents = function () {
		this.cache.$button.on('click', function (event) {
	...
	self.fetchFonts = function () {
		...
		jQuery.post(ajaxurl, {
			action: 'gugur_pro_admin_fetch_fonts',
			kit_id: kitID,
			_nonce: self.cache.$button.data('nonce')
		}).done(function (data) {
			if (data.success) {
...
var CustomIcons = function (_gugurModules$Vie) {
	_inherits(CustomIcons, _gugurModules$Vie);
	...
	_createClass(CustomIcons, [{
		...
		value: function getDefaultSettings() {
			return {
				...
				classes: {
					editPageClass: 'post-type-gugur_icons',
					editPhp: 'edit-php',
					hasIcons: 'gugur--has-icons'
				},
				selectors: {
					editPageClass: 'post-type-gugur_icons',
					title: '#title',
					metaboxContainer: '#gugur-custom-icons-metabox',
					metabox: '.gugur-custom-icons-metabox',
					closeHandle: 'button.handlediv',
					iconsTemplate: '#gugur-icons-template',
					dataInput: '#gugur_custom_icon_set_config',
					dropzone: '.zip_upload',
				...
				templates: {
					...
					header: jQuery('#gugur-custom-icons-template-header').html(),
					footer: jQuery('#gugur-custom-icons-template-footer').html(),
					duplicatePrefix: jQuery('#gugur-custom-icons-template-duplicate-prefix').html()
				}
			};
		}
	}, {
		...
		value: function bindEvents() {
			...
			gugurCommon.elements.$document.on('click', '.remove', triggerDelete);
		...
		value: function prepareIconName(icon) {
			...
			return gugurCommon.helpers.upperCaseWords(iconName);
		}
	}, {
		key: 'getCreatedOn',
		...
		value: function enqueueCSS(url) {
			if (!gugurCommon.elements.$document.find('link[href="' + url + '"]').length) {
				gugurCommon.elements.$document.find('link:last').after('<link href="' + url + '" rel="stylesheet" type="text/css">');
			}
		}
	}, {
		key: 'setData',
		...
		value: function renderIconList(config) {
			...
			return config.icons.map(function (icon) {
				...
				return gugurCommon.compileTemplate(iconTemplate, data);
			}).join('\n');
		}
	}, {
		key: 'renderIcons',
		value: function renderIcons(config) {
			...
			$metabox.html('');
			$metaboxContainer.prepend(gugurCommon.compileTemplate(header, config));
			$metabox.append('<ul>' + this.renderIconList(config) + '</ul>');
			$metaboxContainer.append(gugurCommon.compileTemplate(footer, this.getCreatedOn()));
		}
	}, {
		key: 'onTitleInput',
		...
		value: function showAlertDialog(id, message) {
			...
			return gugurCommon.dialogsManager.createWidget('alert', alertData).show();
		}
	}, {
		key: 'onSuccess',
		...
		value: function onInit() {
			...
			var $body = gugurCommon.elements.$body,
...
var DropZoneField = function (_gugurModules$Vie) {
	_inherits(DropZoneField, _gugurModules$Vie);
	...
	_createClass(DropZoneField, [{
		key: 'getDefaultSettings',
		value: function getDefaultSettings() {
			var baseSelector = '.gugur-dropzone-field';
			return {
				droppedFiles: false,
				selectors: {
					...
					browseButton: baseSelector + ' .gugur--dropzone--upload__browse',
					postId: '#post_ID'
				},
				classes: {
					drag: 'is-dragover',
		...
		value: function onInit() {
			...
			gugurCommon.elements.$document.trigger('onDropzoneLoaded', [this]);
		}
	}]);
...
var _class = function (_gugurModules$Vie) {
	...
	_createClass(_class, [{
		...
		value: function getDefaultSettings() {
			return {
				selectors: {
					button: '#gugur_pro_fa_pro_validate_button',
					kitIdField: '#gugur_font_awesome_pro_kit_id'
				}
			};
		}
	}, {
...
module.exports = function () {
	...
	self.cacheElements = function () {
		this.cache = {
			$checkBox: jQuery('input[name="gugur_exclude_user_roles[]"]'),
			$advanced: jQuery('#gugur_advanced_role_manager')
		};
	};
	self.bindEvents = function () {
	...
	self.init = function () {
		if (!jQuery('body').hasClass('gugur_page_gugur-role-manager')) {
			return;
		}
		this.cacheElements();
		this.bindEvents();
...
module.exports = function () {
	var selectors = {
		templateTypeInput: '#gugur-new-template__form__template-type',
		locationWrapper: '#gugur-new-template__form__location__wrapper',
		postTypeWrapper: '#gugur-new-template__form__post-type__wrapper'
	};
	...
	var setElements = function setElements() {
		jQuery.each(selectors, function (key, selector) {
			key = '$' + key;
			elements[key] = gugurNewTemplate.layout.getModal().getElements('content').find(selector);
		});
	};
	...
	this.init = function () {
		if (!window.gugurNewTemplate) {
			return;
		}

		// Make sure the modal has already been initialized
		gugurNewTemplate.layout.getModal();
