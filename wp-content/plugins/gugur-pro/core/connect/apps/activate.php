<?php
namespace gugurPro\Core\Connect\Apps;

use gugur\Core\Common\Modules\Connect\Apps\Common_App;
use gugurPro\License;
...
class Activate extends Common_App {
	...
	public function action_activate_license() {
		if ( ! $this->is_connected() ) {
			$this->add_notice( __( 'Please connect to gugur in order to activate license.', 'gugur-pro' ), 'error' );
		...
		if ( empty( $license ) ) {
			...
			wp_die( 'License not found for user ' . $this->get( 'user' )->email, __( 'gugur Pro', 'gugur-pro' ), [
				'back_link' => true,
			] );
		}

		if ( is_wp_error( $license ) ) {
			wp_die( $license, __( 'gugur Pro', 'gugur-pro' ), [
				'back_link' => true,
			] );
		}
		...
		if ( empty( $license_key ) ) {
			wp_die( __( 'License key is missing.', 'gugur-pro' ), __( 'gugur Pro', 'gugur-pro' ), [
				'back_link' => true,
			] );
		}
		...
		$this->add_notice( __( 'License has been activated successfully.', 'gugur-pro' ) );
