...
use gugur\Core\Upgrade\Updater;
use gugurPro\Plugin;
...
class Upgrades {
	...
	public static function _v_1_3_0() {
		...
		$post_ids = $wpdb->get_col(
			'SELECT `post_id` FROM `' . $wpdb->postmeta . '` WHERE `meta_key` = "_gugur_data" AND `meta_value` LIKE \'%"widgetType":"form"%\';'
		);
	...
	public static function _v_1_4_0() {
		...
		$post_ids = $wpdb->get_col(
			'SELECT `post_id` FROM `' . $wpdb->postmeta . '` WHERE `meta_key` = "_gugur_data" AND `meta_value` LIKE \'%"widgetType":"posts"%\';'
		);
	...
	public static function _v_1_12_0() {
		...
		foreach ( $post_ids as $post_id ) {
			...
			$data = Plugin::gugur()->db->iterate_data( $data, function( $element ) use ( &$do_update ) {
				if ( empty( $element['widgetType'] ) || 'form' !== $element['widgetType'] ) {
					return $element;
				}
			...
			update_metadata( 'post', $post_id, '_gugur_data', $json_value );
		}
	}
	...
	public static function _v_2_0_3() {
		...
		$post_ids = $wpdb->get_col(
			'SELECT `post_id` FROM `' . $wpdb->postmeta . '` WHERE `meta_key` = "_gugur_data" AND `meta_value` LIKE \'%"sticky":"yes"%\';'
		);
	...
	private static function save_editor( $post_id, $posted ) {
		...
		$is_meta_updated = update_metadata( 'post', $post_id, '_gugur_data', $json_value );
		 ...
		 *
		 * Fires after gugur editor data was saved.
		 *
		 * @since 1.0.0
		 *
		 ...
		 */
		do_action( 'gugur/editor/after_save', $post_id, $editor_data );
	}
	...
	private static function get_editor_data( $data, $with_html_content = false ) {
		...
		foreach ( $data as $element_data ) {
			$element = Plugin::gugur()->elements_manager->create_element_instance( $element_data );
	...
	public static function _v_2_5_0_form( $updater ) {
		$changes = [
			[
				'callback' => [ 'gugurPro\Core\Upgrade\Upgrades', '_rename_repeater_settings' ],
				'control_ids' => [
					'form_fields' => [
						'_id' => 'custom_id',
					],
	...
	public static function _v_2_5_0_woocommerce_menu_cart( $updater ) {
		$changes = [
			[
				'callback' => [ 'gugurPro\Core\Upgrade\Upgrades', '_rename_widget_settings' ],
				'control_ids' => [
			...
			[
				'callback' => [ 'gugurPro\Core\Upgrade\Upgrades', '_slider_to_border_settings' ],
				'control_ids' => [
	...
	public static function _v_2_5_0_posts( $updater ) {
		...
		$changes = [
			...
			[
				'callback' => [ 'gugurPro\Core\Upgrade\Upgrades', '_add_widget_settings_to_array' ],
				'control_ids' => array_merge( $add_taxonomies, [
					'posts_authors' => [ 'posts_include' => 'authors' ],
				] ),
			],
			[
				'callback' => [ 'gugurPro\Core\Upgrade\Upgrades', '_merge_widget_settings' ],
	...
	public static function _v_2_5_0_popup_border_radius( $updater ) {
		...
		$post_ids = $updater->query_col(
			"SELECT pm1.post_id
					...
					WHERE pm1.meta_key = '_gugur_template_type' 
	...
	public static function _v_2_5_4_posts( $updater ) {
		...
		$changes = [
			[
				'callback' => [ 'gugurPro\Core\Upgrade\Upgrades', '_convert_term_id_to_term_taxonomy_id' ],
				'control_ids' => $merge_taxonomies,
				'prefix' => 'posts_',
	...
	public static function _v_2_5_4_form( $updater ) {
		$changes = [
			[
				'callback' => [ 'gugurPro\Core\Upgrade\Upgrades', '_missing_form_custom_id_settings' ],
				'control_ids' => [],
			],
		];
	...
	public static function _update_widget_settings( $widget_id, $updater, $changes ) {
		...
		foreach ( $post_ids as $post_id ) {
			...
			foreach ( $changes as $change ) {
				...
				$data = Plugin::gugur()->db->iterate_data( $data, $change['callback'], $args );
				if ( ! $do_update ) {
					continue;
				}
