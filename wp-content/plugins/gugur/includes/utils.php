 ...
 * gugur utils.
 *
 * gugur utils handler class is responsible for different utility methods
 * used by gugur.
 *
 * @since 1.0.0
 */
class Utils {
	...
	public static function get_edit_link( $post_id = 0 ) {
		 ...
		 * Get edit link.
		 *
		 * Filters the gugur edit link.
		 *
		 * @since 1.0.0
		 * @deprecated 2.0.0 Use `gugur/document/urls/edit` filter instead.
		 *
		 * @param string $edit_link New URL query string (unescaped).
		 * @param int    $post_id   Post ID.
		 */
		$edit_link = apply_filters_deprecated( 'gugur/utils/get_edit_link', [ $edit_link, $post_id ], '2.0.0', 'gugur/document/urls/edit' );
	 ...
	 * Get pro link.
	 *
	 * Retrieve the link to gugur Pro.
	 *
	 * @since 1.7.0
	 * @access public
	 * @static
	 *
	 * @param string $link URL to gugur pro.
	 *
	 * @return string gugur pro link.
	 */
	...
	public static function get_preview_url( $post_id ) {
		 ...
		 * Preview URL.
		 *
		 * Filters the gugur preview URL.
		 *
		 * @since 1.6.4
		 * @deprecated 2.0.0 Use `gugur/document/urls/preview` filter instead.
		 *
		 * @param string $preview_url URL with chosen scheme.
		 * @param int    $post_id     Post ID.
		 */
		$url = apply_filters_deprecated( 'gugur/utils/preview_url', [ $url, $post_id ], '2.0.0', 'gugur/document/urls/preview' );
	...
	public static function get_wp_preview_url( $post_id ) {
		 ...
		 *
		 * @since 1.9.0
		 * @deprecated 2.0.0 Use `gugur/document/urls/wp_preview` filter instead.
		 *
		 * @param string $wp_preview_url WordPress preview URL.
		 * @param int    $post_id        Post ID.
		 */
		$wp_preview_url = apply_filters_deprecated( 'gugur/utils/wp_preview_url', [ $wp_preview_url, $post_id ], '2.0.0', 'gugur/document/urls/wp_preview' );
	 ...
	 * Replace URLs.
	 *
	 * Replace old URLs to new URLs. This method also updates all the gugur data.
	 *
	 * @since 2.1.0
	 * @static
	 * @access public
	...
	public static function replace_urls( $from, $to ) {
		...
		if ( $from === $to ) {
			throw new \Exception( __( 'The `from` and `to` URL\'s must be different', 'gugur' ) );
		}

		$is_valid_urls = ( filter_var( $from, FILTER_VALIDATE_URL ) && filter_var( $to, FILTER_VALIDATE_URL ) );
		if ( ! $is_valid_urls ) {
			throw new \Exception( __( 'The `from` and `to` URL\'s must be valid URL\'s', 'gugur' ) );
		}
		...
		$rows_affected = $wpdb->query(
			...
			"WHERE `meta_key` = '_gugur_data' AND `meta_value` LIKE '[%' ;" ); // meta_value LIKE '[%' are json formatted
		// @codingStandardsIgnoreEnd

		if ( false === $rows_affected ) {
			throw new \Exception( __( 'An error occurred', 'gugur' ) );
		}
		...
		return sprintf(
			...
			_n( '%d row affected.', '%d rows affected.', $rows_affected, 'gugur' ),
			$rows_affected
		);
	}
	...
	/**
	 * Is post supports gugur.
	 *
	 * Whether the post supports editing with gugur.
	 *
	 * @since 1.0.0
	 * @access public
	 * @static
	 ...
	 * @param int $post_id Optional. Post ID. Default is `0`.
	 *
	 * @return string True if post supports editing with gugur, false otherwise.
	 */
	public static function is_post_support( $post_id = 0 ) {
		 ...
		 *
		 * Filters whether the post type supports editing with gugur.
		 *
		 * @since 1.0.0
		 * @deprecated 2.2.0 Use `gugur/utils/is_post_support` Instead
		 *
		 * @param bool $is_supported Whether the post type supports editing with gugur.
		 * @param int $post_id Post ID.
		 * @param string $post_type Post type.
		 */
		$is_supported = apply_filters( 'gugur/utils/is_post_type_support', $is_supported, $post_id, $post_type );

		/**
		 * Is post support.
		 *
		 * Filters whether the post supports editing with gugur.
		 *
		 * @since 2.2.0
		 *
		 * @param bool $is_supported Whether the post type supports editing with gugur.
		 ...
		 * @param string $post_type Post type.
		 */
		$is_supported = apply_filters( 'gugur/utils/is_post_support', $is_supported, $post_id, $post_type );
	...
	/**
	 * Is post type supports gugur.
	 *
	 * Whether the post type supports editing with gugur.
	 *
	 * @since 2.2.0
	 * @access public
	 * @static
	 ...
	 * @param string $post_type Post Type.
	 *
	 * @return string True if post type supports editing with gugur, false otherwise.
	 */
	public static function is_post_type_support( $post_type ) {
		...
		if ( ! post_type_supports( $post_type, 'gugur' ) ) {
			return false;
		}
	 ...
	 * @static
	 *
	 * @return string The source of the default placeholder image used by gugur.
	 */
	public static function get_placeholder_image_src() {
		$placeholder_image = gugur_ASSETS_URL . 'images/placeholder.png';

		/**
		 * Get placeholder image source.
		 *
		 * Filters the source of the default placeholder image used by gugur.
		 *
		 * @since 1.0.0
		 *
		 * @param string $placeholder_image The source of the default placeholder image.
		 */
		$placeholder_image = apply_filters( 'gugur/utils/get_placeholder_image_src', $placeholder_image );
	 ...
	 *
	 * Retrieve a custom URL for creating a new post/page using gugur.
	 *
	 * @since 1.9.0
	 * @access public
	 * @static
	 ...
	 *
	 * @return string A URL for creating new post using gugur.
	 */
	public static function get_create_new_post_url( $post_type = 'page' ) {
		$new_post_url = add_query_arg( [
			'action' => 'gugur_new_post',
			'post_type' => $post_type,
		], admin_url( 'edit.php' ) );

		$new_post_url = add_query_arg( '_wpnonce', wp_create_nonce( 'gugur_action_new_post' ), $new_post_url );
	...
	public static function get_meta_viewport( $context = '' ) {
		 ...
		 */
		return apply_filters( 'gugur/template/viewport_tag', $meta_tag, $context );
	}

	/**
	 * Add gugur Config js vars to the relevant script handle,
	...
	public static function print_js_config( $handle, $js_var, $config ) {
		...
		if ( get_option( 'gugur_editor_break_lines' ) ) {
	...
	public static function handle_deprecation( $item, $version, $replacement = null ) {
		preg_match( '/^[0-9]+\.[0-9]+/', gugur_VERSION, $current_version );
