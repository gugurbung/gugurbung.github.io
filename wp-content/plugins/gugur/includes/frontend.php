...
use gugur\Core\Files\CSS\Global_CSS;
use gugur\Core\Files\CSS\Post as Post_CSS;
use gugur\Core\Files\CSS\Post_Preview;
 ...
 * gugur frontend.
 *
 * gugur frontend handler class is responsible for initializing gugur in
 * the frontend.
 *
 * @since 1.0.0
 */
class Frontend extends App {
	...
	/**
	 * Whether the page is using gugur.
	 *
	 * Used to determine whether the current page is using gugur.
	 *
	 * @since 1.0.0
	 * @access private
	 *
	 * @var bool Whether gugur is being used. Default is false.
	 */
	private $_has_gugur_in_page = false;
	 ...
	 * conflicted with gugur while gugur process the content.
	 *
	 * @since 1.0.0
	 * @access private
	 *
	 ...
	 *
	 * Initializing gugur front end. Make sure we are not in admin, not and
	 * redirect from old URL structure of gugur editor.
	 *
	 * @since 1.0.0
	 * @access public
	 */
	 ...
	 * Init.
	 *
	 * Initialize gugur front end. Hooks the needed actions to run gugur
	...
	public function init() {
		...
		if ( is_singular() && Plugin::$instance->db->is_built_with_gugur( $this->post_id ) ) {
		...
		// Add Edit with the gugur in Admin Bar.
	 ...
	 * Body tag classes.
	 *
	 * Add new gugur classes to the body tag.
	 *
	...
	public function body_class( $classes = [] ) {
		...
		if ( is_singular() && Plugin::$instance->db->is_built_with_gugur( $id ) ) {
			$classes[] = 'gugur-page gugur-page-' . $id;
		}
	 ...
	 * Add content filter.
	 *
	 * Remove plain content and render the content generated by gugur.
	 *
	 * @since 1.8.0
	 * @access public
	 */
	 ...
	 *
	 * When the gugur generated content rendered, we remove the filter to prevent multiple
	 * accuracies. This way we make sure gugur renders the content only once.
	 *
	 * @since 1.8.0
	 * @access public
	 */
	...
	public function register_scripts() {
		 ...
		 *
		 * Fires before gugur frontend scripts are registered.
		 *
		 * @since 1.2.1
		 */
		do_action( 'gugur/frontend/before_register_scripts' );
		 ...
		 *
		 * Fires after gugur frontend scripts are registered.
		 *
		 * @since 1.2.1
		 */
		do_action( 'gugur/frontend/after_register_scripts' );
	}
	...
	public function register_styles() {
		 ...
		 *
		 * Fires before gugur frontend styles are registered.
		 *
		 * @since 1.2.0
		 */
		do_action( 'gugur/frontend/before_register_styles' );
		...
		if ( $has_custom_file ) {
			...
			$frontend_file_url = gugur_ASSETS_URL . 'css/' . $frontend_file_name;
		}
		...
		wp_register_style(
			...
			$frontend_file_url,
			[],
			$has_custom_file ? null : gugur_VERSION
		);

		/**
		 * After frontend register styles.
		 *
		 * Fires after gugur frontend styles are registered.
		 *
		 * @since 1.2.0
		 */
		do_action( 'gugur/frontend/after_register_styles' );
	}
	...
	public function enqueue_scripts() {
		 ...
		 *
		 * Fires before gugur frontend scripts are enqueued.
		 *
		 * @since 1.0.0
		 */
		do_action( 'gugur/frontend/before_enqueue_scripts' );

		wp_enqueue_script( 'gugur-frontend' );
		 ...
		 *
		 * Fires after gugur frontend scripts are enqueued.
		 *
		 * @since 1.0.0
		 */
		do_action( 'gugur/frontend/after_enqueue_scripts' );
	}
	...
	public function enqueue_styles() {
		 ...
		 *
		 * Fires before gugur frontend styles are enqueued.
		 *
		 * @since 1.0.0
		 */
		do_action( 'gugur/frontend/before_enqueue_styles' );

		wp_enqueue_style( 'gugur-icons' );
		wp_enqueue_style( 'gugur-animations' );
		wp_enqueue_style( 'gugur-frontend' );

		/**
		 * After frontend styles enqueued.
		 *
		 * Fires after gugur frontend styles are enqueued.
		 *
		 * @since 1.0.0
		 */
		do_action( 'gugur/frontend/after_enqueue_styles' );
	...
	/**
	 * gugur footer scripts and styles.
	 *
		...
		foreach ( $this->fonts_to_enqueue as $key => $font ) {
			...
			switch ( $font_type ) {
				...
				default:
					 ...
					 * Print font links.
					 *
					 * Fires when gugur frontend fonts are printed on the HEAD tag.
					 *
					 ...
					 */
					do_action( "gugur/fonts/print_font_links/{$font_type}", $font );
			}
		}
	...
	private function enqueue_icon_fonts() {
		...
		foreach ( $this->icon_fonts_to_enqueue as $icon_type => $css_url ) {
			wp_enqueue_style( 'gugur-icons-' . $icon_type );
	...
	private function enqueue_google_fonts( $google_fonts = [] ) {
		 ...
		 */
		$print_google_fonts = apply_filters( 'gugur/frontend/print_google_fonts', $print_google_fonts );
	 ...
	 *
	 * Used to apply the gugur page editor on the post content.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	 ...
	 *
	 * Used to render and return the post content with all the gugur elements.
	 *
	...
	public function get_builder_content( $post_id, $with_css = false ) {
		...
		if ( ! Plugin::$instance->db->is_built_with_gugur( $post_id ) ) {
			return '';
		}
		 ...
		 * @param int   $post_id The post ID.
		 */
		$data = apply_filters( 'gugur/frontend/builder_content_data', $data, $post_id );
		 ...
		 */
		$content = apply_filters( 'gugur/frontend/the_content', $content );

		if ( ! empty( $content ) ) {
			$this->_has_gugur_in_page = true;
		}
	...
	public function add_menu_in_admin_bar( \WP_Admin_Bar $wp_admin_bar ) {
		...
		$menu_args = [
			'id' => 'gugur_edit_page',
			'title' => __( 'Edit with gugur', 'gugur' ),
		];
		...
		foreach ( $this->admin_bar_edit_documents as $document ) {
			$wp_admin_bar->add_menu( [
				'id' => 'gugur_edit_doc_' . $document->get_main_id(),
				'parent' => 'gugur_edit_page',
				'title' => sprintf( '<span class="gugur-edit-link-title">%s</span><span class="gugur-edit-link-type">%s</span>', $document->get_post()->post_title, $document::get_title() ),
	...
	public function get_builder_content_for_display( $post_id, $with_css = false ) {
		...
		if ( get_the_ID() === (int) $post_id ) {
			...
			if ( $editor->is_edit_mode() ) {
				$content = '<div class="gugur-alert gugur-alert-danger">' . __( 'Invalid Data: The Template ID cannot be the same as the currently edited template. Please choose a different one.', 'gugur' ) . '</div>';
			}
	 ...
	 *
	 * Remove WordPress default filters that conflicted with gugur.
	 *
	 * @since 1.5.0
	 * @access public
	 */
	 ...
	 * Has gugur In Page
	 *
	 * Determine whether the current page is using gugur.
	 *
	 * @since 2.0.9
	 *
	 * @access public
	 * @return bool
	 */
	public function has_gugur_in_page() {
		return $this->_has_gugur_in_page;
	}
	 ...
	 *
	 * Restore removed WordPress filters that conflicted with gugur.
	 *
	 * @since 1.5.0
	 * @access private
	 */
	...
	private function process_more_tag( $content ) {
		...
		if ( empty( $parts['more_text'] ) ) {
			$parts['more_text'] = __( '(more&hellip;)', 'gugur' );
		}
		...
		$more_link = apply_filters( 'the_content_more_link', sprintf( ' <a href="%s#more-%s" class="more-link gugur-more-link">%s</a>', get_permalink(), $post->ID, $more_link_text ), $more_link_text );
