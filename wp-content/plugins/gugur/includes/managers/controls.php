...
/**
 * gugur controls manager.
 *
 * gugur controls manager handler class is responsible for registering and
...
class Controls_Manager {
	...
	private static function init_tabs() {
		self::$tabs = [
			self::TAB_CONTENT => __( 'Content', 'gugur' ),
			self::TAB_STYLE => __( 'Style', 'gugur' ),
			self::TAB_ADVANCED => __( 'Advanced', 'gugur' ),
			self::TAB_RESPONSIVE => __( 'Responsive', 'gugur' ),
			self::TAB_LAYOUT => __( 'Layout', 'gugur' ),
			self::TAB_SETTINGS => __( 'Settings', 'gugur' ),
		];
	}
	 ...
	 * `gugur/controls/controls_registered` action.
	 *
	 * @since 1.0.0
	 * @access private
	 */
	private function register_controls() {
		 ...
		 *
		 * Fires after gugur controls are registered.
		 *
		 * @since 1.0.0
		 *
		 * @param Controls_Manager $this The controls manager.
		 */
		do_action( 'gugur/controls/controls_registered', $this );
	}
	 ...
	 * version of gugur uses this method to display an upgrade message to
	 * gugur Pro.
	 *
	 * @since 1.0.0
	 * @access public
	...
	public function add_custom_css_controls( Controls_Stack $controls_stack ) {
		...
		$controls_stack->add_control(
			...
			[
				'type' => self::RAW_HTML,
				'raw' => '<div class="gugur-nerd-box">' .
						'<i class="gugur-nerd-box-icon eicon-hypster" aria-hidden="true"></i>
						<div class="gugur-nerd-box-title">' .
							__( 'Meet Our Custom CSS', 'gugur' ) .
						'</div>
						<div class="gugur-nerd-box-message">' .
							__( 'Custom CSS lets you add CSS code to any widget, and see it render live right in the editor.', 'gugur' ) .
						'</div>
						<div class="gugur-nerd-box-message">' .
							__( 'This feature is only available on gugur Pro.', 'gugur' ) .
						'</div>
						<a class="gugur-nerd-box-link gugur-button gugur-button-default gugur-go-pro" href="' . Utils::get_pro_link( 'https://gugur.com/pro/?utm_source=panel-custom-css&utm_campaign=gopro&utm_medium=wp-dash' ) . '" target="_blank">' .
							__( 'Go Pro', 'gugur' ) .
						'</a>
						</div>',
			]
