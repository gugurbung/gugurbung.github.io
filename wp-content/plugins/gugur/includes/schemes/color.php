 ...
 * gugur color scheme.
 *
 * gugur color scheme class is responsible for initializing a scheme for
 * colors.
 *
 * @since 1.0.0
 */
class Scheme_Color extends Scheme_Base {
	...
	public function get_disabled_title() {
		return __( 'Color Palettes', 'gugur' );
	}
	...
	public function get_scheme_titles() {
		return [
			self::COLOR_1 => __( 'Primary', 'gugur' ),
			self::COLOR_2 => __( 'Secondary', 'gugur' ),
			self::COLOR_3 => __( 'Text', 'gugur' ),
			self::COLOR_4 => __( 'Accent', 'gugur' ),
		];
	}
	...
	public function print_template_content() {
		?>
		<div class="gugur-panel-scheme-content gugur-panel-box">
			<div class="gugur-panel-heading">
				<div class="gugur-panel-heading-title"><?php echo $this->_get_current_scheme_title(); ?></div>
			</div>
			<?php
			...
			if ( $description ) :
				?>
				<div class="gugur-panel-scheme-description gugur-descriptor"><?php echo $description; ?></div>
			<?php endif; ?>
			<div class="gugur-panel-scheme-items gugur-panel-box-content"></div>
		</div>
		<div class="gugur-panel-scheme-colors-more-palettes gugur-panel-box">
			<div class="gugur-panel-heading">
				<div class="gugur-panel-heading-title"><?php echo __( 'More Palettes', 'gugur' ); ?></div>
			</div>
			<div class="gugur-panel-box-content">
				<?php foreach ( $this->_get_system_schemes_to_print() as $scheme_name => $scheme ) : ?>
					<div class="gugur-panel-scheme-color-system-scheme" data-scheme-name="<?php echo esc_attr( $scheme_name ); ?>">
						<div class="gugur-panel-scheme-color-system-items">
							<?php foreach ( $scheme['items'] as $color_value ) : ?>
								<div class="gugur-panel-scheme-color-system-item" style="background-color: <?php echo esc_attr( $color_value ); ?>;"></div>
							<?php endforeach; ?>
						</div>
						<div class="gugur-title"><?php echo $scheme['title']; ?></div>
					</div>
				<?php endforeach; ?>
			</div>
		</div>
