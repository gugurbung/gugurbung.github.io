 ...
 * gugur video widget.
 *
 * gugur widget that displays a video player.
 *
 * @since 1.0.0
 */
class Widget_Video extends Widget_Base {
	...
	protected function _register_controls() {
		...
		$this->add_control(
			'video_type',
			[
				'label' => __( 'Source', 'gugur' ),
				'type' => Controls_Manager::SELECT,
				'default' => 'youtube',
				'options' => [
					'youtube' => __( 'YouTube', 'gugur' ),
					'vimeo' => __( 'Vimeo', 'gugur' ),
					'dailymotion' => __( 'Dailymotion', 'gugur' ),
					'hosted' => __( 'Self Hosted', 'gugur' ),
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'placeholder' => __( 'Enter your URL', 'gugur' ) . ' (YouTube)',
		...
		$this->add_control(
			...
			[
				...
				'placeholder' => __( 'Enter your URL', 'gugur' ) . ' (Vimeo)',
		...
		$this->add_control(
			...
			[
				...
				'placeholder' => __( 'Enter your URL', 'gugur' ) . ' (Dailymotion)',
		...
		$this->add_control(
			...
			[
				...
				'media_type' => 'video',
				'placeholder' => __( 'Enter your URL', 'gugur' ),
				'condition' => [
					'video_type' => 'hosted',
					'insert_url' => 'yes',
				],
		...
		$this->add_control(
			'controls',
			[
				'label' => __( 'Player Controls', 'gugur' ),
				'type' => Controls_Manager::SWITCHER,
		...
		$this->add_control(
			'modestbranding',
			[
				'label' => __( 'Modest Branding', 'gugur' ),
				'type' => Controls_Manager::SWITCHER,
				'condition' => [
					'video_type' => [ 'youtube' ],
					'controls' => 'yes',
		...
		$this->add_control(
			'yt_privacy',
			[
				'label' => __( 'Privacy Mode', 'gugur' ),
				'type' => Controls_Manager::SWITCHER,
				'description' => __( 'When you turn on privacy mode, YouTube won\'t store information about visitors on your website unless they play the video.', 'gugur' ),
				'condition' => [
					'video_type' => 'youtube',
				],
			]
		...
		$this->add_control(
			'rel',
			[
				'label' => __( 'Suggested Videos', 'gugur' ),
				'type' => Controls_Manager::SELECT,
				'options' => [
					'' => __( 'Current Video Channel', 'gugur' ),
					'yes' => __( 'Any Video', 'gugur' ),
				],
				'condition' => [
					'video_type' => 'youtube',
				],
		...
		$this->add_control(
			'download_button',
			[
				'label' => __( 'Download Button', 'gugur' ),
				'type' => Controls_Manager::SWITCHER,
				'label_off' => __( 'Hide', 'gugur' ),
				'label_on' => __( 'Show', 'gugur' ),
				'condition' => [
		...
		$this->add_group_control(
			...
			[
				'name' => 'css_filters',
				'selector' => '{{WRAPPER}} .gugur-wrapper',
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'fields_options' => [
					'text_shadow_type' => [
						'label' => _x( 'Shadow', 'Text Shadow Control', 'gugur' ),
					],
				],
				'condition' => [
					'show_image_overlay' => 'yes',
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'#gugur-lightbox-{{ID}}' => 'background-color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'#gugur-lightbox-{{ID}} .dialog-lightbox-close-button' => 'color: {{VALUE}}',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'#gugur-lightbox-{{ID}} .dialog-lightbox-close-button:hover' => 'color: {{VALUE}}',
				],
				'separator' => 'after',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'(desktop+)#gugur-lightbox-{{ID}} .gugur-video-container' => 'width: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				'label' => __( 'Content Position', 'gugur' ),
				'type' => Controls_Manager::SELECT,
				'frontend_available' => true,
				'options' => [
					'' => __( 'Center', 'gugur' ),
				...
				'selectors' => [
					'#gugur-lightbox-{{ID}} .gugur-video-container' => '{{VALUE}}; transform: translateX(-50%);',
				],
	...
	protected function render() {
		...
		$this->add_render_attribute( 'video-wrapper', 'class', 'gugur-wrapper' );

		if ( ! $settings['lightbox'] ) {
			$this->add_render_attribute( 'video-wrapper', 'class', 'gugur-fit-aspect-ratio' );
		}

		$this->add_render_attribute( 'video-wrapper', 'class', 'gugur-open-' . ( $settings['lightbox'] ? 'lightbox' : 'inline' ) );
		?>
		<div <?php echo $this->get_render_attribute_string( 'video-wrapper' ); ?>>
			...
			if ( $this->has_image_overlay() ) {
				$this->add_render_attribute( 'image-overlay', 'class', 'gugur-custom-embed-image-overlay' );
				...
				if ( $settings['lightbox'] ) {
					...
					$this->add_render_attribute( 'image-overlay', [
						'data-gugur-open-lightbox' => 'yes',
						'data-gugur-lightbox' => wp_json_encode( $lightbox_options ),
					] );
				...
				<div <?php echo $this->get_render_attribute_string( 'image-overlay' ); ?>>
					...
					<?php if ( 'yes' === $settings['show_play_icon'] ) : ?>
						<div class="gugur-custom-embed-play" role="button">
							<i class="eicon-play" aria-hidden="true"></i>
							<span class="gugur-screen-only"><?php echo __( 'Play Video', 'gugur' ); ?></span>
						</div>
					<?php endif; ?>
				</div>
			<?php } ?>
	...
	private function render_hosted_video() {
		...
		?>
		<video class="gugur-video" src="<?php echo esc_url( $video_url ); ?>" <?php echo Utils::render_html_attributes( $video_params ); ?>></video>
		<?php
	}
}
