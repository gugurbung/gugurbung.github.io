...
/**
 * gugur Deprecated Notice control.
 *
...
class Control_Deprecated_Notice extends Base_UI_Control {
	...
	public function content_template() {
		...
		let notice = gugur.translate( 'deprecated_notice', [ data.widget, data.plugin, data.since ] );
		if ( data.replacement ) {
			notice += '<br>' + gugur.translate( 'deprecated_notice_replacement', [ data.replacement ] );
		}
		if ( data.last ) {
			notice += '<br>' + gugur.translate( 'deprecated_notice_last', [ data.widget, data.plugin, data.last ] );
		}
		#>
		<div class="gugur-control-deprecated-notice gugur-panel-alert gugur-panel-alert-warning">{{{ notice }}}</div>
		<?php
	}
