...
use gugur\Core\DocumentTypes\PageBase;
 ...
 * gugur compatibility.
 *
 * gugur compatibility handler class is responsible for compatibility with
...
class Compatibility {
	 ...
	 * Register actions.
	 *
	 * Run gugur compatibility with external plugins using custom filters and
	 * actions.
	 *
	 * @since 1.0.0
	 * @access public
	...
	public static function register_actions() {
		...
		add_action( 'gugur/maintenance_mode/mode_changed', [ __CLASS__, 'clear_3rd_party_cache' ] );

		add_action( 'gugur/element/before_section_start', [ __CLASS__, 'document_post_deprecated_hooks' ], 10, 3 );
		add_action( 'gugur/element/after_section_start', [ __CLASS__, 'document_post_deprecated_hooks' ], 10, 3 );
		add_action( 'gugur/element/before_section_end', [ __CLASS__, 'document_post_deprecated_hooks' ], 10, 3 );
		add_action( 'gugur/element/after_section_end', [ __CLASS__, 'document_post_deprecated_hooks' ], 10, 3 );
	}
	...
	public static function document_post_deprecated_hooks( $instance, $section_id, $args ) {
		...
		$deprecated_action = "gugur/element/post/{$section_id}/{$current_sub_action}";
		...
		// $replacement = "`gugur/element/wp-post/{$section_id}/{$current_sub_action}` or `gugur/element/wp-page/{$section_id}/{$current_sub_action}`";
	 ...
	 *
	 * Insert new "gugur" button to the gutenberg editor to create new post
	 * using gugur page builder.
	 *
	 * @since 1.9.0
	 * @access public
	 * @static
	...
	public static function add_new_button_to_gutenberg() {
		...
		<script type="text/javascript">
			document.addEventListener( 'DOMContentLoaded', function() {
				...
				dropdown.insertAdjacentHTML( 'afterbegin', '<a href="' + url + '">gugur</a>' );
			} );
		</script>
		<?php
	}
	 ...
	 * Init.
	 *
	 * Initialize gugur compatibility with external plugins.
	 *
	...
	public static function init() {
		...
		if ( defined( 'NGG_PLUGIN_VERSION' ) ) {
			add_filter( 'gugur/document/urls/edit', function( $edit_link ) {
		...
		if ( class_exists( '\Ninja_Forms' ) && class_exists( '\NF_Display_Render' ) ) {
			add_action( 'gugur/preview/enqueue_styles', function() {
				ob_start();
		...
		// Add the description (content) tab for a new product, so it can be edited with gugur.
		add_filter( 'woocommerce_product_tabs', function( $tabs ) {
			if ( ! isset( $tabs['description'] ) && Plugin::$instance->preview->is_preview_mode() ) {
				$post = get_post();
				if ( empty( $post->post_content ) ) {
					$tabs['description'] = [
						'title' => __( 'Description', 'gugur' ),
						'priority' => 10,
		...
		if ( class_exists( 'woocommerce' ) ) {
			add_action( 'gugur/editor/before_enqueue_scripts', function() {
			...
			add_filter( 'gugur/maintenance_mode/is_login_page', function( $value ) {
		...
		if ( class_exists( 'domain_map' ) ) {
			add_filter( 'gugur/document/urls/preview', function( $preview_url ) {
	 ...
	 *
	 * Fix Polylang compatibility with gugur.
	 *
	 * @since 2.0.0
	 * @access private
	 * @static
	...
	private static function polylang_compatibility() {
		...
		if ( isset( $_REQUEST['action'] ) && 0 === strpos( $_REQUEST['action'], 'gugur' ) ) {
		...
		// Copy gugur data while polylang creates a translation copy
	...
	public static function save_polylang_meta( $keys, $sync, $from, $to ) {
		...
		if ( ! $sync ) {
			Plugin::$instance->db->copy_gugur_meta( $from, $to );
		}
	 ...
	 *
	 * Normalize gugur post meta on import, We need the `wp_slash` in order
	 ...
	 *
	 * Normalize gugur post meta on import with the new WP_importer, We need
