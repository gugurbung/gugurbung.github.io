...
use gugur\Core\Common\Modules\Connect\Admin;
...
abstract class Base_App {
	...
	const OPTION_NAME_PREFIX = 'gugur_connect_';

	const SITE_URL = 'https://my.gugur.com/connect/v1';

	const API_URL = 'https://my.gugur.com/api/connect/v1';
	...
	public function action_get_token() {
		...
		$this->add_notice( __( 'Connected Successfully.', 'gugur' ) );
	...
	public function action_disconnect() {
		if ( $this->is_connected() ) {
			$this->disconnect();
			$this->add_notice( __( 'Disconnected Successfully.', 'gugur' ) );
		}
	...
	protected function request( $action, $request_body = [] ) {
		...
		if ( $this->is_connected() ) {
			$headers['X-gugur-Signature'] = hash_hmac( 'sha256', wp_json_encode( $request_body, JSON_NUMERIC_CHECK ), $this->get( 'access_token_secret' ) );
		}
	...
	protected function print_popup_close_script( $url ) {
		...
		<script>
			if ( opener && opener !== window ) {
				opener.jQuery( 'body' ).trigger( 'gugurConnected' );
				window.close();
				opener.focus();
			} else {
	...
	protected function get_site_key() {
		$site_key = get_option( 'gugur_connect_site_key' );

		if ( ! $site_key ) {
			$site_key = md5( uniqid( wp_generate_password() ) );
			update_option( 'gugur_connect_site_key', $site_key );
		}
