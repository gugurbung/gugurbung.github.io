...
use gugur\Core\DocumentTypes\Page;
...
/**
 * gugur documents manager.
 *
 * gugur documents manager handler class is responsible for registering and
 * managing gugur documents.
 *
 * @since 2.0.0
 */
class Documents_Manager {
	 ...
	 *
	 * Initializing the gugur documents manager.
	 *
	 * @since 2.0.0
	 * @access public
	 */
	public function __construct() {
		add_action( 'gugur/documents/register', [ $this, 'register_default_types' ], 0 );
	...
	public function get( $post_id, $from_cache = true ) {
		...
		$post_id = apply_filters( 'gugur/documents/get/post_id', $post_id );
	...
	public function create( $type, $post_data = [], $meta_data = [] ) {
		...
		if ( empty( $post_data['post_title'] ) ) {
			$post_data['post_title'] = __( 'gugur', 'gugur' );
			if ( 'post' !== $type ) {
		...
		$meta_data['_gugur_edit_mode'] = 'builder';
	 ...
	 *
	 * @throws \Exception If current user don't have permissions to edit the post or the post is not using gugur.
	 *
	...
	public function ajax_save( $request ) {
		...
		if ( ! $document->is_built_with_gugur() || ! $document->is_editable_by_current_user() ) {
		 ...
		 */
		$return_data = apply_filters( 'gugur/documents/ajax_save/return_data', $return_data, $document );
	...
	private function register_types() {
		if ( ! did_action( 'gugur/documents/register' ) ) {
			/**
			 * Register gugur documents.
			 *
			 * @since 2.0.0
			 *
			 * @param Documents_Manager $this The document manager instance.
			 */
			do_action( 'gugur/documents/register', $this );
		}
	}
}
