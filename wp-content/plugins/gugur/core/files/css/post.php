 ...
 * gugur post CSS file.
 *
 * gugur CSS file handler class is responsible for generating the single
 * post CSS file.
 *
 * @since 1.2.0
 */
class Post extends Base {
	...
	/**
	 * gugur post CSS file prefix.
	 */
	const FILE_PREFIX = 'post-';

	const META_KEY = '_gugur_css';
	...
	public function get_element_unique_selector( Element_Base $element ) {
		return '.gugur-' . $this->post_id . ' .gugur-element' . $element->get_unique_selector();
	}
	 ...
	 * Enqueue CSS.
	 *
	 * Enqueue the post CSS file in gugur.
	 *
	 * This method ensures that the post was actually built with gugur before
	...
	public function enqueue() {
		if ( ! Plugin::$instance->db->is_built_with_gugur( $this->post_id ) ) {
			return;
		}
	...
	protected function get_file_handle_id() {
		return 'gugur-post-' . $this->post_id;
	}
	...
	protected function render_styles( Element_Base $element ) {
		 ...
		 * @param Element_Base $element The element.
		 */
		do_action( 'gugur/element/before_parse_css', $this, $element );
		 ...
		 * @param Element_Base $element The element.
		 */
		do_action( 'gugur/element/parse_css', $this, $element );
	}
}
