...
use gugur\Modules\Usage\Module;
use gugur\Plugin;
 ...
 * gugur upgrades.
 *
 * gugur upgrades handler class is responsible for updating different
 * gugur versions.
 *
 * @since 1.0.0
 */
class Upgrades {
	...
	public static function _v_2_1_0() {
		...
		$post_ids = $wpdb->get_col(
			'SELECT `post_id` FROM `' . $wpdb->postmeta . '` WHERE `meta_key` = "_gugur_data" AND `meta_value` LIKE \'%"widgetType":"video"%\';'
		);
	...
	public static function _v_2_3_0_widget_image( $updater ) {
		...
		$post_ids = $updater->query_col(
			'SELECT `post_id` FROM `' . $wpdb->postmeta . '` WHERE `meta_key` = "_gugur_data" AND (
	...
	public static function _v_2_3_0_template_type( $updater ) {
		...
		$post_ids = $updater->query_col(
			'SELECT p.ID
					...
					LEFT JOIN `' . $wpdb->postmeta . '` AS pm2 ON (pm1.post_id = pm2.post_id AND pm2.meta_key = "_gugur_template_type")
					WHERE p.post_status != "inherit" AND pm1.`meta_key` = "_gugur_data" AND pm2.post_id IS NULL;'
		);
	...
	public static function _v_2_6_0_fa4_migration_flag() {
		add_option( 'gugur_icon_manager_needs_update', 'yes' );
		add_option( 'gugur_load_fa4_shim', 'yes' );
	}
	...
	public static function _v_2_6_6_fa4_migration_button( $updater ) {
		$changes = [
			[
				'callback' => [ 'gugur\Core\Upgrade\Upgrades', '_migrate_icon_fa4_value' ],
				'control_ids' => [
					'icon' => 'selected_icon',
				],
			],
	...
	public static function rename_document_base_to_wp( $updater, $type ) {
		...
		$wpdb->query( $wpdb->prepare(
			...
			WHERE meta_key = '_gugur_template_type' && post_id in ( %s );
	 ...
	 * Since 2.7.1 the key was changed from `gugur_elements_usage` to `gugur_controls_usage`.
	 */
	public static function _v_2_7_1_remove_old_usage_data() {
		delete_option( 'gugur_elements_usage' );
		delete_post_meta_by_key( '_gugur_elements_usage' );
	}
