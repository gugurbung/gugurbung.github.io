...
use gugur\Core\Settings\Base\Manager as BaseManager;
...
/**
 * gugur general settings manager.
 *
 * gugur general settings manager handler class is responsible for registering
 * and managing gugur general settings managers.
 *
 * @since 1.6.0
 */
class Manager extends BaseManager {
	 ...
	 */
	const META_KEY = '_gugur_general_settings';

	/**
	 * General settings manager constructor.
	 *
	 * Initializing gugur general settings manager.
	 *
	 * @since 1.6.0
	 * @access public
	 */
			...
			foreach ( $sections as $section_name => $section_data ) {
				...
				foreach ( $section_data['controls'] as $control_name => $control_data ) {
					if ( isset( $settings[ $control_name ] ) ) {
						$one_list_control_name = str_replace( 'gugur_', '', $control_name );
	...
	private function add_panel_tabs() {
		Controls_Manager::add_tab( self::PANEL_TAB_LIGHTBOX, __( 'Lightbox', 'gugur' ) );
	}
}
