...
var _module = _interopRequireDefault(__webpack_require__(504));

var _introduction = _interopRequireDefault(__webpack_require__(505));

var _controlsStack = _interopRequireDefault(__webpack_require__(506));

var _baseSettings = _interopRequireDefault(__webpack_require__(507));
...
var EditorModule = gugurModules.Module.extend({
  onInit: function onInit() {
    jQuery(window).on('gugur:init', this.ongugurReady);
  },
  ...
  ongugurReady: function ongugurReady() {
    this.ongugurInit();
    gugur.on('frontend:init', this.ongugurFrontendInit.bind(this)).on('preview:loaded', this.ongugurPreviewLoaded.bind(this));
  }
});

EditorModule.prototype.ongugurInit = function () {};

EditorModule.prototype.ongugurPreviewLoaded = function () {};

EditorModule.prototype.ongugurFrontendInit = function () {};
...
function (_gugurModules$Mod) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function initDialog() {
      ...
      this.getDialog = function () {
        if (!dialog) {
          ...
          dialog = gugurCommon.dialogsManager.createWidget(settings.dialogType, settings.dialogOptions);
    ...
    value: function setViewed() {
      ...
      gugurCommon.ajax.addRequest('introduction_viewed', {
        data: {
...
ControlsStack = Marionette.CompositeView.extend({
  classes: {
    popover: 'gugur-controls-popover'
  },
  activeTab: null,
  activeSection: null,
  className: function className() {
    return 'gugur-controls-stack';
  },
  templateHelpers: function templateHelpers() {
    return {
      elementData: gugur.getElementData(this.model)
    };
  },
  ...
  ui: function ui() {
    return {
      ...
      reloadButton: '.gugur-update-preview-button'
    };
  },
  ...
  initCollection: function initCollection() {
    this.collection = new Backbone.Collection(_.values(gugur.mergeControlsSettings(this.getOption('controls'))));
  },
  ...
  getNamespaceArray: function getNamespaceArray() {
    return [gugur.getPanelView().getCurrentPageName()];
  },
  openActiveSection: function openActiveSection() {
    ...
    if (activeSectionView[0]) {
      activeSectionView[0].$el.addClass('gugur-open');
      var eventNamespace = this.getNamespaceArray();
      eventNamespace.push(activeSection, 'activated');
      gugur.channels.editor.trigger(eventNamespace.join(':'), this);
    }
  },
  onRenderCollection: function onRenderCollection() {
  ...
  onDeviceModeChange: function onDeviceModeChange() {
    if ('desktop' === gugur.channels.deviceMode.request('currentMode')) {
      this.$el.removeClass('gugur-responsive-switchers-open');
    }
  },
  onChildviewControlSectionClicked: function onChildviewControlSectionClicked(childView) {
    var isSectionOpen = childView.$el.hasClass('gugur-open');
  ...
  onChildviewResponsiveSwitcherClick: function onChildviewResponsiveSwitcherClick(childView, device) {
    if ('desktop' === device) {
      this.$el.toggleClass('gugur-responsive-switchers-open');
    }
  }
});
...
BaseSettingsModel = Backbone.Model.extend({
  ...
  initialize: function initialize(data, options) {
    ...
    self.controls = gugur.mergeControlsSettings(options.controls);
    self.validators = {};
    ...
    _.each(self.controls, function (control) {
      ...
      if ('object' === (0, _typeof2.default)(control.default)) {
        defaults[controlName] = gugurCommon.helpers.cloneObject(control.default);
      } else {
      ...
      if (undefined !== attrs[controlName] && isMultipleControl && !_.isObject(attrs[controlName]) && !hasDynamicSettings) {
        gugur.debug.addCustomError(new TypeError('An invalid argument supplied as multiple control value'), 'InvalidElementData', 'Element `' + (self.get('widgetType') || self.get('elType')) + '` got <' + attrs[controlName] + '> as `' + controlName + '` value. Expected array or object.');
        ...
        if (!(attrs[field.name] instanceof Backbone.Collection)) {
          attrs[field.name] = new Backbone.Collection(attrs[field.name], {
            model: function model(attributes, options) {
              ...
              if (!attributes._id) {
                attributes._id = gugur.helpers.getUniqueID();
              }
  ...
  getStyleControls: function getStyleControls(controls, attributes) {
    var self = this;
    controls = gugurCommon.helpers.cloneObject(self.getActiveControls(controls, attributes));
    var styleControls = [];
    jQuery.each(controls, function () {
      var control = this,
          controlDefaultSettings = gugur.config.controls[control.type];
  ...
  getActiveControls: function getActiveControls(controls, attributes) {
    ...
    jQuery.each(controls, function (controlKey, control) {
      if (gugur.helpers.isActiveControl(control, attributes)) {
  ...
  clone: function clone() {
    return new BaseSettingsModel(gugurCommon.helpers.cloneObject(this.attributes), gugurCommon.helpers.cloneObject(this.options));
  },
  ...
  parseDynamicSettings: function parseDynamicSettings(settings, options, controls) {
    var self = this;
    settings = gugurCommon.helpers.cloneObject(settings || self.attributes);
    options = options || {};
    controls = controls || this.controls;
    jQuery.each(controls, function () {
      var control = this,
      ...
      if (undefined === dynamicSettings) {
        dynamicSettings = gugur.config.controls[control.type].dynamic;
      }
      ...
      try {
        dynamicValue = gugur.dynamicTags.parseTagsText(valueToParse, dynamicSettings, gugur.dynamicTags.getTagDataContent);
      } catch (error) {
        if (gugur.dynamicTags.CACHE_KEY_NOT_FOUND_ERROR !== error.message) {
          throw error;
        }
        ...
        gugur.dynamicTags.refreshCacheFromServer(function () {
  ...
  toJSON: function toJSON(options) {
    ...
    return gugurCommon.helpers.cloneObject(data);
  }
});
