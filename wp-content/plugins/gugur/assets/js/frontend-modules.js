...
var _default = window.gugurModules = {
...
var _stretchElement = _interopRequireDefault(__webpack_require__(509));

var _base = _interopRequireDefault(__webpack_require__(510));
...
module.exports = gugurModules.ViewModule.extend({
  getDefaultSettings: function getDefaultSettings() {
    return {
      element: null,
      direction: gugurFrontend.config.is_rtl ? 'right' : 'left',
      selectors: {
        container: window
      }
    };
...
module.exports = gugurModules.ViewModule.extend({
  ...
  __construct: function __construct(settings) {
    ...
    this.isEdit = this.$element.hasClass('gugur-element-edit-mode');
  ...
  findElement: function findElement(selector) {
    ...
    return $mainElement.find(selector).filter(function () {
      return jQuery(this).closest('.gugur-element').is($mainElement);
    });
  },
  ...
  initEditorListeners: function initEditorListeners() {
    ...
    ['page', 'general'].forEach(function (settingsType) {
      ...
      if (self[listenerMethodName]) {
        self.editorListeners.push({
          event: 'change',
          to: gugur.settings[settingsType].model,
  ...
  addEditorListeners: function addEditorListeners() {
    ...
    this.getEditorListeners().forEach(function (listener) {
      gugurFrontend.addListenerOnce(uniqueHandlerID, listener.event, listener.callback, listener.to);
    });
  },
  removeEditorListeners: function removeEditorListeners() {
    var uniqueHandlerID = this.getUniqueHandlerID();
    this.getEditorListeners().forEach(function (listener) {
      gugurFrontend.removeListeners(uniqueHandlerID, listener.event, null, listener.to);
    });
  },
  ...
  getElementSettings: function getElementSettings(setting) {
    ...
    if (this.isEdit && modelCID) {
      var settings = gugurFrontend.config.elements.data[modelCID],
      ...
      var settingsKeys = gugurFrontend.config.elements.keys[type];

      if (!settingsKeys) {
        settingsKeys = gugurFrontend.config.elements.keys[type] = [];
  ...
  getEditSettings: function getEditSettings(setting) {
    ...
    if (this.isEdit) {
      attributes = gugurFrontend.config.elements.editSettings[this.getModelCID()].attributes;
    }

    return this.getItems(attributes, setting);
  },
  getCurrentDeviceSetting: function getCurrentDeviceSetting(settingKey) {
    return gugurFrontend.getCurrentDeviceSetting(this.getElementSettings(), settingKey);
  },
