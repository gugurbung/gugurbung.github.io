...
ControlBaseDataView = ControlBaseView.extend({
  ui: function ui() {
    ...
    _.extend(ui, {
      ...
      responsiveSwitchers: '.gugur-responsive-switcher',
  ...
  behaviors: function behaviors() {
    ...
    if (dynamicSettings && dynamicSettings.active) {
      var tags = _.filter(gugur.dynamicTags.getConfig('tags'), function (tag) {
  ...
  onResponsiveSwitchersClick: function onResponsiveSwitchersClick(event) {
    ...
    gugur.changeDeviceMode(device);
  },
  renderResponsiveSwitchers: function renderResponsiveSwitchers() {
    var templateHtml = Marionette.Renderer.render('#tmpl-gugur-control-responsive-switchers', this.model.attributes);
...
ControlBaseMultipleItemView = ControlBaseDataView.extend({
  ...
  getControlValue: function getControlValue(key) {
    ...
    return gugurCommon.helpers.cloneObject(values);
  },
...
ControlSelect2ItemView = ControlBaseDataView.extend({
  ...
  getSelect2DefaultOptions: function getSelect2DefaultOptions() {
    return {
      ...
      dir: gugurCommon.config.isRTL ? 'rtl' : 'ltr'
    };
  },
...
ControlBaseView = Marionette.CompositeView.extend({
  ui: function ui() {
    return {
      controlTitle: '.gugur-control-title'
    };
  },
  behaviors: function behaviors() {
    var behaviors = {};
    return gugur.hooks.applyFilters('controls/base/behaviors', behaviors, this);
  },
  ...
  className: function className() {
    ...
    var classes = 'gugur-control gugur-control-' + this.model.get('name') + ' gugur-control-type-' + this.model.get('type'),
    ...
    if (!_.isEmpty(responsive)) {
      classes += ' gugur-control-responsive-' + responsive.max;
    }
  ...
  getTemplate: function getTemplate() {
    return Marionette.TemplateCache.get('#tmpl-gugur-control-' + this.model.get('type') + '-content');
  },
  initialize: function initialize(options) {
    this.elementSettingsModel = options.elementSettingsModel;
    var controlType = this.model.get('type'),
        controlSettings = jQuery.extend(true, {}, gugur.config.controls[controlType], this.model.attributes);
  ...
  toggleControlVisibility: function toggleControlVisibility() {
    var isVisible = gugur.helpers.isActiveControl(this.model, this.elementSettingsModel.attributes);
    this.$el.toggleClass('gugur-hidden-control', !isVisible);
    gugur.getPanelView().updateScrollbar();
  },
  onRender: function onRender() {
    var layoutType = this.model.get('label_block') ? 'block' : 'inline',
        showLabel = this.model.get('show_label'),
        elClasses = 'gugur-label-' + layoutType;
    elClasses += ' gugur-control-separator-' + this.model.get('separator');

    if (!showLabel) {
      elClasses += ' gugur-control-hidden-label';
    }
...
BaseElementView = BaseContainer.extend({
  ...
  renderAttributes: {},
  className: function className() {
    var classes = 'gugur-element gugur-element-edit-mode ' + this.getElementUniqueID();

    if (this.toggleEditTools) {
      classes += ' gugur-element--toggle-edit-tools';
    }
  ...
  ui: function ui() {
    return {
      tools: '> .gugur-element-overlay > .gugur-editor-element-settings',
      editButton: '> .gugur-element-overlay .gugur-editor-element-edit',
      duplicateButton: '> .gugur-element-overlay .gugur-editor-element-duplicate',
      addButton: '> .gugur-element-overlay .gugur-editor-element-add',
      removeButton: '> .gugur-element-overlay .gugur-editor-element-remove'
    };
  },
  behaviors: function behaviors() {
    var groups = gugur.hooks.applyFilters('elements/' + this.options.model.get('elType') + '/contextMenuGroups', this.getContextMenuGroups(), this);
    var behaviors = {
      contextMenu: {
    ...
    return gugur.hooks.applyFilters('elements/base/behaviors', behaviors, this);
  },
  getBehavior: function getBehavior(name) {
    return this._behaviors[(0, _keys.default)(this.behaviors()).indexOf(name)];
  },
  ...
  getChildType: function getChildType() {
    return gugur.helpers.getElementChildType(this.getElementType());
  },
  getChildView: function getChildView(model) {
    ...
    if ('section' === elType) {
      ...
      ChildView = gugur.modules.elements.views.Widget;
    }

    return gugur.hooks.applyFilters('element/view', ChildView, model, this);
  },
  ...
  getContextMenuGroups: function getContextMenuGroups() {
    ...
    return [{
      ...
      actions: [{
        name: 'edit',
        icon: 'eicon-edit',
        title: gugur.translate('edit_element', [this.options.model.getTitle()]),
        callback: this.options.model.trigger.bind(this.options.model, 'request:edit')
      }, {
        name: 'duplicate',
        icon: 'eicon-clone',
        title: gugur.translate('duplicate'),
      ...
      actions: [{
        name: 'copy',
        title: gugur.translate('copy'),
        shortcut: controlSign + '+C',
        callback: this.copy.bind(this)
      }, {
        name: 'paste',
        title: gugur.translate('paste'),
        shortcut: controlSign + '+V',
        callback: this.paste.bind(this),
        isEnabled: this.isPasteEnabled.bind(this)
      }, {
        name: 'pasteStyle',
        title: gugur.translate('paste_style'),
        shortcut: controlSign + '+⇧+V',
        callback: this.pasteStyle.bind(this),
        isEnabled: function isEnabled() {
          return !!gugurCommon.storage.get('transfer');
        }
      }, {
        name: 'resetStyle',
        title: gugur.translate('reset_style'),
  ...
  getHandlesOverlay: function getHandlesOverlay() {
    var $handlesOverlay = jQuery('<div>', {
      class: 'gugur-element-overlay'
    }),
        $overlayList = jQuery('<ul>', {
      class: "gugur-editor-element-settings gugur-editor-".concat(this.getElementType(), "-settings")
    });
    jQuery.each(this.getEditButtons(), function (toolName, tool) {
      var $item = jQuery('<li>', {
        class: "gugur-editor-element-setting gugur-editor-element-".concat(toolName),
        title: tool.title
      }),
          $icon = jQuery('<i>', {
  ...
  startTransport: function startTransport(type) {
    gugurCommon.storage.set('transfer', {
      type: type,
  ...
  isPasteEnabled: function isPasteEnabled() {
    var transferData = gugurCommon.storage.get('transfer');
  ...
  duplicate: function duplicate() {
    var oldTransport = gugurCommon.storage.get('transfer');
    this.copy();
    this.paste();
    gugurCommon.storage.set('transfer', oldTransport);
  },
  pasteStyle: function pasteStyle() {
    var self = this,
        transferData = gugurCommon.storage.get('transfer'),
    ...
    jQuery.each(controls, function (controlName, control) {
      ...
      var ControlView = gugur.getControlView(control.type);
    ...
    gugur.channels.data.trigger('element:before:paste:style', editModel);
    editModel.setSetting(diffSettings);
    gugur.channels.data.trigger('element:after:paste:style', editModel);
  ...
  resetStyle: function resetStyle() {
    var self = this,
        ...
        defaultValues = {};
    self.allowRender = false;
    gugur.channels.data.trigger('element:before:reset:style', editModel);
    jQuery.each(controls, function (controlName, control) {
      if (!self.isStyleTransferControl(control)) {
        return;
      }
    ...
    gugur.channels.data.trigger('element:after:reset:style', editModel);
    self.allowRender = true;
    self.renderOnChange();
  },
  ...
  toggleVisibilityClass: function toggleVisibilityClass() {
    this.$el.toggleClass('gugur-edit-hidden', !!this.model.get('hidden'));
  },
  addElementFromPanel: function addElementFromPanel(options) {
    options = options || {};
    var elementView = gugur.channels.panelElements.request('element:selected');
    var itemData = {
      elType: elementView.model.get('elType')
    };

    if (gugur.helpers.maybeDisableWidget()) {
      return;
    }
  ...
  enqueueFonts: function enqueueFonts() {
    ...
    jQuery.each(settings.getFontControls(), function (index, control) {
      ...
      gugur.helpers.enqueueFont(fontFamilyName);
    }); // Enqueue Icon Fonts
    ...
    jQuery.each(settings.getIconsControls(), function (index, control) {
      ...
      gugur.helpers.enqueueIconFonts(iconType.library);
    });
  },
  renderStyles: function renderStyles(settings) {
    ...
    this.controlsCSSParser.addStyleRules(settings.getStyleControls(), settings.attributes, this.getEditModel().get('settings').controls, [/{{ID}}/g, /{{WRAPPER}}/g], [this.getID(), '#gugur .' + this.getElementUniqueID()]);
    this.controlsCSSParser.addStyleToDocument();
    var extraCSS = gugur.hooks.applyFilters('editor/style/styleText', '', this);
  ...
  renderCustomClasses: function renderCustomClasses() {
    ...
    _.each(classControls, function (control) {
      ...
      var isVisible = gugur.helpers.isActiveControl(control, settings.attributes);
  ...
  runReadyTrigger: function runReadyTrigger() {
    ...
    _.defer(function () {
      gugurFrontend.elementsHandler.runReadyTrigger(self.el);

      if (!gugurFrontend.isEditMode()) {
        return;
      } // In edit mode - handle an external elements that loaded by another elements like shortcode etc.


      self.$el.find('.gugur-element.gugur-' + self.model.get('elType') + ':not(.gugur-element-edit-mode)').each(function () {
        gugurFrontend.elementsHandler.runReadyTrigger(this);
      });
    });
  },
  ...
  getElementUniqueID: function getElementUniqueID() {
    return 'gugur-element-' + this.getID();
  },
  renderOnChange: function renderOnChange(settings) {
    ...
    if (settings instanceof gugurModules.editor.elements.models.BaseSettings) {
  ...
  removeElement: function removeElement() {
    gugur.channels.data.trigger('element:before:remove', this.model);
    var parent = this._parent;
    parent.isManualRemoving = true;
    this.model.destroy();
    parent.isManualRemoving = false;
    gugur.channels.data.trigger('element:after:remove', this.model);
  },
  ...
  onRender: function onRender() {
    ...
    if (this.toggleEditTools) {
      ...
      this.ui.tools.hoverIntent(function () {
        editButton.addClass('gugur-active');
      }, function () {
        editButton.removeClass('gugur-active');
      }, {
        timeout: 500
      });
    }
  ...
  onCollectionChanged: function onCollectionChanged() {
    gugur.saver.setFlagEditorChange(true);
  },
  onEditSettingsChanged: function onEditSettingsChanged(changedModel) {
    gugur.channels.editor.trigger('change:editSettings', changedModel, this);
  },
  ...
  onEditRequest: function onEditRequest() {
    ...
    if ('edit' !== gugur.channels.dataEditMode.request('activeMode')) {
      return;
    }

    var model = this.getEditModel(),
        panel = gugur.getPanelView();
    ...
    if (options.scrollIntoView) {
      gugur.helpers.scrollToView(this.$el, 200);
    }
  ...
  onMouseDown: function onMouseDown(event) {
    if (jQuery(event.target).closest('.gugur-inline-editing').length) {
      return;
    }

    gugurFrontend.elements.window.document.activeElement.blur();
  },
  onDestroy: function onDestroy() {
    this.controlsCSSParser.removeStyleFromDocument();
    this.getEditModel().get('settings').validators = {};
    gugur.channels.data.trigger('element:destroy', this.model);
  }
});
...
module.exports = Marionette.Behavior.extend({
  ...
  initContextMenu: function initContextMenu() {
    ...
    contextMenuGroups.splice(afterGroupIndex, 0, {
      ...
      actions: [{
        name: 'navigator',
        title: gugur.translate('navigator'),
  ...
  onContextMenu: function onContextMenu(event) {
    if ($e.shortcuts.isControlEvent(event) || !gugur.userCan('design')) {
      return;
    }
    ...
    gugur.channels.editor.reply('contextMenu:targetView', this.view);
  },
  ...
  onContextMenuHide: function onContextMenuHide() {
    gugur.channels.editor.reply('contextMenu:targetView', null);
  },
  onDestroy: function onDestroy() {
    if (this.contextMenu) {
...
function (_Marionette$LayoutVie) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function initModal() {
      var modalOptions = {
        className: 'gugur-templates-modal',
        closeButton: false,
        draggable: false,
        hide: {
      ...
      this.modal = gugurCommon.dialogsManager.createWidget('lightbox', modalOptions);
...
function (_Marionette$LayoutVie) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function className() {
      return 'gugur-templates-modal__header';
    }
  }, {
    key: "getTemplate",
    value: function getTemplate() {
      return '#tmpl-gugur-templates-modal__header';
    }
  }, {
    key: "regions",
    value: function regions() {
      return {
        logoArea: '.gugur-templates-modal__header__logo-area',
        tools: '#gugur-template-library-header-tools',
        menuArea: '.gugur-templates-modal__header__menu-area'
      };
    }
  }, {
    key: "ui",
    value: function ui() {
      return {
        closeModal: '.gugur-templates-modal__header__close'
      };
    }
  }, {
    key: "events",
...
function (_Marionette$ItemView) {
  ...
  (0, _createClass2.default)(_default, [{
    key: "getTemplate",
    value: function getTemplate() {
      return '#tmpl-gugur-templates-modal__header__logo';
    }
  }, {
    key: "className",
    value: function className() {
      return 'gugur-templates-modal__header__logo';
    }
  }, {
    key: "events",
    value: function events() {
...
function (_Marionette$ItemView) {
  ...
  (0, _createClass2.default)(_default, [{
    key: "id",
    value: function id() {
      return 'gugur-template-library-loading';
    }
  }, {
    key: "getTemplate",
    value: function getTemplate() {
      return '#tmpl-gugur-template-library-loading';
    }
  }]);
  return _default;
}(Marionette.ItemView);
...
ControlsCSSParser = gugurModules.ViewModule.extend({
  stylesheet: null,
  getDefaultSettings: function getDefaultSettings() {
    return {
      id: 0,
  ...
  getDefaultElements: function getDefaultElements() {
    return {
      $stylesheetElement: jQuery('<style>', {
        id: 'gugur-style-' + this.getSettings('id')
      })
    };
  },
  initStylesheet: function initStylesheet() {
    var breakpoints = gugurFrontend.config.breakpoints;
  ...
  addRepeaterControlsStyleRules: function addRepeaterControlsStyleRules(repeaterValues, repeaterControlsItems, controls, placeholders, replacements) {
    ...
    repeaterControlsItems.forEach(function (item, index) {
      ...
      self.addStyleRules(item, itemModel.attributes, controls, placeholders.concat(['{{CURRENT_ITEM}}']), replacements.concat(['.gugur-repeater-item-' + itemModel.get('_id')]));
    });
  },
  addDynamicControlStyleRules: function addDynamicControlStyleRules(value, control) {
    var self = this;
    gugur.dynamicTags.parseTagsText(value, control.dynamic, function (id, name, settings) {
      var tag = gugur.dynamicTags.createTag(id, name, settings);
      ...
      self.addStyleRules(tagSettingsModel.getStyleControls(), tagSettingsModel.attributes, tagSettingsModel.controls, ['{{WRAPPER}}'], ['#gugur-tag-' + id]);
    });
  },
  addStyleToDocument: function addStyleToDocument() {
    gugur.$previewContents.find('head').append(this.elements.$stylesheetElement);
...
ControlsCSSParser.addControlStyleRules = function (stylesheet, control, controls, valueCallback, placeholders, replacements) {
  ...
  _.each(control.selectors, function (cssProperty, selector) {
    ...
    if (!(0, _keys.default)(query).length && control.responsive) {
      query = _.pick(gugurCommon.helpers.cloneObject(control.responsive), ['min', 'max']);
...
ControlsCSSParser.parsePropertyPlaceholder = function (control, value, controls, valueCallback, placeholder, parserControlName) {
  ...
  return gugur.getControlView(control.type).getStyleValue(placeholder, value, control);
};
...
SortableBehavior = Marionette.Behavior.extend({
  ...
  initialize: function initialize() {
    this.listenTo(gugur.channels.dataEditMode, 'switch', this.onEditModeSwitched).listenTo(this.view.options.model, 'request:sort:start', this.startSort).listenTo(this.view.options.model, 'request:sort:update', this.updateSort).listenTo(this.view.options.model, 'request:sort:receive', this.receiveSort);
  },
  ...
  onRender: function onRender() {
    ...
    _.defer(function () {
      self.onEditModeSwitched(gugur.channels.dataEditMode.request('activeMode'));
    });
  },
  onDestroy: function onDestroy() {
    this.deactivate();
  ...
  activate: function activate() {
    ...
    var $childViewContainer = this.getChildViewContainer(),
      ...
      placeholder: 'gugur-sortable-placeholder gugur-' + this.getOption('elChildType') + '-placeholder',
      cursorAt: {
        top: 20,
        left: 25
      },
      helper: this._getSortableHelper.bind(this),
      cancel: 'input, textarea, button, select, option, .gugur-inline-editing, .gugur-tab-title'
    },
  ...
  _getSortableHelper: function _getSortableHelper(event, $item) {
    ...
    return '<div style="height: 84px; width: 125px;" class="gugur-sortable-helper gugur-sortable-helper-' + model.get('elType') + '"><div class="icon"><i class="' + model.getIcon() + '"></i></div><div class="gugur-element-title-wrapper"><div class="title">' + model.getTitle() + '</div></div></div>';
  },
  ...
  startSort: function startSort(event, ui) {
    ...
    gugur.channels.data.reply('dragging:model', model).reply('dragging:parent:view', this.view).trigger('drag:start', model).trigger(model.get('elType') + ':drag:start');
  },
  updateSort: function updateSort(ui) {
    var model = gugur.channels.data.request('dragging:model'),
  ...
  receiveSort: function receiveSort(event, ui) {
    ...
    this.view.addChildElement(modelData, {
      ...
      onAfterAdd: function onAfterAdd() {
        var senderSection = gugur.channels.data.request('dragging:parent:view');
  ...
  onSortOver: function onSortOver(event) {
    event.stopPropagation();
    var model = gugur.channels.data.request('dragging:model');
    jQuery(event.target).addClass('gugur-draggable-over').attr({
      'data-dragged-element': model.get('elType'),
      'data-dragged-is-inner': model.get('isInner')
    });
    this.$el.addClass('gugur-dragging-on-child');
  },
  onSortOut: function onSortOut(event) {
    event.stopPropagation();
    jQuery(event.target).removeClass('gugur-draggable-over').removeAttr('data-dragged-element data-dragged-is-inner');
    this.$el.removeClass('gugur-dragging-on-child');
  },
...
module.exports = gugurModules.ViewModule.extend({
  ...
  bindEvents: function bindEvents() {
    gugur.on('preview:loaded', this.ongugurPreviewLoaded);
    this.model.on('change', this.onModelChange);
  },
  addPanelPage: function addPanelPage() {
    var name = this.getSettings('name');
    gugur.getPanelView().addPage(name + '_settings', {
      view: gugur.settings.panelPages[name] || gugur.settings.panelPages.base,
  ...
  initModel: function initModel() {
    this.model = new gugurModules.editor.elements.models.BaseSettings(this.getSettings('settings'), {
  ...
  save: function save(callback) {
    ...
    NProgress.start();
    gugurCommon.ajax.addRequest('save_' + this.getSettings('name') + '_settings', {
      data: data,
  ...
  addPanelMenuItem: function addPanelMenuItem() {
    ...
    gugur.modules.layouts.panel.pages.menu.Menu.addItem(menuItemOptions, 'settings', menuSettings.beforeItem);
  },
...
module.exports = Marionette.Region.extend({
  ...
  constructor: function constructor() {
    ...
    var savedStorage = gugurCommon.storage.get(this.getStorageKey());
  ...
  saveStorage: function saveStorage(key, value) {
    ...
    gugurCommon.storage.set(this.getStorageKey(), this.storage);
  },
  saveSize: function saveSize() {
    this.saveStorage('size', gugur.helpers.getElementInlineStyle(this.$el, this.storageSizeKeys));
  }
});
...
InsertTemplateHandler = Marionette.Behavior.extend({
  ui: {
    insertButton: '.gugur-template-library-template-insert'
  },
  events: {
    'click @ui.insertButton': 'onInsertButtonClick'
  },
  onInsertButtonClick: function onInsertButtonClick() {
    var autoImportSettings = gugur.config.document.remoteLibrary.autoImportSettings;
    ...
    gugur.templates.importTemplate(this.view.model, {
  ...
  showImportDialog: function showImportDialog(model) {
    ...
    dialog.onConfirm = function () {
      gugur.templates.importTemplate(model, {
  ...
  initDialog: function initDialog() {
    InsertTemplateHandler.dialog = gugurCommon.dialogsManager.createWidget('confirm', {
      id: 'gugur-insert-template-settings-dialog',
      headerMessage: gugur.translate('import_template_dialog_header'),
      message: gugur.translate('import_template_dialog_message') + '<br>' + gugur.translate('import_template_dialog_message_attention'),
      strings: {
        confirm: gugur.translate('yes'),
        cancel: gugur.translate('no')
      }
    });
  },
...
TemplateLibraryTemplateView = Marionette.ItemView.extend({
  className: function className() {
    var classes = 'gugur-template-library-template',
        source = this.model.get('source');
    classes += ' gugur-template-library-template-' + source;

    if ('remote' === source) {
      classes += ' gugur-template-library-template-' + this.model.get('type');
    }

    if (this.model.get('isPro')) {
      classes += ' gugur-template-library-pro-template';
    }
  ...
  ui: function ui() {
    return {
      previewButton: '.gugur-template-library-template-preview'
    };
  },
  events: function events() {
    return {
...
module.exports = Marionette.CompositeView.extend({
  ...
  addChildElement: function addChildElement(data, options) {
    ...
    if (options.trigger) {
      gugur.channels.data.trigger(options.trigger.beforeAdd, newItem);
    }
    ...
    if (options.trigger) {
      gugur.channels.data.trigger(options.trigger.afterAdd, newItem);
    }
  ...
  cloneItem: function cloneItem(item) {
    ...
    item.id = gugur.helpers.getUniqueID();
  ...
  onChildviewRequestPaste: function onChildviewRequestPaste(childView) {
    ...
    var elements = gugurCommon.storage.get('transfer').elements,
        index = self.collection.indexOf(childView.model);
    gugur.channels.data.trigger('element:before:add', elements[0]);
    ...
    gugur.channels.data.trigger('element:after:add', elements[0]);
  }
});
...
SectionView = BaseElementView.extend({
  template: Marionette.TemplateCache.get('#tmpl-gugur-section-content'),
  addSectionView: null,
  className: function className() {
    var classes = BaseElementView.prototype.className.apply(this, arguments),
        type = this.isInner() ? 'inner' : 'top';
    return classes + ' gugur-section gugur-' + type + '-section';
  },
  tagName: function tagName() {
    return this.model.getSetting('html_tag') || 'section';
  },
  childViewContainer: '> .gugur-container > .gugur-row',
  behaviors: function behaviors() {
    ...
    return gugur.hooks.applyFilters('elements/section/behaviors', behaviors, this);
  },
  errors: {
  ...
  getEditButtons: function getEditButtons() {
    var elementData = gugur.getElementData(this.model),
        editTools = {};

    if (!this.isInner()) {
      editTools.add = {
        title: gugur.translate('add_element', [elementData.title]),
        icon: 'plus'
      };
    }

    editTools.edit = {
      title: gugur.translate('edit_element', [elementData.title]),
      icon: 'handle'
    };

    if (gugur.config.editButtons) {
      editTools.duplicate = {
        title: gugur.translate('duplicate_element', [elementData.title]),
        icon: 'clone'
      };
    }

    editTools.remove = {
      title: gugur.translate('delete_element', [elementData.title]),
      icon: 'close'
    };
    return editTools;
  },
  ...
  getContextMenuGroups: function getContextMenuGroups() {
    ...
    groups.splice(transferGroupIndex + 1, 0, {
      ...
      actions: [{
        name: 'save',
        title: gugur.translate('save_as_block'),
  ...
  getSortableOptions: function getSortableOptions() {
    var sectionConnectClass = this.isInner() ? '.gugur-inner-section' : '.gugur-top-section';
    return {
      connectWith: sectionConnectClass + ' > .gugur-container > .gugur-row',
      handle: '> .gugur-element-overlay .gugur-editor-element-edit',
      items: '> .gugur-column',
  ...
  setStructure: function setStructure(structure) {
    var parsedStructure = gugur.presetsFactory.getParsedStructure(structure);
  ...
  redefineLayout: function redefineLayout() {
    var preset = gugur.presetsFactory.getPresetByStructure(this.getStructure());
  ...
  _checkIsFull: function _checkIsFull() {
    this.$el.toggleClass('gugur-section-filled', this.isCollectionFilled());
  },
  ...
  showChildrenPercentsTooltip: function showChildrenPercentsTooltip(columnView, nextColumnView) {
    ...
    columnView.ui.percentsTooltip.attr('data-side', gugurCommon.config.isRTL ? 'right' : 'left');
    nextColumnView.ui.percentsTooltip.show();
    nextColumnView.ui.percentsTooltip.attr('data-side', gugurCommon.config.isRTL ? 'left' : 'right');
  },
  ...
  onChildviewRequestResizeStart: function onChildviewRequestResizeStart(columnView) {
    ...
    gugur.helpers.disableElementEvents($iframes);
  },
  onChildviewRequestResizeStop: function onChildviewRequestResizeStop(columnView) {
    ...
    var $iframes = columnView.$el.find('iframe').add(nextColumnView.$el.find('iframe'));
    gugur.helpers.enableElementEvents($iframes);
  },
...
function (_Marionette$ItemView) {
  ...
  (0, _createClass2.default)(AddSectionBase, [{
    ...
    value: function template() {
      return Marionette.TemplateCache.get('#tmpl-gugur-add-section');
    }
  }, {
    key: "attributes",
    ...
    value: function ui() {
      return {
        addNewSection: '.gugur-add-new-section',
        closeButton: '.gugur-add-section-close',
        addSectionButton: '.gugur-add-section-button',
        addTemplateButton: '.gugur-add-template-button',
        selectPreset: '.gugur-select-preset',
        presets: '.gugur-preset'
      };
    }
  }, {
    key: "events",
    ...
    key: "className",
    value: function className() {
      return 'gugur-add-section gugur-visible-desktop';
    }
  }, {
    key: "addSection",
    value: function addSection(properties, options) {
      return gugur.getPreviewView().addChildElement(properties, jQuery.extend({}, this.options, options));
    }
  }, {
    key: "setView",
    ...
    value: function getContextMenuGroups() {
      var hasContent = function hasContent() {
        return gugur.elements.length > 0;
      };
      ...
      return [{
        ...
        actions: [{
          ...
          title: gugur.translate('copy_all_content'),
          callback: this.copy.bind(this),
          isEnabled: hasContent
        }, {
          name: 'delete_all_content',
          title: gugur.translate('delete_all_content'),
          callback: gugur.clearPage.bind(gugur),
          isEnabled: hasContent
        }]
      }];
    }
    ...
    key: "copy",
    value: function copy() {
      gugur.getPreviewView().copy();
    }
  }, {
    key: "paste",
    value: function paste() {
      gugur.getPreviewView().paste(this.getOption('at'));
    }
  }, {
    key: "isPasteEnabled",
    ...
    value: function onRender() {
      this.$el.html5Droppable({
        axis: ['vertical'],
        groups: ['gugur-element'],
        placeholder: false,
        currentElementClass: 'gugur-html5dnd-current-element',
        hasDraggingOnChildClass: 'gugur-dragging-on-child',
    ...
    value: function onPresetSelected(event) {
      ...
      var selectedStructure = event.currentTarget.dataset.structure,
          parsedStructure = gugur.presetsFactory.getParsedStructure(selectedStructure),
          elements = [];
      var loopIndex;
      ...
      gugur.channels.data.trigger('element:before:add', {
        elType: 'section'
      });
      ...
      gugur.channels.data.trigger('element:after:add');
    }
  }, {
    key: "onDropping",
    value: function onDropping() {
      gugur.channels.data.trigger('section:before:drop');
      ...
      gugur.channels.data.trigger('section:after:drop');
    }
  }]);
...
ControlBoxShadowItemView = ControlMultipleBaseItemView.extend({
  ui: function ui() {
    ...
    ui.sliders = '.gugur-slider';
    ui.colors = '.gugur-shadow-color-picker';
    return ui;
  },
  initSliders: function initSliders() {
    ...
    this.ui.sliders.each(function (index, slider) {
      var $input = jQuery(slider).next('.gugur-slider-input').find('input');
  ...
  initColors: function initColors() {
    var self = this;
    gugur.helpers.wpColorPicker(this.ui.colors, {
...
RepeaterRowView = Marionette.CompositeView.extend({
  template: Marionette.TemplateCache.get('#tmpl-gugur-repeater-row'),
  className: 'gugur-repeater-fields',
  ui: {
    duplicateButton: '.gugur-repeater-tool-duplicate',
    editButton: '.gugur-repeater-tool-edit',
    removeButton: '.gugur-repeater-tool-remove',
    itemTitle: '.gugur-repeater-row-item-title'
  },
  behaviors: {
    HandleInnerTabs: {
  ...
  childViewContainer: '.gugur-repeater-row-controls',
  getChildView: function getChildView(item) {
    var controlType = item.get('type');
    return gugur.getControlView(controlType);
  },
  ...
  setTitle: function setTitle() {
    ...
    if (!title) {
      title = gugur.translate('Item #%s', [this.getOption('itemIndex')]);
    }
  ...
  initialize: function initialize(options) {
    ...
    this.collection = new Backbone.Collection(_.values(gugur.mergeControlsSettings(options.controlFields)));
  },
  onRender: function onRender() {
    this.setTitle();
  },
  ...
  onChildviewResponsiveSwitcherClick: function onChildviewResponsiveSwitcherClick(childView, device) {
    if ('desktop' === device) {
      gugur.getPanelView().getCurrentPageView().$el.toggleClass('gugur-responsive-switchers-open');
    }
  }
});
...
ElementModel = Backbone.Model.extend({
  ...
  initSettings: function initSettings() {
    var elType = this.get('elType'),
      ...
      column: ColumnSettingsModel
        ...
        SettingsModel = settingModels[elType] || gugurModules.editor.elements.models.BaseSettings;

    if (jQuery.isEmptyObject(settings)) {
      settings = gugurCommon.helpers.cloneObject(settings);
    }
    ...
    settings = new SettingsModel(settings, {
      controls: gugur.getElementControls(this)
    });
    this.set('settings', settings);
    gugurFrontend.config.elements.data[this.cid] = settings;
  },
  initEditSettings: function initEditSettings() {
    ...
    gugurFrontend.config.elements.editSettings[this.cid] = editSettings;
  },
  ...
  getDefaultTitle: function getDefaultTitle() {
    return gugur.getElementData(this).title;
  },
  ...
  getIcon: function getIcon() {
    return gugur.getElementData(this).icon;
  },
  createRemoteRenderRequest: function createRemoteRenderRequest() {
    var data = this.toJSON();
    return gugurCommon.ajax.addRequest('render_widget', {
      unique_id: this.cid,
      data: {
        data: data
      },
  ...
  clone: function clone() {
    var newModel = new this.constructor(gugurCommon.helpers.cloneObject(this.attributes));
    newModel.set('id', gugur.helpers.getUniqueID());
...
var ElementsCollection = Backbone.Collection.extend({
  ...
  model: function model(attrs, options) {
    ...
    if (attrs.elType) {
      ModelClass = gugur.hooks.applyFilters('element/model', ElementModel, attrs);
    }
...
module.exports = Marionette.ItemView.extend({
  template: '#tmpl-gugur-panel-global',
  id: 'gugur-panel-global',
  initialize: function initialize() {
    gugur.getPanelView().getCurrentPageView().search.reset();
  }
});
...
module.exports = Marionette.ItemView.extend({
  template: '#tmpl-gugur-element-library-element',
  className: 'gugur-element-wrapper',
  ui: {
    element: '.gugur-element'
  },
  onRender: function onRender() {
    var _this = this;
    ...
    this.ui.element.html5Draggable({
      onDragStart: function onDragStart() {
        gugur.channels.panelElements.reply('element:selected', _this).trigger('element:drag:start');
      },
      onDragEnd: function onDragEnd() {
        gugur.channels.panelElements.trigger('element:drag:end');
      },
      groups: ['gugur-element']
    });
  }
...
PanelSchemeColorsView = PanelSchemeBaseView.extend({
  ui: function ui() {
    ...
    ui.systemSchemes = '.gugur-panel-scheme-color-system-scheme';
    return ui;
  },
  events: function events() {
  ...
  onSystemSchemeClick: function onSystemSchemeClick(event) {
    var $schemeClicked = jQuery(event.currentTarget),
        ...
        scheme = gugur.config.system_schemes[this.getType()][schemeName].items;
...
PanelSchemeBaseView = Marionette.CompositeView.extend({
  id: function id() {
    return 'gugur-panel-scheme-' + this.getType();
  },
  className: function className() {
    return 'gugur-panel-scheme gugur-panel-scheme-' + this.getUIType();
  },
  childViewContainer: '.gugur-panel-scheme-items',
  getTemplate: function getTemplate() {
    return Marionette.TemplateCache.get('#tmpl-gugur-panel-schemes-' + this.getType());
  },
  ...
  ui: function ui() {
    return {
      saveButton: '.gugur-panel-scheme-save .gugur-button',
      discardButton: '.gugur-panel-scheme-discard .gugur-button',
      resetButton: '.gugur-panel-scheme-reset .gugur-button'
    };
  },
  events: function events() {
    return {
  ...
  getScheme: function getScheme() {
    return gugur.schemes.getScheme(this.getType());
  },
  ...
  discardScheme: function discardScheme() {
    gugur.schemes.resetSchemes(this.getType());
    this.onSchemeChange();
  ...
  setSchemeValue: function setSchemeValue(key, value) {
    gugur.schemes.setSchemeValue(this.getType(), key, value);
    this.onSchemeChange();
  },
  saveScheme: function saveScheme() {
    gugur.schemes.saveScheme(this.getType());
    this.ui.saveButton.prop('disabled', true);
    this.resetScheme();
  ...
  setDefaultScheme: function setDefaultScheme() {
    var defaultScheme = gugur.config.default_schemes[this.getType()].items;
  ...
  onSchemeChange: function onSchemeChange() {
    gugur.schemes.printSchemesStyle();
  },
...
PanelSchemeItemView = Marionette.ItemView.extend({
  getTemplate: function getTemplate() {
    return Marionette.TemplateCache.get('#tmpl-gugur-panel-scheme-' + this.getUIType() + '-item');
  },
  className: function className() {
    return 'gugur-panel-scheme-item';
  }
});
 ...
 */
(function ($) {
  $.fn.gugurSerializeObject = function () {
...
var App = Marionette.Application.extend({
  ...
  // TODO: BC Since 2.3.0
  ajax: gugurCommon.ajax,
  conditions: __webpack_require__(357),
  history: __webpack_require__(358),
  channels: {
    editor: Backbone.Radio.channel('gugur:editor'),
    data: Backbone.Radio.channel('gugur:data'),
    panelElements: Backbone.Radio.channel('gugur:panelElements'),
    dataEditMode: Backbone.Radio.channel('gugur:editmode'),
    deviceMode: Backbone.Radio.channel('gugur:deviceMode'),
    templates: Backbone.Radio.channel('gugur:templates')
  },

  /**
   * Exporting modules that can be used externally
   * @TODO: All of the following entries should move to `gugurModules.editor`
   */
  modules: {
    // TODO: Deprecated alias since 2.3.0
    get Module() {
      gugurCommon.helpers.deprecatedMethod('gugur.modules.Module', '2.3.0', 'gugurModules.Module');
      return gugurModules.Module;
    },
    ...
    components: {
      templateLibrary: {
        views: {
          ...
          get BaseModalLayout() {
            gugurCommon.helpers.deprecatedMethod('gugur.modules.components.templateLibrary.views.BaseModalLayout', '2.4.0', 'gugurModules.common.views.modal.Layout');
            return gugurModules.common.views.modal.Layout;
          }
    ...
    elements: {
      models: {
        // TODO: Deprecated alias since 2.4.0
        get BaseSettings() {
          gugurCommon.helpers.deprecatedMethod('gugur.modules.elements.models.BaseSettings', '2.4.0', 'gugurModules.editor.elements.models.BaseSettings');
          return gugurModules.editor.elements.models.BaseSettings;
        },
    ...
    views: {
      // TODO: Deprecated alias since 2.4.0
      get ControlsStack() {
        gugurCommon.helpers.deprecatedMethod('gugur.modules.views.ControlsStack', '2.4.0', 'gugurModules.editor.views.ControlsStack');
        return gugurModules.editor.views.ControlsStack;
      }
  ...
  backgroundClickListeners: {
    popover: {
      element: '.gugur-controls-popover',
      ignore: '.gugur-control-popover-toggle-toggle, .gugur-control-popover-toggle-toggle-label, .select2-container'
    },
    tagsList: {
      element: '.gugur-tags-list',
      ignore: '.gugur-control-dynamic-switcher'
    },
    panelFooterSubMenus: {
      element: '.gugur-panel-footer-tool.gugur-toggle-state',
      ignore: '.gugur-panel-footer-tool.gugur-toggle-state, #gugur-panel-saver-button-publish-label',
      callback: function callback($elementsToHide) {
        $elementsToHide.removeClass('gugur-open');
      }
    }
  },
  ...
  addControlView: function addControlView(controlID, ControlView) {
    this.modules.controls[gugurCommon.helpers.upperCaseWords(controlID)] = ControlView;
  },
  ...
  getElementData: function getElementData(model) {
    ...
    var elementConfig = gugurCommon.helpers.cloneObject(this.config.elements[elType]);
  ...
  getControlView: function getControlView(controlID) {
    var capitalizedControlName = gugurCommon.helpers.upperCaseWords(controlID),
  ...
  initDialogsManager: function initDialogsManager() {
    this.dialogsManager = gugurCommon.dialogsManager;
  },
  ...
  initPreview: function initPreview() {
    var $ = jQuery;
    this.$previewWrapper = $('#gugur-preview');
    this.$previewResponsiveWrapper = $('#gugur-preview-responsive-wrapper');
    var previewIframeId = 'gugur-preview-iframe'; // Make sure the iFrame does not exist.
  ...
  initFrontend: function initFrontend() {
    ...
    window.gugurFrontend = frontendWindow.gugurFrontend;
    frontendWindow.gugur = this;
    gugurFrontend.init();
  ...
  initClearPageDialog: function initClearPageDialog() {
    ...
    self.getClearPageDialog = function () {
      ...
      dialog = gugurCommon.dialogsManager.createWidget('confirm', {
        id: 'gugur-clear-page-dialog',
        headerMessage: gugur.translate('clear_page'),
        message: gugur.translate('dialog_confirm_clear_page'),
        position: {
          my: 'center center',
          at: 'center center'
        },
        strings: {
          confirm: gugur.translate('delete'),
          cancel: gugur.translate('cancel')
        },
  ...
  getCurrentElement: function getCurrentElement() {
    var isPreview = -1 !== ['BODY', 'IFRAME'].indexOf(document.activeElement.tagName) && 'BODY' === gugurFrontend.elements.window.document.activeElement.tagName;

    if (!isPreview) {
      return false;
    }

    var targetElement = gugur.channels.editor.request('contextMenu:targetView');
    ...
    if (!targetElement) {
      targetElement = gugur.getPreviewView();
    }
  ...
  setAjax: function setAjax() {
    gugurCommon.ajax.addRequestConstant('editor_post_id', this.config.document.id);
    gugurCommon.ajax.on('request:unhandledError', function (xmlHttpRequest) {
      gugur.notifications.showToast({
        message: gugur.createAjaxErrorMessage(xmlHttpRequest)
      });
    });
  },
  ...
  preventClicksInsideEditor: function preventClicksInsideEditor() {
    ...
    this.$previewContents.on('click', function (event) {
      var $target = jQuery(event.target),
          editMode = gugur.channels.dataEditMode.request('activeMode'),
          isClickInsidegugur = !!$target.closest('#gugur, .pen-menu').length,
          isTargetInsideDocument = this.contains($target[0]);

      if (isClickInsidegugur && 'edit' === editMode || !isTargetInsideDocument) {
        return;
      }

      if ($target.closest('a:not(.gugur-clickable)').length) {
        event.preventDefault();
      }

      if (!isClickInsidegugur) {
        $e.route('panel/elements/categories');
      }
    });
  },
  ...
  showFatalErrorDialog: function showFatalErrorDialog(options) {
    ...
    gugurCommon.dialogsManager.createWidget('confirm', options).show();
  },
  showFlexBoxAttentionDialog: function showFlexBoxAttentionDialog() {
    var _this2 = this;

    var introduction = new gugurModules.editor.utils.Introduction({
      introductionKey: 'flexbox',
      dialogType: 'confirm',
      dialogOptions: {
        id: 'gugur-flexbox-attention-dialog',
  ...
  checkPageStatus: function checkPageStatus() {
    if (gugur.config.current_revision_id !== gugur.config.document.id) {
      this.notifications.showToast({
        message: this.translate('working_on_draft_notification'),
        buttons: [{
          name: 'view_revisions',
          text: gugur.translate('view_all_revisions'),
          callback: function callback() {
  ...
  enterPreviewMode: function enterPreviewMode(hidePanel) {
    var $elements = gugurFrontend.elements.$body;

    if (hidePanel) {
      $elements = $elements.add(gugurCommon.elements.$body);
    }

    $elements.removeClass('gugur-editor-active').addClass('gugur-editor-preview');
    this.$previewgugurEl.removeClass('gugur-edit-area-active').addClass('gugur-edit-area-preview');

    if (hidePanel) {
      // Handle panel resize
      this.$previewWrapper.css(gugurCommon.config.isRTL ? 'right' : 'left', '');
      this.panel.$el.css('width', '');
    }
  },
  exitPreviewMode: function exitPreviewMode() {
    gugurFrontend.elements.$body.add(gugurCommon.elements.$body).removeClass('gugur-editor-preview').addClass('gugur-editor-active');
    this.$previewgugurEl.removeClass('gugur-edit-area-preview').addClass('gugur-edit-area-active');
  },
  changeEditMode: function changeEditMode(newMode) {
    var dataEditMode = gugur.channels.dataEditMode,
  ...
  reloadPreview: function reloadPreview() {
    jQuery('#gugur-preview-loading').show();
  ...
  changeDeviceMode: function changeDeviceMode(newDeviceMode) {
    ...
    gugurCommon.elements.$body.removeClass('gugur-device-' + oldDeviceMode).addClass('gugur-device-' + newDeviceMode);
  ...
  translate: function translate(stringKey, templateArgs, i18nStack) {
    ...
    return gugurCommon.translate(stringKey, null, templateArgs, i18nStack);
  },
  logSite: function logSite() {
    ...
    if (_environment.default.firefox) {
      ...
      text += '%c00';
      style = 'font-size: 22px; background-image: url("' + gugurCommon.config.urls.assets + 'images/logo-icon.png"); color: transparent; background-repeat: no-repeat';
    }

    setTimeout(console.log.bind(console, text, style)); // eslint-disable-line

    text = '%cLove using gugur? Join our growing community of gugur developers: %chttps://github.com/gugur/gugur';
  ...
  requestWidgetsConfig: function requestWidgetsConfig() {
    ...
    gugurCommon.ajax.addRequest('get_widgets_config', {
      data: {
      ...
      success: function success(data) {
        ...
        gugurCommon.elements.$body.addClass('gugur-controls-ready');
      }
    });
  },
  onStart: function onStart() {
    NProgress.start();
    NProgress.inc(0.2);
    this.config = gugurConfig;
    Backbone.Radio.DEBUG = false;
    Backbone.Radio.tuneIn('gugur');
    this.initComponents();
    ...
    gugurCommon.elements.$window.trigger('gugur:init');
    this.initPreview();
    this.logSite();
  },
  onPreviewLoaded: function onPreviewLoaded() {
    ...
    if (!previewWindow.gugurFrontend) {
      this.onPreviewLoadingError();
      return;
    }

    this.$previewContents = this.$preview.contents();
    this.$previewgugurEl = this.$previewContents.find('#gugur');

    if (!this.$previewgugurEl.length) {
    ...
    var iframeRegion = new Marionette.Region({
      ...
      el: this.$previewgugurEl[0]
    });
    this.schemes.init();
    this.schemes.printSchemesStyle();
    this.preventClicksInsideEditor();
    this.addBackgroundClickArea(gugurFrontend.elements.window.document);
    ...
    this.$previewContents.children().addClass('gugur-html');
    var $frontendBody = gugurFrontend.elements.$body;
    $frontendBody.addClass('gugur-editor-active');

    if (!gugur.userCan('design')) {
      $frontendBody.addClass('gugur-editor-content-only');
    }

    this.changeDeviceMode(this._defaultDeviceMode);
    jQuery('#gugur-loading, #gugur-preview-loading').fadeOut(600);

    _.defer(function () {
      gugurFrontend.elements.window.jQuery.holdReady(false);
    });

    this.enqueueTypographyFonts();
    this.onEditModeSwitched();
    $e.shortcuts.bindListener(gugurFrontend.elements.$window);
  ...
  onEnvNotCompatible: function onEnvNotCompatible() {
    this.showFatalErrorDialog({
      ...
      strings: {
        confirm: gugur.translate('proceed_anyway')
      },
      hide: {
        onButtonClick: true
      },
  ...
  onPreviewLoadingError: function onPreviewLoadingError() {
    ...
    var debugUrl = self.config.document.urls.preview + '&preview-debug',
        ...
        previewDebugLink = '<div id="gugur-preview-debug-link-text"><a href="' + debugUrl + '" target="_blank">' + previewDebugLinkText + '</a></div>',
        debugData = gugur.config.preview.debug_data,
        dialogOptions = {
      className: 'gugur-preview-loading-error',
    ...
    jQuery.get(debugUrl, function () {
      ...
      self.showFatalErrorDialog({
        ...
        onConfirm: function onConfirm() {
          var url = 500 <= response.status ? gugur.config.preview.help_preview_http_error_500_url : gugur.config.preview.help_preview_http_error_url;
          open(url, '_blank');
        }
      });
    });
...
window.gugur = new App();

if (-1 === location.href.search('gugur_TESTS=1')) {
  gugur.start();
}

module.exports = gugur;
...
function () {
  function Heartbeat() {
    ...
    jQuery(document).on({
      'heartbeat-send': function heartbeatSend(event, data) {
        data.gugur_post_lock = {
          post_ID: gugur.config.document.id
        };
      },
      'heartbeat-tick': function heartbeatTick(event, response) {
        if (response.locked_user) {
          if (gugur.saver.isEditorChanged()) {
            gugur.saver.saveEditor({
              status: 'autosave'
            });
          }
        ...
        gugurCommon.ajax.addRequestConstant('_nonce', response.gugurNonce);
      },
      'heartbeat-tick.wp-refresh-nonces': function heartbeatTickWpRefreshNonces(event, response) {
        var nonces = response['gugur-refresh-nonces'];

        if (nonces) {
          if (nonces.heartbeatNonce) {
            gugurCommon.ajax.addRequestConstant('_nonce', nonces.gugurNonce);
          }
    ...
    if (gugur.config.locked_user) {
      this.showLockMessage(gugur.config.locked_user);
    }
  }
  ...
  (0, _createClass2.default)(Heartbeat, [{
    key: "initModal",
    value: function initModal() {
      var modal = gugurCommon.dialogsManager.createWidget('lightbox', {
        headerMessage: gugur.translate('take_over')
      });
      modal.addButton({
        name: 'go_back',
        text: gugur.translate('go_back'),
        callback: function callback() {
          parent.history.go(-1);
        }
      });
      modal.addButton({
        name: 'take_over',
        text: gugur.translate('take_over'),
        callback: function callback() {
          wp.heartbeat.enqueue('gugur_force_post_lock', true);
    ...
    value: function showLockMessage(lockedUser) {
      ...
      modal.setMessage(gugur.translate('dialog_user_taken_over', [lockedUser])).show();
    }
  }]);
  return Heartbeat;
}();
...
function (_BaseRegion) {
  ...
  function _default(options) {
    ...
    _this.indicators = {
      customPosition: {
        title: gugur.translate('custom_positioning'),
        icon: 'cursor-move',
    ...
    _this.listenTo(gugur.channels.dataEditMode, 'switch', _this.onEditModeSwitched);

    gugur.on('navigator:init', function () {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function getDraggableOptions() {
      return {
        iframeFix: true,
        handle: '#gugur-navigator__header',
    ...
    value: function getResizableOptions() {
      ...
      return {
        ...
        start: function start() {
          gugur.$previewWrapper.addClass('ui-resizable-resizing');
        },
        stop: function stop() {
          gugur.$previewWrapper.removeClass('ui-resizable-resizing');

          if (_this2.isDocked) {
            _this2.storage.size.width = gugur.helpers.getElementInlineStyle(_this2.$el, ['width']).width;
            gugurCommon.storage.set('navigator', _this2.storage);
          } else {
            _this2.saveSize();
          }
        }
    ...
    value: function open(model) {
      ...
      this.ensurePosition();
      gugurCommon.elements.$window.on('resize', this.ensurePosition);
    }
  }, {
    key: "close",
    value: function close(silent) {
      ...
      gugurCommon.elements.$window.off('resize', this.ensurePosition);
    }
  }, {
    key: "isOpen",
    ...
    key: "dock",
    value: function dock() {
      gugurCommon.elements.$body.addClass('gugur-navigator-docked');
      var side = gugurCommon.config.isRTL ? 'left' : 'right',
      ...
      gugur.$previewWrapper.css(side, this.storage.size.width);
      this.$el.resizable('destroy');
      resizableOptions.handles = gugurCommon.config.isRTL ? 'e' : 'w';

      resizableOptions.resize = function (event, ui) {
        gugur.$previewWrapper.css(side, ui.size.width);
      };
    ...
    value: function undock(silent) {
      gugurCommon.elements.$body.removeClass('gugur-navigator-docked');
      gugur.$previewWrapper.css(gugurCommon.config.isRTL ? 'left' : 'right', '');
      this.setSize();
      this.$el.resizable('destroy');
      this.$el.resizable(this.getResizableOptions());
      this.isDocked = false;
    ...
    value: function onDrag(event, ui) {
      ...
      gugurCommon.elements.$body.toggleClass('gugur-navigator--dock-hint', gugurCommon.config.isRTL ? isOutOfLeft : isOutOfRight);
    }
  }, {
    key: "onDragStop",
    value: function onDragStop(event, ui) {
      ...
      gugurCommon.elements.$body.removeClass('gugur-navigator--dock-hint');
    }
  }, {
...
var _default =
/*#__PURE__*/
function (_gugurModules$com) {
  (0, _inherits2.default)(_default, _gugurModules$com);
      ...
      return {
        toggle: {
          ...
          dependency: function dependency() {
            return 'edit' === gugur.channels.dataEditMode.request('activeMode');
          }
        }
      };
    }
...
function (_Marionette$LayoutVie) {
  ...
  (0, _createClass2.default)(_default, [{
    key: "getTemplate",
    value: function getTemplate() {
      return '#tmpl-gugur-navigator';
    }
  }, {
    key: "id",
    value: function id() {
      return 'gugur-navigator__inner';
    }
  }, {
    key: "ui",
    value: function ui() {
      return {
        toggleAll: '#gugur-navigator__toggle-all',
        close: '#gugur-navigator__close'
      };
    }
  }, {
    key: "events",
    ...
    value: function regions() {
      return {
        elements: '#gugur-navigator__elements'
      };
    }
  }, {
    key: "toggleAll",
    value: function toggleAll() {
      var state = 'expand' === this.ui.toggleAll.data('gugur-action'),
          classes = ['eicon-collapse', 'eicon-expand'];
      this.ui.toggleAll.data('gugur-action', state ? 'collapse' : 'expand').removeClass(classes[+state]).addClass(classes[+!state]);
...
function (_Marionette$Composite) {
  ...
  (0, _createClass2.default)(_default, [{
    key: "getTemplate",
    value: function getTemplate() {
      return '#tmpl-gugur-navigator__elements';
    }
  }, {
    key: "ui",
    value: function ui() {
      return {
        item: '> .gugur-navigator__item',
        title: '> .gugur-navigator__item .gugur-navigator__element__title__text',
        toggle: '> .gugur-navigator__item > .gugur-navigator__element__toggle',
        toggleList: '> .gugur-navigator__item > .gugur-navigator__element__list-toggle',
        indicators: '> .gugur-navigator__item > .gugur-navigator__element__indicators',
        indicator: '> .gugur-navigator__item > .gugur-navigator__element__indicators > .gugur-navigator__element__indicator',
        elements: '> .gugur-navigator__elements'
      };
    }
  }, {
    ...
    value: function className() {
      ...
      var classes = 'gugur-navigator__element';

      if (elType) {
        classes += ' gugur-navigator__element-' + elType;
      }

      if (this.hasChildren()) {
        classes += ' gugur-navigator__element--has-children';
      }
    ...
    value: function initialize() {
      this.collection = this.model.get('elements');
      this.childViewContainer = '.gugur-navigator__elements';
    ...
    value: function toggleList(state, callback) {
      ...
      var isActive = this.ui.item.hasClass('gugur-active');

      if (isActive === state) {
        return;
      }

      this.ui.item.toggleClass('gugur-active', state);
    ...
    value: function toggleHiddenClass() {
      this.$el.toggleClass('gugur-navigator__element--hidden', !!this.model.get('hidden'));
    }
  }, {
    ...
    value: function dragShouldBeIgnored(draggedModel) {
      var childTypes = gugur.helpers.getElementChildType(this.model.get('elType')),
    ...
    value: function addEditingClass() {
      this.ui.item.addClass('gugur-editing');
    }
  }, {
    key: "removeEditingClass",
    value: function removeEditingClass() {
      this.ui.item.removeClass('gugur-editing');
    }
  }, {
    ...
    value: function enterTitleEditing() {
      ...
      gugur.addBackgroundClickListener('navigator', {
    ...
    value: function exitTitleEditing() {
      ...
      gugur.removeBackgroundClickListener('navigator');
    }
  }, {
    ...
    value: function activateSortable() {
      ...
      this.ui.elements.sortable({
        items: '> .gugur-navigator__element',
        placeholder: 'ui-sortable-placeholder',
        axis: 'y',
        forcePlaceholderSize: true,
        connectWith: '.gugur-navigator__element-' + this.model.get('elType') + ' ' + this.ui.elements.selector,
    ...
    value: function renderIndicators() {
      ...
      jQuery.each(gugur.navigator.indicators, function (indicatorName, indicatorSettings) {
        ...
        var $indicator = jQuery('<div>', {
          class: 'gugur-navigator__element__indicator',
    ...
    value: function onRender() {
      ...
      this.ui.item.css('padding-' + (gugurCommon.config.isRTL ? 'right' : 'left'), this.getIndent());
    ...
    value: function onSortStart(event, ui) {
      ...
      jQuery(ui.item).children('.gugur-navigator__item').trigger('click');
      gugur.navigator.getLayout().activateElementsMouseInteraction();
    }
  }, {
    key: "onSortStop",
    value: function onSortStop() {
      gugur.navigator.getLayout().deactivateElementsMouseInteraction();
    }
  }, {
    key: "onSortOver",
    ...
    value: function onMouseEnter(event) {
      ...
      event.stopPropagation();
      var dragShouldBeIgnored = this.recursiveChildAgreement('dragShouldBeIgnored', gugur.channels.data.request('dragging:model'));
    ...
    value: function onEditRequest() {
      ...
      gugur.navigator.getLayout().elements.currentView.recursiveChildInvoke('removeEditingClass');
      this.addEditingClass();
      gugur.helpers.scrollToView(this.$el, 400, gugur.navigator.getLayout().elements.$el);
    }
  }, {
    ...
    value: function onIndicatorClick(event) {
      ...
      setTimeout(function () {
        var editor = gugur.getPanelView().currentPageView,
...
function (_Marionette$ItemView) {
  ...
  (0, _createClass2.default)(_default, [{
    key: "getTemplate",
    value: function getTemplate() {
      return '#tmpl-gugur-navigator__elements--empty';
    }
  }, {
    key: "className",
    value: function className() {
      return 'gugur-empty-view';
    }
  }, {
    key: "onRendr",
    value: function onRendr() {
      this.$el.css('padding-' + (gugurCommon.config.isRTL ? 'right' : 'left'), this.getOption('indent'));
    }
  }]);
  return _default;
}(Marionette.ItemView);
...
function (_Marionette$ItemView) {
  ...
  (0, _createClass2.default)(_default, [{
    key: "getTemplate",
    value: function getTemplate() {
      return '#tmpl-gugur-navigator__root--empty';
    }
  }, {
    key: "className",
    value: function className() {
      return 'gugur-nerd-box';
    }
  }]);
  return _default;
}(Marionette.ItemView);
...
var _default =
/*#__PURE__*/
function (_gugurModules$Mod) {
  (0, _inherits2.default)(_default, _gugurModules$Mod);
...
function (_gugurModules$com) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function getLogoOptions() {
      return {
        title: gugur.translate('keyboard_shortcuts')
      };
    }
  }, {
    key: "initialize",
...
module.exports = Marionette.Behavior.extend({
  ...
  ui: {
    tagArea: '.gugur-control-tag-area',
    dynamicSwitcher: '.gugur-control-dynamic-switcher'
  },
  events: {
  ...
  renderTools: function renderTools() {
    ...
    var $dynamicSwitcher = jQuery(Marionette.Renderer.render('#tmpl-gugur-control-dynamic-switcher'));

    if (this.view.model.get('label_block')) {
      this.ui.controlTitle.after($dynamicSwitcher);
      var $responsiveSwitchers = $dynamicSwitcher.next('.gugur-control-responsive-switchers');
  ...
  toggleDynamicClass: function toggleDynamicClass() {
    this.$el.toggleClass('gugur-control-dynamic-value', this.isDynamicMode());
  },
  ...
  createTagsList: function createTagsList() {
    var tags = _.groupBy(this.getOption('tags'), 'group'),
        groups = gugur.dynamicTags.getConfig('groups'),
        $tagsList = this.ui.tagsList = jQuery('<div>', {
      class: 'gugur-tags-list'
    }),
        $tagsListInner = jQuery('<div>', {
      class: 'gugur-tags-list__inner'
    });
    ...
    jQuery.each(groups, function (groupName) {
      ...
      var group = this,
        ...
        class: 'gugur-tags-list__group-title'
      }).text(group.title);
      $tagsListInner.append($groupTitle);
      groupTags.forEach(function (tag) {
        var $tag = jQuery('<div>', {
          class: 'gugur-tags-list__item'
        });
        $tag.text(tag.title).attr('data-tag-name', tag.name);
        $tagsListInner.append($tag);
      });
    });
    $tagsListInner.on('click', '.gugur-tags-list__item', this.onTagsListItemClick.bind(this));
    gugurCommon.elements.$body.append($tagsList);
  },
  ...
  toggleTagsList: function toggleTagsList() {
    ...
    var direction = gugurCommon.config.isRTL ? 'left' : 'right';
  ...
  setDefaultTagView: function setDefaultTagView() {
    var tagData = gugur.dynamicTags.tagTextToTagData(this.getDynamicValue());
  ...
  tagViewToTagText: function tagViewToTagText() {
    ...
    return gugur.dynamicTags.tagDataToTagText(tagView.getOption('id'), tagView.getOption('name'), tagView.model);
  },
  ...
  setDynamicValue: function setDynamicValue(value) {
    ...
    dynamicSettings = gugurCommon.helpers.cloneObject(dynamicSettings);
  ...
  onRender: function onRender() {
    this.$el.addClass('gugur-control-dynamic');
    this.renderTools();
    this.toggleDynamicClass();
  ...
  onTagsListItemClick: function onTagsListItemClick(event) {
    ...
    this.setTagView(gugur.helpers.getUniqueID(), $tag.data('tagName'), {});
...
module.exports = Marionette.ItemView.extend({
  className: 'gugur-dynamic-cover gugur-input-style',
  ...
  ui: {
    remove: '.gugur-dynamic-cover__remove'
  },
  events: function events() {
    var events = {
  ...
  getTemplate: function getTemplate() {
    var config = this.getTagConfig(),
        templateFunction = Marionette.TemplateCache.get('#tmpl-gugur-control-dynamic-cover'),
  ...
  getTagConfig: function getTagConfig() {
    return gugur.dynamicTags.getConfig('tags.' + this.getOption('name'));
  },
  initSettingsPopup: function initSettingsPopup() {
    var settingsPopupOptions = {
      className: 'gugur-tag-settings-popup',
      position: {
        my: 'left top+5',
        at: 'left bottom',
        of: this.$el,
    ...
    var settingsPopup = gugurCommon.dialogsManager.createWidget('buttons', settingsPopupOptions);
  ...
  initModel: function initModel() {
    this.model = new gugurModules.editor.elements.models.BaseSettings(this.getOption('settings'), {
...
module.exports = gugurModules.editor.views.ControlsStack.extend({
  ...
  getNamespaceArray: function getNamespaceArray() {
    var currentPageView = gugur.getPanelView().getCurrentPageView(),
...
module.exports = Marionette.ItemView.extend({
  className: 'gugur-tag-controls-stack-empty',
  template: '#tmpl-gugur-tag-controls-stack-empty'
});
...
var _default =
/*#__PURE__*/
function (_gugurModules$Vie) {
  (0, _inherits2.default)(_default, _gugurModules$Vie);
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function getDefaultSettings() {
      return {
        selectors: {
          notice: '#gugur-notice-bar',
          close: '#gugur-notice-bar__close'
        }
      };
    }
  }, {
    ...
    value: function onCloseClick() {
      ...
      gugurCommon.ajax.addRequest('notice_bar_dismiss');
    }
  }]);
  return _default;
}(gugurModules.ViewModule);
...
function (_gugurModules$Mod) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function onInit() {
      ...
      gugur.helpers.fetchFa4ToFa5Mapping();
      this.cache = {};
    }
  }, {
    key: "getLayout",
    value: function getLayout() {
      ...
      if (!this.layout) {
        ...
        layoutModal.addButton({
          name: 'insert_icon',
          text: gugur.translate('Insert'),
          classes: 'gugur-button gugur-button-success',
          callback: function callback() {
    ...
    value: function unMountIconManager() {
      var containerElement = document.querySelector('#gugur-icons-manager-modal .dialog-content');
    ...
    value: function loadIconLibraries() {
      if (!this.cache.loaded) {
        gugur.config.icons.libraries.forEach(function (library) {
          if ('all' === library.name) {
            return;
          }

          gugur.iconManager.library.initIconType(library);
        });
    ...
    value: function onPickerShow() {
      ...
      var selected = controlView.getControlValue(),
          icons = gugur.config.icons.libraries;
      ...
      icons.forEach(function (tab, index) {
        if (-1 === ['all', 'recommended'].indexOf(tab.name)) {
          gugur.iconManager.library.initIconType(tab, function (lib) {
            icons[index] = lib;
          });
        }
      ...
      iconManagerConfig.customIconsURL = gugur.config.customIconsURL;
...
function (_BaseModalLayout) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function getLogoOptions() {
      return {
        title: gugur.translate('icon_library')
      };
    }
  }, {
    key: "initialize",
...
function (_Component) {
  ...
  function IconsManager() {
    ...
    (0, _defineProperty2.default)((0, _assertThisInitialized2.default)(_this), "state", {
      ...
      iconTabs: gugur.config.icons.libraries,
    ...
    (0, _defineProperty2.default)((0, _assertThisInitialized2.default)(_this), "loadAllTabs", function () {
      ...
      icons.forEach(function (tabSettings) {
        ...
        gugur.iconManager.library.initIconType((0, _objectSpread2.default)({}, tabSettings), function (library) {
    ...
    (0, _defineProperty2.default)((0, _assertThisInitialized2.default)(_this), "getIconTabsLinks", function () {
      ...
      return _this.props.icons.map(function (tab) {
        ...
        var className = 'gugur-icons-manager__tab-link';

        if (isCurrentTab) {
          className += ' gugur-active';
        }
    ...
    (0, _defineProperty2.default)((0, _assertThisInitialized2.default)(_this), "getActiveTabIcons", function (activeTab) {
      ...
      if (!_this.state.loaded[activeTab]) {
        ...
        return gugur.iconManager.library.initIconType((0, _objectSpread2.default)({}, librarySettings[0]), function (library) {
          _this.cache[library.name] = library;
      ...
      return gugur.iconManager.store.getIcons(activeTab);
    });
    ...
    (0, _defineProperty2.default)((0, _assertThisInitialized2.default)(_this), "setSelected", function (selected) {
      gugur.iconManager.setSettings('selectedIcon', selected);
    ...
    (0, _defineProperty2.default)((0, _assertThisInitialized2.default)(_this), "render", function () {
      ...
      return _react.default.createElement(_react.Fragment, null, _react.default.createElement("div", {
        id: "gugur-icons-manager__sidebar",
        className: 'gugur-templates-modal__sidebar'
      }, _react.default.createElement("div", {
        id: "gugur-icons-manager__tab-links"
      }, _this.getIconTabsLinks(), _this.getUploadCustomButton(), _this.getIconTabsLinks(false))), _react.default.createElement("div", {
        id: "gugur-icons-manager__main",
        className: 'gugur-templates-modal__content'
        ...
        id: "gugur-icons-manager__tab__wrapper",
        ref: _this.scrollViewRef
      }, _react.default.createElement("div", {
        id: "gugur-icons-manager__tab__title"
      }, activeTab.label), _react.default.createElement("div", {
        id: "gugur-icons-manager__tab__content_wrapper"
  ...
  (0, _createClass2.default)(IconsManager, [{
    ...
    value: function getUploadCustomButton() {
      ...
      return _react.default.createElement("div", {
        id: "gugur-icons-manager__upload"
      }, _react.default.createElement("div", {
        id: "gugur-icons-manager__upload__title"
      }, gugur.translate('my_libraries')), _react.default.createElement("button", {
        id: "gugur-icons-manager__upload__button",
        className: "gugur-button gugur-button-default",
        onClick: onClick
      }, gugur.translate('upload')));
    }
  }, {
    key: "getSearchHTML",
    value: function getSearchHTML() {
      return _react.default.createElement("div", {
        id: "gugur-icons-manager__search"
      }, _react.default.createElement("input", {
  ...
  function Tab() {
    ...
    (0, _defineProperty2.default)((0, _assertThisInitialized2.default)(_this), "componentDidMount", function () {
      if (_this.props.selected && _this.props.selected.value) {
        setTimeout(function () {
          var element = document.querySelector('.gugur-selected');
    ...
    (0, _defineProperty2.default)((0, _assertThisInitialized2.default)(_this), "getLibrary", function (libraryName) {
      var icons = gugur.config.icons.libraries.filter(function (library) {
    ...
    (0, _defineProperty2.default)((0, _assertThisInitialized2.default)(_this), "render", function () {
      ...
      try {
        for (var _iterator = (0, _getIterator2.default)(icons.entries()), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          ...
          if (icon.props.containerClass.includes('gugur-selected')) {
            selectedIndex = index;
            break;
          }
        }
  ...
  (0, _createClass2.default)(Tab, [{
    ...
    value: function getIconsOfType(type, icons) {
      ...
      return (0, _entries.default)(icons).map(function (icon) {
        ...
        var containerClass = 'gugur-icons-manager__tab__item';

        if (selected.value === className) {
          containerClass += ' gugur-selected';
        }
...
function (_Component) {
  ...
  function LazyIconList() {
    ...
    (0, _defineProperty2.default)((0, _assertThisInitialized2.default)(_this), "selectors", {
      item: '.gugur-icons-manager__tab__item',
      wrapper: 'gugur-icons-manager__tab__wrapper'
    });
    ...
    (0, _defineProperty2.default)((0, _assertThisInitialized2.default)(_this), "render", function () {
      ...
      return _react.default.createElement(_react.Fragment, null, _react.default.createElement("div", {
        className: 'gugur-icons-manager__tab__content__offset',
        style: offsetStyle
      }), _react.default.createElement("div", {
        id: 'gugur-icons-manager__tab__content'
      }, itemsInView), _react.default.createElement("div", {
        className: 'gugur-icons-manager__tab__content__tail',
        style: tailStyle
      }));
    });
    return _this;
  ...
  function Icon() {
    ...
    (0, _defineProperty2.default)((0, _assertThisInitialized2.default)(_this), "render", function () {
      return _react.default.createElement("div", {
        ...
        className: "gugur-icons-manager__tab__item__content"
      }, _react.default.createElement("i", {
        className: 'gugur-icons-manager__tab__item__icon ' + _this.props.className
      }), _react.default.createElement("div", {
        className: 'gugur-icons-manager__tab__item__name',
...
function (_Component) {
  ...
  function IconsGoPro() {
    ...
    (0, _defineProperty2.default)((0, _assertThisInitialized2.default)(_this), "render", function () {
      return _react.default.createElement("div", {
        id: "gugur-icons-manager__promotion"
      }, _react.default.createElement("i", {
        id: "gugur-icons-manager__promotion__icon",
        className: "eicon-nerd"
      }), _react.default.createElement("div", {
        id: "gugur-icons-manager__promotion__text"
      }, gugur.translate('icons_promotion')), _react.default.createElement("a", {
        href: gugur.config.icons.goProURL,
        id: "gugur-icons-manager__promotion__link",
        target: "_blank",
        rel: "noopener noreferrer"
      }, gugur.translate('go_pro')));
    });
    return _this;
  }
...
function () {
  function _default() {
    ...
    (0, _defineProperty2.default)(this, "initIconType", function (libraryConfig, callback) {
      ...
      var store = gugur.iconManager.store;
      ...
      if (libraryConfig.enqueue) {
        libraryConfig.enqueue.forEach(function (assetURL) {
          gugur.helpers.enqueueEditorStylesheet(assetURL);
        });
      }

      if (libraryConfig.url) {
        gugur.helpers.enqueueEditorStylesheet(libraryConfig.url);
    ...
    value: function normalizeIconList(library) {
      ...
      jQuery.each(library.icons, function (index, icon) {
        ...
        icons[name] = {
          ...
          name: gugurCommon.helpers.upperCaseWords(name).trim(':').split('-').join(' '),
          filter: name.trim(':'),
      ...
      if ((0, _keys.default)(icons).length) {
        ...
        gugur.iconManager.store.save(library);
...
function () {
  ...
  (0, _createClass2.default)(Store, [{
    ...
    value: function save(library) {
      gugurCommon.storage.set(Store.getKey(library), library);
    }
  }, {
    key: "getIcons",
    ...
    value: function get(key) {
      return gugurCommon.storage.get(key);
    }
  }, {
    key: "isValid",
    ...
    value: function getKey(library) {
      ...
      return "gugur_".concat(name, "_icons");
    }
  }]);
  return Store;
}();
...
function (_gugurModules$com) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function dependency() {
      return !!gugur.getCurrentElement();
    }
  }, {
    key: "defaultCommands",
    value: function defaultCommands() {
      return {
        copy: function copy() {
          return gugur.getCurrentElement().copy();
        },
        duplicate: function duplicate() {
          return gugur.getCurrentElement().duplicate();
        },
        delete: function _delete() {
          return gugur.getCurrentElement().removeElement();
        },
        paste: function paste() {
          return gugur.getCurrentElement().paste();
        },
        pasteStyle: function pasteStyle() {
          return gugur.getCurrentElement().pasteStyle();
        }
      };
    }
  }, {
    ...
    value: function defaultShortcuts() {
      return {
        ...
        paste: {
          ...
          exclude: ['input'],
          dependency: function dependency() {
            return gugur.getCurrentElement().isPasteEnabled();
          }
        },
        pasteStyle: {
          keys: 'ctrl+shift+v',
          exclude: ['input'],
          dependency: function dependency() {
            return gugur.getCurrentElement().pasteStyle && gugurCommon.storage.get('transfer');
          }
        }
      };
    }
...
helpers = {
  ...
  enqueuePreviewStylesheet: function enqueuePreviewStylesheet(url) {
    this.enqueueCSS(url, gugur.$previewContents);
  },
  enqueueEditorStylesheet: function enqueueEditorStylesheet(url) {
    this.enqueueCSS(url, gugurCommon.elements.$document);
  },
  ...
  enqueueStylesheet: function enqueueStylesheet(url) {
    gugurCommon.helpers.deprecatedMethod('gugur.helpers.enqueueStylesheet()', '2.6.0', 'gugur.helpers.enqueuePreviewStylesheet()');
  ...
  getInlineSvg: function getInlineSvg(value, view) {
    ...
    this.fetchInlineSvg(value.url, function (data) {
      if (data) {
        ...
        gugur.channels.editor.trigger('svg:insertion', data, value.id);
      }
    });
  },
  enqueueIconFonts: function enqueueIconFonts(iconType) {
    ...
    if (-1 !== this._enqueuedIconFonts.indexOf(iconType) || !!gugurConfig['icons_update_needed']) {
      return;
    }
    ...
    gugur.channels.editor.trigger('fontIcon:insertion', iconType, iconSetting);
  },
  getIconLibrarySettings: function getIconLibrarySettings(iconType) {
    var iconSetting = gugur.config.icons.libraries.filter(function (library) {
  ...
  renderIcon: function renderIcon(view, icon) {
    ...
    gugur.channels.editor.trigger('Icon:insertion', iconType, iconValue, attributes, tag, view);
  ...
  fetchFa4ToFa5Mapping: function fetchFa4ToFa5Mapping() {
    ...
    var mapping = gugurCommon.storage.get(storageKey);

    if (!mapping) {
      jQuery.getJSON(gugurConfig.fa4_to_fa5_mapping_url, function (data) {
        mapping = data;
        gugurCommon.storage.set(storageKey, data);
      });
    }
  ...
  enqueueFont: function enqueueFont(font) {
    ...
    var fontType = gugur.config.controls.font.options[font],
        subsets = {
      ru_RU: 'cyrillic',
      uk: 'cyrillic',
      bg_BG: 'cyrillic',
    ...
    switch (fontType) {
      case 'googlefonts':
        ...
        if (subsets[gugur.config.locale]) {
          fontUrl += '&subset=' + subsets[gugur.config.locale];
        }
    ...
    gugur.channels.editor.trigger('font:insertion', fontType, font);
  },
  ...
  getSocialNetworkNameFromIcon: function getSocialNetworkNameFromIcon(iconsControl, fallbackControl) {
    ...
    social = gugur.hooks.applyFilters('gugur/social_icons/network_name', social, iconsControl, fallbackControl, toUpperCase, withIcon);
  ...
  maybeDisableWidget: function maybeDisableWidget() {
    if (!gugurConfig['icons_update_needed']) {
      return false;
    }

    var elementView = gugur.channels.panelElements.request('element:selected'),
        widgetType = elementView.model.get('widgetType'),
        widgetData = gugur.config.widgets[widgetType],
    ...
    if (widgetData) {
      ...
      if (hasIconsControl) {
        var onConfirm = function onConfirm() {
          window.location.href = gugurConfig.tools_page_link + '&redirect_to=' + encodeURIComponent(document.location.href) + '#tab-fontawesome4_migration';
        };

        gugur.helpers.getSimpleDialog('gugur-enable-fa5-dialog', gugur.translate('enable_fa5'), gugur.translate('dialog_confirm_enable_fa5'), gugur.translate('update'), onConfirm).show();
        return true;
      }
    }
  ...
  */
  ...
  isActiveControl: function isActiveControl(controlModel, values) {
    ...
    if (conditions && !gugur.conditions.check(conditions, values)) {
      return false;
    }
  ...
  cloneObject: function cloneObject(object) {
    gugurCommon.helpers.deprecatedMethod('gugur.helpers.cloneObject', '2.3.0', 'gugurCommon.helpers.cloneObject');
    return gugurCommon.helpers.cloneObject(object);
  },
  firstLetterUppercase: function firstLetterUppercase(string) {
    gugurCommon.helpers.deprecatedMethod('gugur.helpers.upperCaseWords', '2.3.0', 'gugurCommon.helpers.upperCaseWords');
    return gugurCommon.helpers.upperCaseWords(string);
  },
  ...
  wpColorPicker: function wpColorPicker($element, options) {
    var self = this,
        colorPickerScheme = gugur.schemes.getScheme('color-picker'),
  ...
  scrollToView: function scrollToView($element, timeout, $parent) {
    ...
    var $scrolled = $parent;
    var $gugurFrontendWindow = gugurFrontend.elements.$window;

    if (!$parent) {
      $parent = $gugurFrontendWindow;
      $scrolled = gugur.$previewContents.find('html, body');
    }

    setTimeout(function () {
      var parentHeight = $parent.height(),
          parentScrollTop = $parent.scrollTop(),
          elementTop = $parent === $gugurFrontendWindow ? $element.offset().top : $element[0].offsetTop,
  ...
  elementSizeToUnit: function elementSizeToUnit($element, size, unit) {
    var window = gugurFrontend.elements.window;
  ...
  compareVersions: function compareVersions(versionA, versionB, operator) {
    ...
    for (var i = 0; i < longestVersionParts; i++) {
      ...
      if (valueA !== valueB) {
        return gugur.conditions.compare(valueA, valueB, operator);
      }
    }
  }
};
...
ImagesManager = function ImagesManager() {
  ...
  self.updateOnReceiveImage = function () {
    var elementView = gugur.getPanelView().getCurrentPageView().getOption('editedElementView');
    elementView.$el.addClass('gugur-loading'); // Add per cid for multiple images in a single view.

    viewsToUpdate[elementView.cid] = elementView;
    gugur.channels.editor.once('imagesManager:detailsReceived', function () {
      if (!_.isEmpty(viewsToUpdate)) {
        _(viewsToUpdate).each(function (view) {
          view.render();
          view.$el.removeClass('gugur-loading');
        });
      }
  ...
  self.getRemoteItems = function () {
    ...
    gugurCommon.ajax.send('get_images_details', {
      data: {
      ...
      success: function success(data) {
        ...
        registeredItems = [];
        gugur.channels.editor.trigger('imagesManager:detailsReceived', data);
      }
    });
  };
...
var Debug = function Debug() {
  ...
  this.sendErrors = function () {
    ...
    jQuery.ajax({
      url: gugurCommon.config.ajax.url,
      method: 'POST',
      data: {
        action: 'gugur_js_log',
        _nonce: gugurCommon.ajax.getSettings('nonce'),
        data: errorStack
      },
      success: function success() {
...
Schemes = function Schemes() {
  var self = this,
      ...
      schemes = {},
      settings = {
    selectorWrapperPrefix: '.gugur-widget-'
  },
      elements = {};
  ...
  var initElements = function initElements() {
    elements.$style = jQuery('<style>', {
      id: 'gugur-style-scheme'
    });
    elements.$previewHead = gugur.$previewContents.find('head');
  };

  var initSchemes = function initSchemes() {
    schemes = gugurCommon.helpers.cloneObject(gugur.config.schemes.items);
  };
  ...
  var fetchAllWidgetsSchemesStyle = function fetchAllWidgetsSchemesStyle() {
    _.each(gugur.config.widgets, function (widget) {
  ...
  this.getEnabledSchemesTypes = function () {
    return gugur.config.schemes.enabled_schemes;
  };
  ...
  this.getSchemeValue = function (schemeType, value, key) {
    ...
    if (key && _.isObject(schemeValue)) {
      var clonedSchemeValue = gugurCommon.helpers.cloneObject(schemeValue);
  ...
  this.resetSchemes = function (schemeName) {
    schemes[schemeName] = gugurCommon.helpers.cloneObject(gugur.config.schemes.items[schemeName]);
  };

  this.saveScheme = function (schemeName) {
    gugur.config.schemes.items[schemeName].items = gugurCommon.helpers.cloneObject(schemes[schemeName].items);
    var itemsToSave = {};
    ...
    NProgress.start();
    gugurCommon.ajax.addRequest('apply_scheme', {
      data: {
...
presetsFactory = {
  ...
  getAbsolutePresetValues: function getAbsolutePresetValues(preset) {
    var clonedPreset = gugurCommon.helpers.cloneObject(preset),
  ...
  getPresets: function getPresets(columnsCount, presetIndex) {
    var presets = gugurCommon.helpers.cloneObject(gugur.config.elements.section.presets);
...
TemplateLibraryManager = function TemplateLibraryManager() {
  ...
  var registerDefaultTemplateTypes = function registerDefaultTemplateTypes() {
    var data = {
      saveDialog: {
        description: gugur.translate('save_your_template_description')
      },
      ajaxParams: {
    ...
    _.each(['page', 'section', gugur.config.document.type], function (type) {
      var safeData = jQuery.extend(true, {}, data, {
        saveDialog: {
          title: gugur.translate('save_your_template', [gugur.translate(type)])
        }
      });
  ...
  this.init = function () {
    ...
    gugur.addBackgroundClickListener('libraryToggleMore', {
      element: '.gugur-template-library-template-more'
    });
  };
  ...
  this.deleteTemplate = function (templateModel, options) {
    ...
    dialog.onConfirm = function () {
      ...
      gugurCommon.ajax.addRequest('delete_template', {
        data: {
  ...
  this.importTemplate = function (templateModel, options) {
    ...
    self.requestTemplateContent(templateModel.get('source'), templateModel.get('template_id'), {
      ...
      success: function success(data) {
        ...
        gugur.channels.data.trigger('template:before:insert', templateModel);
        gugur.getPreviewView().addChildModel(data.content, importOptions);
        gugur.channels.data.trigger('template:after:insert', templateModel);

        if (options.withPageSettings) {
          gugur.settings.page.model.setExternalChange(data.page_settings);
        }
      },
  ...
  this.saveTemplate = function (type, data) {
    ...
    gugurCommon.ajax.addRequest('save_template', ajaxParams);
  };
  ...
  this.requestTemplateContent = function (source, id, ajaxOptions) {
    ...
    return gugurCommon.ajax.addRequest('get_template_data', options);
  };
  ...
  this.markAsFavorite = function (templateModel, favorite) {
    ...
    return gugurCommon.ajax.addRequest('mark_template_as_favorite', options);
  };

  this.getDeleteDialog = function () {
    if (!deleteDialog) {
      deleteDialog = gugurCommon.dialogsManager.createWidget('confirm', {
        id: 'gugur-template-library-delete-dialog',
        headerMessage: gugur.translate('delete_template'),
        message: gugur.translate('delete_template_confirm'),
        strings: {
          confirm: gugur.translate('delete')
        }
      });
  ...
  this.getErrorDialog = function () {
    if (!errorDialog) {
      errorDialog = gugurCommon.dialogsManager.createWidget('alert', {
        id: 'gugur-template-library-error-dialog',
        headerMessage: gugur.translate('an_error_occurred')
      });
    }
  ...
  this.requestLibraryData = function (options) {
    ...
    gugurCommon.ajax.addRequest('get_library_data', ajaxOptions);
  };

  this.getFilter = function (name) {
    return gugur.channels.templates.request('filter:' + name);
  };

  this.setFilter = function (name, value, silent) {
    gugur.channels.templates.reply('filter:' + name, value);

    if (!silent) {
      gugur.channels.templates.trigger('filter:change');
    }
  };
  ...
  this.setScreen = function (args) {
    gugur.channels.templates.stopReplying();
  ...
  this.showErrorDialog = function (errorMessage) {
    ...
    self.getErrorDialog().setMessage(gugur.translate('templates_request_error') + '<div id="gugur-template-library-error-info">' + errorMessage + '</div>').show();
  };
};
...
function (_gugurModules$com) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function __construct(args) {
      ...
      this.docLibraryConfig = gugur.config.document.remoteLibrary;
    ...
    value: function defaultTabs() {
      return {
        ...
        'templates/my-templates': {
          title: gugur.translate('my_templates'),
          filter: {
            source: 'local'
          }
        }
    ...
    value: function getTabsWrapperSelector() {
      return '#gugur-template-library-header-menu';
    }
  }, {
    key: "renderTab",
...
module.exports = gugurModules.common.views.modal.Layout.extend({
  getModalOptions: function getModalOptions() {
    return {
      id: 'gugur-template-library-modal'
    };
  },
  getLogoOptions: function getLogoOptions() {
    return {
      title: gugur.translate('library'),
  ...
  getTemplateActionButton: function getTemplateActionButton(templateData) {
    var viewId = '#tmpl-gugur-template-library-' + (templateData.isPro ? 'get-pro-button' : 'insert-button');
    viewId = gugur.hooks.applyFilters('gugur/editor/template-library/template/action-button', viewId, templateData);
...
module.exports = Marionette.ItemView.extend({
  template: '#tmpl-gugur-template-library-header-actions',
  id: 'gugur-template-library-header-actions',
  ui: {
    import: '#gugur-template-library-header-import i',
    sync: '#gugur-template-library-header-sync i',
    save: '#gugur-template-library-header-save i'
  },
  events: {
  ...
  onSyncClick: function onSyncClick() {
    ...
    gugur.templates.requestLibraryData({
      onUpdate: function onUpdate() {
...
module.exports = Marionette.ItemView.extend({
  template: '#tmpl-gugur-template-library-header-menu',
  id: 'gugur-template-library-header-menu',
  templateHelpers: function templateHelpers() {
    return {
...
module.exports = Marionette.ItemView.extend({
  template: '#tmpl-gugur-template-library-header-preview',
  id: 'gugur-template-library-header-preview',
  behaviors: {
    insertTemplate: {
      behaviorClass: TemplateLibraryInsertTemplateBehavior
    }
...
module.exports = Marionette.ItemView.extend({
  template: '#tmpl-gugur-template-library-header-back',
  id: 'gugur-template-library-header-preview-back',
  events: {
    click: 'onClick'
  },
  onClick: function onClick() {
...
TemplateLibraryCollectionView = Marionette.CompositeView.extend({
  template: '#tmpl-gugur-template-library-templates',
  id: 'gugur-template-library-templates',
  childViewContainer: '#gugur-template-library-templates-container',
  reorderOnSort: true,
  ...
  ui: {
    textFilter: '#gugur-template-library-filter-text',
    selectFilter: '.gugur-template-library-filter-select',
    myFavoritesFilter: '#gugur-template-library-filter-my-favorites',
    orderInputs: '.gugur-template-library-order-input',
    orderLabels: 'label.gugur-template-library-order-label'
  },
  events: {
  ...
  initialize: function initialize() {
    this.listenTo(gugur.channels.templates, 'filter:change', this._renderChildren);
  },
  filter: function filter(childModel) {
    var filterTerms = gugur.templates.getFilterTerms(),
        passingFilter = true;
    jQuery.each(filterTerms, function (filterTermName) {
      var filterValue = gugur.templates.getFilter(filterTermName);
  ...
  addSourceData: function addSourceData() {
    ...
    this.$el.attr('data-template-source', isEmpty ? 'empty' : gugur.templates.getFilter('source'));
  },
  setFiltersUI: function setFiltersUI() {
    var $filters = this.$(this.ui.selectFilter);
    $filters.select2({
      placeholder: gugur.translate('category'),
      allowClear: true,
      width: 150
    });
  },
  setMasonrySkin: function setMasonrySkin() {
    var masonry = new gugurModules.utils.Masonry({
  ...
  toggleFilterClass: function toggleFilterClass() {
    this.$el.toggleClass('gugur-templates-filter-active', !!(gugur.templates.getFilter('text') || gugur.templates.getFilter('favorite')));
  },
  onRender: function onRender() {
    if ('remote' === gugur.templates.getFilter('source') && 'page' !== gugur.templates.getFilter('type')) {
      this.setFiltersUI();
    }
  },
  onRenderCollection: function onRenderCollection() {
  ...
  onTextFilterInput: function onTextFilterInput() {
    gugur.templates.setFilter('text', this.ui.textFilter.val());
  },
  onSelectFilterChange: function onSelectFilterChange(event) {
    var $select = jQuery(event.currentTarget),
        filterName = $select.data('gugur-filter');
    gugur.templates.setFilter(filterName, $select.val());
  },
  onMyFavoritesFilterChange: function onMyFavoritesFilterChange() {
    gugur.templates.setFilter('favorite', this.ui.myFavoritesFilter[0].checked);
  },
  onOrderLabelsClick: function onOrderLabelsClick(event) {
    ...
    $clickedInput.toggleClass('gugur-template-library-order-reverse', toggle);
    this.order($clickedInput.val(), $clickedInput.hasClass('gugur-template-library-order-reverse'));
  }
});
...
TemplateLibraryTemplateLocalView = TemplateLibraryTemplateView.extend({
  template: '#tmpl-gugur-template-library-template-local',
  ui: function ui() {
    return _.extend(TemplateLibraryTemplateView.prototype.ui.apply(this, arguments), {
      deleteButton: '.gugur-template-library-template-delete',
      morePopup: '.gugur-template-library-template-more',
      toggleMore: '.gugur-template-library-template-more-toggle',
      toggleMoreIcon: '.gugur-template-library-template-more-toggle i'
    });
  },
  events: function events() {
...
TemplateLibraryTemplateRemoteView = TemplateLibraryTemplateView.extend({
  template: '#tmpl-gugur-template-library-template-remote',
  ui: function ui() {
    return jQuery.extend(TemplateLibraryTemplateView.prototype.ui.apply(this, arguments), {
      favoriteCheckbox: '.gugur-template-library-template-favorite-input'
    });
  },
  events: function events() {
  ...
  onFavoriteCheckboxChange: function onFavoriteCheckboxChange() {
    ...
    gugur.templates.markAsFavorite(this.model, isFavorite);

    if (!isFavorite && gugur.templates.getFilter('favorite')) {
      gugur.channels.templates.trigger('filter:change');
    }
  }
});
...
TemplateLibraryTemplatesEmptyView = Marionette.ItemView.extend({
  id: 'gugur-template-library-templates-empty',
  template: '#tmpl-gugur-template-library-templates-empty',
  ui: {
    title: '.gugur-template-library-blank-title',
    message: '.gugur-template-library-blank-message'
  },
  modesStrings: {
    empty: {
      title: gugur.translate('templates_empty_title'),
      message: gugur.translate('templates_empty_message')
    },
    noResults: {
      title: gugur.translate('templates_no_results_title'),
      message: gugur.translate('templates_no_results_message')
    },
    noFavorites: {
      title: gugur.translate('templates_no_favorites_title'),
      message: gugur.translate('templates_no_favorites_message')
    }
  },
  getCurrentMode: function getCurrentMode() {
    if (gugur.templates.getFilter('text')) {
      return 'noResults';
    }

    if (gugur.templates.getFilter('favorite')) {
      return 'noFavorites';
    }
...
TemplateLibrarySaveTemplateView = Marionette.ItemView.extend({
  id: 'gugur-template-library-save-template',
  template: '#tmpl-gugur-template-library-save-template',
  ui: {
    form: '#gugur-template-library-save-template-form',
    submitButton: '#gugur-template-library-save-template-submit'
  },
  events: {
  ...
  getSaveType: function getSaveType() {
    ...
    } else if (gugur.config.document.library && gugur.config.document.library.save_as_same_type) {
      type = gugur.config.document.type;
    } else {
      type = 'page';
    }
  ...
  templateHelpers: function templateHelpers() {
    var saveType = this.getSaveType(),
        templateType = gugur.templates.getTemplateTypes(saveType);
    return templateType.saveDialog;
  },
  onFormSubmit: function onFormSubmit(event) {
    event.preventDefault();
    var formData = this.ui.form.gugurSerializeObject(),
        saveType = this.getSaveType(),
        JSONParams = {
      remove: ['default']
    };
    formData.content = this.model ? [this.model.toJSON(JSONParams)] : gugur.elements.toJSON(JSONParams);
    this.ui.submitButton.addClass('gugur-button-state');
    gugur.templates.saveTemplate(saveType, formData);
  }
});
...
TemplateLibraryImportView = Marionette.ItemView.extend({
  template: '#tmpl-gugur-template-library-import',
  id: 'gugur-template-library-import',
  ui: {
    uploadForm: '#gugur-template-library-import-form',
    fileInput: '#gugur-template-library-import-form-input'
  },
  events: {
  ...
  importTemplate: function importTemplate(fileName, fileData) {
    var layout = gugur.templates.layout;
    var options = {
      data: {
        fileName: fileName,
        fileData: fileData
      ...
      success: function success(successData) {
        gugur.templates.getTemplatesCollection().add(successData);
        $e.route('library/templates/my-templates');
      },
      error: function error(errorData) {
        gugur.templates.showErrorDialog(errorData);
    ...
    gugurCommon.ajax.addRequest('import_template', options);
  ...
  onFormDragEnter: function onFormDragEnter() {
    this.ui.uploadForm.addClass('gugur-drag-over');
  },
  onFormDragLeave: function onFormDragLeave(event) {
    ...
    this.ui.uploadForm.removeClass('gugur-drag-over');
  },
...
TemplateLibraryPreviewView = Marionette.ItemView.extend({
  template: '#tmpl-gugur-template-library-preview',
  id: 'gugur-template-library-preview',
  ui: {
    iframe: '> iframe'
  },
  onRender: function onRender() {
...
Manager = function Manager() {
  ...
  var addPanelPage = function addPanelPage() {
    gugur.getPanelView().addPage('historyPage', {
      view: HistoryPageView,
      title: gugur.translate('history')
    });
  };

  var init = function init() {
    gugur.on('preview:loaded', addPanelPage);
    $e.components.register(new _component.default({
      manager: self
    }));
  ...
  jQuery(window).on('gugur:init', init);
};
...
function (_gugurModules$com) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function defaultTabs() {
      return {
        actions: {
          title: gugur.translate('actions')
        },
        revisions: {
          title: gugur.translate('revisions')
        }
      };
    }
  }, {
    ...
    key: "getTabsWrapperSelector",
    value: function getTabsWrapperSelector() {
      return '#gugur-panel-elements-navigation';
    }
  }, {
    key: "renderTab",
    value: function renderTab(tab) {
      gugur.getPanelView().setPage('historyPage').showView(tab);
    }
  }]);
  return _default;
}(gugurModules.common.Component);
...
module.exports = Marionette.LayoutView.extend({
  template: '#tmpl-gugur-panel-history-page',
  regions: {
    content: '#gugur-panel-history-content'
  },
  ui: {
    tabs: '.gugur-panel-navigation-tab'
  },
  regionViews: {},
  currentTab: null,
  initialize: function initialize() {
  ...
  initRegionViews: function initRegionViews() {
    var historyItems = gugur.history.history.getItems();
    this.regionViews = {
      actions: {
        view: function view() {
      ...
      revisions: {
        view: function view() {
          var revisionsItems = gugur.history.revisions.getItems();
...
function (_Marionette$ItemView) {
  ...
  (0, _createClass2.default)(_default, [{
    key: "getTemplate",
    value: function getTemplate() {
      return '#tmpl-gugur-panel-revisions-loading';
    }
  }, {
    key: "id",
    value: function id() {
      return 'gugur-panel-revisions-loading';
    }
  }, {
    key: "onRender",
    value: function onRender() {
      gugur.history.revisions.requestRevisions(function () {
        setTimeout(function () {
...
module.exports = Marionette.CompositeView.extend({
  id: 'gugur-panel-revisions',
  template: '#tmpl-gugur-panel-revisions',
  childView: __webpack_require__(363),
  childViewContainer: '#gugur-revisions-list',
  ui: {
    discard: '.gugur-panel-scheme-discard .gugur-button',
    apply: '.gugur-panel-scheme-save .gugur-button'
  },
  events: {
  ...
  initialize: function initialize() {
    this.collection = gugur.history.revisions.getItems();
    this.listenTo(gugur.channels.editor, 'saved', this.onEditorSaved);
    this.currentPreviewId = gugur.config.current_revision_id;
  },
  getRevisionViewData: function getRevisionViewData(revisionView) {
    var self = this;
    gugur.history.revisions.getRevisionDataAsync(revisionView.model.get('id'), {
      success: function success(data) {
        gugur.history.revisions.setEditorData(data.elements);
        gugur.settings.page.model.set(data.settings);
        self.setRevisionsButtonsActive(true);
        revisionView.$el.removeClass('gugur-revision-item-loading');
  ...
  deleteRevision: function deleteRevision(revisionView) {
    var self = this;
    revisionView.$el.addClass('gugur-revision-item-loading');
    gugur.history.revisions.deleteRevision(revisionView.model, {
      success: function success() {
  ...
  enterReviewMode: function enterReviewMode() {
    gugur.changeEditMode('review');
  },
  exitReviewMode: function exitReviewMode() {
    gugur.changeEditMode('edit');
  },
  ...
  onApplyClick: function onApplyClick() {
    ...
    gugur.history.history.getItems().reset();
  },
  onDiscardClick: function onDiscardClick() {
    gugur.history.revisions.setEditorData(gugur.config.data);
    gugur.saver.setFlagEditorChange(this.isRevisionApplied);
    ...
    if (this.currentPreviewItem) {
      this.currentPreviewItem.$el.removeClass('gugur-revision-current-preview');
    }
  },
  onDestroy: function onDestroy() {
    if (this.currentPreviewId && this.currentPreviewId !== gugur.config.current_revision_id) {
      this.onDiscardClick();
    }
  },
  onRenderCollection: function onRenderCollection() {
    ...
    if (currentPreviewModel) {
      ...
      this.currentPreviewItem.$el.addClass('gugur-revision-current-preview');
    }
  },
  onChildviewDetailsAreaClick: function onChildviewDetailsAreaClick(childView) {
    ...
    if (self.currentPreviewItem) {
      self.currentPreviewItem.$el.removeClass('gugur-revision-current-preview gugur-revision-item-loading');
    }

    childView.$el.addClass('gugur-revision-current-preview gugur-revision-item-loading');

    if (gugur.saver.isEditorChanged() && (null === self.currentPreviewId || gugur.config.current_revision_id === self.currentPreviewId)) {
      gugur.saver.saveEditor({
        status: 'autosave',
        onSuccess: function onSuccess() {
  ...
  onChildviewDeleteClick: function onChildviewDeleteClick(childView) {
    ...
    var removeDialog = gugurCommon.dialogsManager.createWidget('confirm', {
      message: gugur.translate('dialog_confirm_delete', [type]),
      headerMessage: gugur.translate('delete_element', [type]),
      strings: {
        confirm: gugur.translate('delete'),
        cancel: gugur.translate('cancel')
      },
...
module.exports = Marionette.ItemView.extend({
  template: '#tmpl-gugur-panel-revisions-revision-item',
  className: 'gugur-revision-item',
  ui: {
    detailsArea: '.gugur-revision-item__details',
    deleteButton: '.gugur-revision-item__tools-delete'
  },
  triggers: {
...
module.exports = Marionette.ItemView.extend({
  template: '#tmpl-gugur-panel-revisions-no-revisions',
  id: 'gugur-panel-revisions-no-revisions',
  className: 'gugur-nerd-box'
});
...
module.exports = Marionette.CompositeView.extend({
  id: 'gugur-panel-history',
  template: '#tmpl-gugur-panel-history-tab',
  childView: _itemView.default,
  childViewContainer: '#gugur-history-list',
  ...
  updateCurrentItem: function updateCurrentItem() {
    ...
    _.defer(function () {
      ...
      var currentItemClass = 'gugur-history-item-current';
  ...
  onChildviewClick: function onChildviewClick(childView, event) {
    ...
    gugur.history.history.doItem(itemIndex);
  }
});
...
function (_Marionette$ItemView) {
  ...
  (0, _createClass2.default)(_default, [{
    key: "getTemplate",
    value: function getTemplate() {
      return '#tmpl-gugur-panel-history-item';
    }
  }, {
    key: "className",
    value: function className() {
      return 'gugur-history-item gugur-history-item-' + this.model.get('status');
    }
  }, {
    key: "triggers",
...
function (_Marionette$ItemView) {
  ...
  (0, _createClass2.default)(_default, [{
    key: "getTemplate",
    value: function getTemplate() {
      return '#tmpl-gugur-panel-history-no-items';
    }
  }, {
    key: "id",
    value: function id() {
      return 'gugur-panel-history-no-items';
    }
  }, {
    key: "onDestroy",
    value: function onDestroy() {
      this._parent.$el.removeClass('gugur-empty');
    }
  }]);
  return _default;
}(Marionette.ItemView);
...
var Manager = function Manager() {
  var self = this,
    ...
    model: _itemModel.default
      ...
      active = true;
  var translations = {
    add: gugur.translate('added'),
    remove: gugur.translate('removed'),
    change: gugur.translate('edited'),
    move: gugur.translate('moved'),
    paste_style: gugur.translate('style_pasted'),
    reset_style: gugur.translate('style_reset')
  };
  ...
  var updatePanelPageCurrentItem = function updatePanelPageCurrentItem() {
    if ($e.routes.is('panel/history/actions')) {
      gugur.getPanelView().getCurrentPageView().getCurrentTab().updateCurrentItem();
    }
  };
  ...
  this.addItem = function (itemData) {
    ...
    if (!items.length) {
      items.add({
        status: 'not_applied',
        title: gugur.translate('editing_started'),
        subTitle: '',
        action: '',
  ...
  this.doItem = function (index) {
    ...
    if (viewToScroll && !gugur.helpers.isInViewport(viewToScroll.$el[0], gugur.$previewContents.find('html')[0])) {
      gugur.helpers.scrollToView(viewToScroll.$el);
    }

    if (item.get('editing_started')) {
      if (!editorSaved) {
        gugur.saver.setFlagEditorChange(false);
      }
    }
  };
  ...
  this.getModelLabel = function (model) {
    ...
    return gugur.getElementData(model).title;
  };
  ...
  this.findView = function (modelID, views) {
    ...
    if (!views) {
      views = gugur.getPreviewView().children;
    }
  ...
  this.startInsertTemplate = function (model) {
    gugur.history.history.startItem({
      type: 'add',
      title: gugur.translate('template'),
...
module.exports = Marionette.Behavior.extend({
  ...
  saveTextHistory: function saveTextHistory(model, changed, control) {
    ...
    var historyItem = {
      type: 'change',
      elementType: 'control',
      title: gugur.history.history.getModelLabel(model),
  ...
  saveHistory: function saveHistory(model, options) {
    ...
    if (1 === changed.length) {
      ...
      if (gugur.history.history.isItemStarted()) {
  ...
  restore: function restore(historyItem, isRedo) {
    var history = historyItem.get('history'),
        ...
        view = gugur.history.history.findView(modelID);
...
module.exports = Marionette.Behavior.extend({
  ...
  onDeleteAllContent: function onDeleteAllContent(collection, event) {
    ...
    var historyItem = {
      type: 'remove',
      elementType: 'section',
      title: gugur.translate('all_content'),
      history: {
        behavior: this,
  ...
  saveCollectionHistory: function saveCollectionHistory(collection, event) {
    ...
    var title = gugur.history.history.getModelLabel(firstModel); // If it's an unknown model - don't save
  ...
  add: function add(models, toView, position) {
    ...
    } // Fix for case the iframe has been reloaded and the old `gugur-inner` is not exist.


    if (toView.$el.hasClass('gugur-inner') && toView.$el[0].ownerDocument !== gugur.$previewContents[0]) {
      toView = gugur.getPreviewView();
    }
...
module.exports = gugurModules.Module.extend({
  getDefaultSettings: function getDefaultSettings() {
    return {
      actions: {},
      classes: {
        list: 'gugur-context-menu-list',
        group: 'gugur-context-menu-list__group',
        groupPrefix: 'gugur-context-menu-list__group-',
        item: 'gugur-context-menu-list__item',
        itemTypePrefix: 'gugur-context-menu-list__item-',
        itemTitle: 'gugur-context-menu-list__item__title',
        itemShortcut: 'gugur-context-menu-list__item__shortcut',
        iconShortcut: 'gugur-context-menu-list__item__icon',
        itemDisabled: 'gugur-context-menu-list__item--disabled',
        divider: 'gugur-context-menu-list__divider',
        hidden: 'gugur-hidden'
      }
    };
  },
  ...
  initModal: function initModal() {
    ...
    this.getModal = function () {
      if (!modal) {
        modal = gugurCommon.dialogsManager.createWidget('simple', {
          className: 'gugur-context-menu',
          message: this.buildActionsList(),
          iframe: gugur.$preview,
          effects: {
            hide: 'hide',
            show: 'show'
          },
          ...
          position: {
            my: (gugurCommon.config.isRTL ? 'right' : 'left') + ' top',
            collision: 'fit'
          }
        });
      }
...
function (_BaseAddSectionView) {
  ...
  (0, _createClass2.default)(AddSectionView, [{
    key: "className",
    value: function className() {
      return (0, _get2.default)((0, _getPrototypeOf2.default)(AddSectionView.prototype), "className", this).call(this) + ' gugur-add-section-inline';
    }
  }, {
    key: "fadeToDeath",
...
ColumnView = BaseElementView.extend({
  template: Marionette.TemplateCache.get('#tmpl-gugur-column-content'),
  emptyView: ColumnEmptyView,
  childViewContainer: '> .gugur-column-wrap > .gugur-widget-wrap',
  ...
  behaviors: function behaviors() {
    ...
    return gugur.hooks.applyFilters('elements/column/behaviors', behaviors, this);
  },
  className: function className() {
    var classes = BaseElementView.prototype.className.apply(this, arguments),
        type = this.isInner() ? 'inner' : 'top';
    return classes + ' gugur-column gugur-' + type + '-column';
  },
  tagName: function tagName() {
  ...
  ui: function ui() {
    ...
    ui.columnInner = '> .gugur-column-wrap';
    ui.percentsTooltip = '> .gugur-element-overlay .gugur-column-percents-tooltip';
    return ui;
  },
  getEditButtons: function getEditButtons() {
    var elementData = gugur.getElementData(this.model),
  ...
  attachElContent: function attachElContent() {
    ...
    var $tooltip = jQuery('<div>', {
      class: 'gugur-column-percents-tooltip'
    });
    this.$el.children('.gugur-element-overlay').append($tooltip);
  },
  getContextMenuGroups: function getContextMenuGroups() {
    var groups = BaseElementView.prototype.getContextMenuGroups.apply(this, arguments),
    ...
    groups.splice(generalGroupIndex + 1, 0, {
      ...
      actions: [{
        name: 'addNew',
        icon: 'eicon-plus',
        title: gugur.translate('new_column'),
  ...
  getSortableOptions: function getSortableOptions() {
    return {
      connectWith: '.gugur-widget-wrap',
      items: '> .gugur-element'
    };
  },
  changeChildContainerClasses: function changeChildContainerClasses() {
    var emptyClass = 'gugur-element-empty',
        populatedClass = 'gugur-element-populated';
  ...
  onRender: function onRender() {
    ...
    self.$el.html5Droppable({
      items: ' > .gugur-column-wrap > .gugur-widget-wrap > .gugur-element, >.gugur-column-wrap > .gugur-widget-wrap > .gugur-empty-view > .gugur-first-add',
      axis: ['vertical'],
      groups: ['gugur-element'],
      isDroppingAllowed: self.isDroppingAllowed.bind(self),
      currentElementClass: 'gugur-html5dnd-current-element',
      placeholderClass: 'gugur-sortable-placeholder gugur-widget-placeholder',
      hasDraggingOnChildClass: 'gugur-dragging-on-child',
      onDropping: function onDropping(side, event) {
        ...
        gugur.getPreviewView().onPanelElementDragEnd();
...
module.exports = Marionette.ItemView.extend({
  template: '#tmpl-gugur-empty-preview',
  className: 'gugur-empty-view',
  events: {
    click: 'onClickAdd'
  },
  behaviors: function behaviors() {
  ...
  paste: function paste() {
    var self = this,
        elements = gugurCommon.storage.get('transfer').elements,
        index = 0;
    elements.forEach(function (item) {
...
ResizableBehavior = Marionette.Behavior.extend({
  defaults: {
    handles: gugurCommon.config.isRTL ? 'w' : 'e'
  },
  events: {
  ...
  initialize: function initialize() {
    ...
    this.listenTo(gugur.channels.dataEditMode, 'switch', this.onEditModeSwitched);
  },
...
RevisionsManager = function RevisionsManager() {
  ...
  var attachEvents = function attachEvents() {
    gugur.channels.editor.on('saved', onEditorSaved);
  };
  ...
  this.requestRevisions = function (callback) {
    ...
    gugurCommon.ajax.addRequest('get_revisions', {
      success: function success(data) {
  ...
  this.setEditorData = function (data) {
    var collection = gugur.getRegion('sections').currentView.collection; // Don't track in history.
  ...
  this.getRevisionDataAsync = function (id, options) {
    ...
    return gugurCommon.ajax.addRequest('get_revision_data', options);
  };
  ...
  this.deleteRevision = function (revisionModel, options) {
    ...
    gugurCommon.ajax.addRequest('delete_revision', params);
  };
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function navigate(up) {
      if (this.manager.getItems().length > 1) {
        gugur.getPanelView().getCurrentPageView().currentTab.navigate(up);
      }
    }
  }]);
  return _default;
...
module.exports = Marionette.Behavior.extend({
  ...
  ui: function ui() {
    return {
      buttonPreview: '#gugur-panel-footer-saver-preview',
      buttonPublish: '#gugur-panel-saver-button-publish',
      buttonSaveOptions: '#gugur-panel-saver-button-save-options',
      buttonPublishLabel: '#gugur-panel-saver-button-publish-label',
      menuSaveDraft: '#gugur-panel-footer-sub-menu-item-save-draft',
      lastEditedWrapper: '.gugur-last-edited-wrapper'
    };
  },
  events: function events() {
    return {
  ...
  initialize: function initialize() {
    gugur.saver.on('before:save', this.onBeforeSave.bind(this)).on('after:save', this.onAfterSave.bind(this)).on('after:saveError', this.onAfterSaveError.bind(this)).on('page:status:change', this.onPageStatusChange);
    gugur.settings.page.model.on('change', this.onPageSettingsChange.bind(this));
    gugur.channels.editor.on('status:change', this.activateSaveButtons.bind(this));
  },
  activateSaveButtons: function activateSaveButtons(hasChanges) {
    hasChanges = hasChanges || 'draft' === gugur.settings.page.model.get('post_status');
    this.ui.buttonPublish.add(this.ui.menuSaveDraft).toggleClass('gugur-disabled', !hasChanges);
    this.ui.buttonSaveOptions.toggleClass('gugur-disabled', !hasChanges);
  },
  onRender: function onRender() {
    this.setMenuItems(gugur.settings.page.model.get('post_status'));
    this.addTooltip();
  },
  ...
  onPageStatusChange: function onPageStatusChange(newStatus) {
    if ('publish' === newStatus) {
      gugur.notifications.showToast({
        message: gugur.config.document.panel.messages.publish_notification,
        buttons: [{
          name: 'view_page',
          text: gugur.translate('have_a_look'),
          callback: function callback() {
            open(gugur.config.document.urls.permalink);
          }
        }]
      });
    }
  ...
  onBeforeSave: function onBeforeSave(options) {
    ...
    if ('autosave' === options.status) {
      this.ui.lastEditedWrapper.addClass('gugur-state-active');
    } else {
      this.ui.buttonPublish.addClass('gugur-button-state');
    }
  },
  onAfterSave: function onAfterSave(data) {
    NProgress.done();
    this.ui.buttonPublish.removeClass('gugur-button-state');
    this.ui.lastEditedWrapper.removeClass('gugur-state-active');
    this.refreshWpPreview();
    this.setLastEdited(data);
  },
  setLastEdited: function setLastEdited(data) {
    this.ui.lastEditedWrapper.removeClass('gugur-button-state').find('.gugur-last-edited').html(data.config.document.last_edited);
  },
  ...
  onClickButtonPreview: function onClickButtonPreview() {
    ...
    this.previewWindow = open(gugur.config.document.urls.wp_preview, 'wp-preview-' + gugur.config.document.id);

    if (gugur.saver.isEditorChanged()) {
      // Force save even if it's saving now.
      if (gugur.saver.isSaving) {
        gugur.saver.isSaving = false;
      }

      gugur.saver.doAutoSave();
    }
  },
  onClickButtonPublish: function onClickButtonPublish() {
    if (this.ui.buttonPublish.hasClass('gugur-disabled')) {
      return;
    }
  ...
  setMenuItems: function setMenuItems(postStatus) {
    ...
    switch (postStatus) {
      ...
      case 'draft':
        if (!gugur.config.current_user_can_publish) {
          publishLabel = 'submit';
        }
    ...
    this.ui.buttonPublishLabel.html(gugur.translate(publishLabel));
  },
  addTooltip: function addTooltip() {
    // Create tooltip on controls
  ...
  refreshWpPreview: function refreshWpPreview() {
    if (this.previewWindow) {
      ...
      try {
        this.previewWindow.location.href = gugur.config.document.urls.wp_preview;
...
module.exports = ControlBaseView.extend({
  ...
  onButtonClick: function onButtonClick() {
    ...
    gugur.channels.editor.trigger(eventName, this);
  }
});
...
ControlCodeEditorItemView = ControlBaseDataView.extend({
  ...
  onReady: function onReady() {
    ...
    jQuery(self.editor.container).addClass('gugur-input-style gugur-code-editor');
    ...
    gugur.panel.$el.on('resize.aceEditor', self.onResize.bind(this));
  ...
  onDestroy: function onDestroy() {
    gugur.panel.$el.off('resize.aceEditor');
  }
});
...
ControlColorItemView = ControlBaseDataView.extend({
  applySavedValue: function applySavedValue() {
    ...
    if (colorInstance) {
      ...
      gugur.helpers.wpColorPicker(self.ui.input, {
        change: function change() {
...
ControlDimensionsItemView = ControlBaseUnitsItemView.extend({
  ui: function ui() {
    ...
    ui.controls = '.gugur-control-dimension > input:enabled';
    ui.link = 'button.gugur-link-dimensions';
    return ui;
  },
  events: function events() {
...
ControlMediaItemView = ControlBaseDataView.extend({
  ui: function ui() {
    var ui = ControlBaseDataView.prototype.ui.apply(this, arguments);
    ui.addImages = '.gugur-control-gallery-add';
    ui.clearGallery = '.gugur-control-gallery-clear';
    ui.galleryThumbnails = '.gugur-control-gallery-thumbnails';
    ui.status = '.gugur-control-gallery-status-title';
    return ui;
  },
  events: function events() {
    return _.extend(ControlBaseDataView.prototype.events.apply(this, arguments), {
  ...
  applySavedValue: function applySavedValue() {
    ...
    this.$el.toggleClass('gugur-gallery-has-images', hasImages).toggleClass('gugur-gallery-empty', !hasImages);
    var $galleryThumbnails = this.ui.galleryThumbnails;
    $galleryThumbnails.empty();
    this.ui.status.text(gugur.translate(hasImages ? 'gallery_images_selected' : 'gallery_no_images_selected', [imagesCount]));
    ...
    this.getControlValue().forEach(function (image) {
      var $thumbnail = jQuery('<div>', {
        class: 'gugur-control-gallery-thumbnail'
      });
  ...
  initFrame: function initFrame(action) {
    ...
    var options = {
      ...
      button: {
        text: gugur.translate('insert_media')
      }
    };
  ...
  initRemoveDialog: function initRemoveDialog() {
    ...
    this.getRemoveDialog = function () {
      if (!removeDialog) {
        removeDialog = gugurCommon.dialogsManager.createWidget('confirm', {
          message: gugur.translate('dialog_confirm_gallery_delete'),
          headerMessage: gugur.translate('delete_gallery'),
          strings: {
            confirm: gugur.translate('delete'),
            cancel: gugur.translate('cancel')
          },
...
function (_ControlMultipleBaseI) {
  ...
  (0, _createClass2.default)(ControlIconsView, [{
    ...
    value: function enqueueIconFonts(iconType) {
      var iconSetting = gugur.helpers.getIconLibrarySettings(iconType);
      ...
      if (iconSetting.enqueue) {
        iconSetting.enqueue.forEach(function (assetURL) {
          gugur.helpers.enqueueEditorStylesheet(assetURL);
          gugur.helpers.enqueuePreviewStylesheet(assetURL);
        });
      }

      if (iconSetting.url) {
        gugur.helpers.enqueueEditorStylesheet(iconSetting.url);
        gugur.helpers.enqueuePreviewStylesheet(iconSetting.url);
      }
    }
  }, {
    key: "ui",
    value: function ui() {
      ...
      ui.controlMedia = '.gugur-control-media';
      ui.svgUploader = '.gugur-control-svg-uploader';
      ui.iconPickers = '.gugur-control-icon-picker, .gugur-control-media__preview, .gugur-control-media-upload-button';
      ui.deleteButton = '.gugur-control-media__remove';
      ui.previewPlaceholder = '.gugur-control-media__preview';
      ui.previewContainer = '.gugur-control-preview-area';
      return ui;
    }
  }, {
    key: "events",
    ...
    value: function migrateFa4toFa5(fa4Value) {
      var fa5Value = gugur.helpers.mapFa4ToFa5(fa4Value);
    ...
    value: function isMigrationAllowed() {
      return !gugurConfig['icons_update_needed'];
    }
  }, {
    ...
    value: function onReady() {
      ...
      if (!this.isMigrationAllowed()) {
        this.ui.previewContainer[0].addEventListener('click', function (event) {
          ...
          var enableMigrationDialog = gugur.helpers.getSimpleDialog('gugur-enable-fa5-dialog', gugur.translate('enable_fa5'), gugur.translate('dialog_confirm_enable_fa5'), gugur.translate('update'), onConfirm);
    ...
    value: function onRender() {
      ...
      if (this.isMigrationAllowed()) {
        gugur.iconManager.loadIconLibraries();
      }
    }
  }, {
    key: "initFrame",
    value: function initFrame() {
      ...
      wp.media.view.settings.post.id = gugur.config.document.id;
      this.frame = wp.media({
        button: {
          text: gugur.translate('insert_media')
        },
        ...
        states: [new wp.media.controller.Library({
          title: gugur.translate('insert_media'),
    ...
    value: function getSvgNotEnabledDialog() {
      ...
      var onConfirm = function onConfirm() {
        gugurCommon.ajax.addRequest('enable_svg_uploads', {}, true);

        _this4.openFrame();
      };

      return gugur.helpers.getSimpleDialog('gugur-enable-svg-dialog', gugur.translate('enable_svg'), gugur.translate('dialog_confirm_enable_svg'), gugur.translate('enable'), onConfirm);
    }
  }, {
    key: "isSvgEnabled",
    ...
    value: function openFrame() {
      if (!this.isSvgEnabled() && !gugur.iconManager.cache.svgDialogShown) {
        var dialog = this.getSvgNotEnabledDialog();
        gugur.iconManager.cache.svgDialogShown = true;
        return dialog.show();
      }
      ...
      this.frame.uploader.uploader.param('uploadTypeCaller', 'gugur-editor-upload');
    ...
    value: function applySavedValue() {
      ...
      this.ui.controlMedia.toggleClass('gugur-media-empty', !iconValue);
      ...
      if ('svg' === iconType) {
        return gugur.helpers.fetchInlineSvg(iconValue.url, function (data) {
...
ControlImageDimensionsItemView = ControlMultipleBaseItemView.extend({
  ui: function ui() {
    return {
      ...
      btnApply: 'button.gugur-image-dimensions-apply-button'
    };
  },
...
ControlMediaItemView = ControlMultipleBaseItemView.extend({
  ui: function ui() {
    var ui = ControlMultipleBaseItemView.prototype.ui.apply(this, arguments);
    ui.controlMedia = '.gugur-control-media';
    ui.mediaImage = '.gugur-control-media__preview';
    ui.mediaVideo = '.gugur-control-media-video';
    ui.frameOpeners = '.gugur-control-preview-area';
    ui.removeButton = '.gugur-control-media__remove';
    return ui;
  },
  events: function events() {
  ...
  applySavedValue: function applySavedValue() {
    ...
    this.ui.controlMedia.toggleClass('gugur-media-empty', !url);
  },
...
ControlOrderItemView = ControlMultipleBaseItemView.extend({
  ui: function ui() {
    var ui = ControlMultipleBaseItemView.prototype.ui.apply(this, arguments);
    ui.reverseOrderLabel = '.gugur-control-order-label';
    return ui;
  },
  changeLabelTitle: function changeLabelTitle() {
    var reverseOrder = this.getControlValue('reverse_order');
    this.ui.reverseOrderLabel.attr('title', gugur.translate(reverseOrder ? 'asc' : 'desc'));
  },
  onRender: function onRender() {
...
ControlPopoverStarterView = ControlChooseView.extend({
  ui: function ui() {
    ...
    ui.popoverToggle = '.gugur-control-popover-toggle-toggle';
    return ui;
  },
  events: function events() {
  ...
  onPopoverToggleClick: function onPopoverToggleClick() {
    this.$el.next('.gugur-controls-popover').toggle();
  }
}, {
  onPasteStyle: function onPasteStyle(control, clipboardValue) {
...
ControlRepeaterItemView = ControlBaseDataView.extend({
  ui: {
    btnAddRow: '.gugur-repeater-add',
    fieldContainer: '.gugur-repeater-fields-wrapper'
  },
  events: function events() {
    return {
  ...
  childViewContainer: '.gugur-repeater-fields-wrapper',
  templateHelpers: function templateHelpers() {
    return {
  ...
  createItemModel: function createItemModel(attrs, options, controlView) {
    ...
    if (!attrs._id) {
      attrs._id = gugur.helpers.getUniqueID();
    }

    return new gugurModules.editor.elements.models.BaseSettings(attrs, options);
  },
  ...
  addRow: function addRow(data, options) {
    var id = gugur.helpers.getUniqueID();
  ...
  editRow: function editRow(rowView) {
    if (this.currentEditableChild) {
      ...
      currentEditable.find('.gugur-wp-editor').each(function () {
  ...
  toggleMinRowsClass: function toggleMinRowsClass() {
    ...
    this.$el.toggleClass('gugur-repeater-has-minimum-rows', 1 >= this.collection.length);
  },
  ...
  onRender: function onRender() {
    ...
    if (this.model.get('item_actions').sort) {
      this.ui.fieldContainer.sortable({
        axis: 'y',
        handle: '.gugur-repeater-row-tools'
      });
    }
...
ControlSectionItemView = ControlBaseView.extend({
  ui: function ui() {
    var ui = ControlBaseView.prototype.ui.apply(this, arguments);
    ui.heading = '.gugur-panel-heading';
    return ui;
  },
  triggers: {
...
ControlSliderItemView = ControlBaseUnitsItemView.extend({
  ...
  initSlider: function initSlider() {
    ...
    var isMultiple = this.isMultiple(),
        unitRange = gugurCommon.helpers.cloneObject(this.getCurrentRange()),
        step = unitRange.step;
  ...
  onReady: function onReady() {
    if (this.isMultiple()) {
      this.$el.addClass('gugur-control-type-slider--multiple gugur-control-type-slider--handles-' + this.model.get('handles'));
    }
  ...
  onSlideChange: function onSlideChange(values, index) {
    if (this.isMultiple()) {
      var sizes = gugurCommon.helpers.cloneObject(this.getSize()),
...
ControlStructureItemView = ControlBaseDataView.extend({
  ui: function ui() {
    var ui = ControlBaseDataView.prototype.ui.apply(this, arguments);
    ui.resetStructure = '.gugur-control-structure-reset';
    return ui;
  },
  events: function events() {
    return _.extend(ControlBaseDataView.prototype.events.apply(this, arguments), {
  ...
  getCurrentEditedSection: function getCurrentEditedSection() {
    var editor = gugur.getPanelView().getCurrentPageView();
    return editor.getOption('editedElementView');
  },
  getMorePresets: function getMorePresets() {
    var parsedStructure = gugur.presetsFactory.getParsedStructure(this.getControlValue());
    return gugur.presetsFactory.getPresets(parsedStructure.columnsCount);
  },
  onInputChange: function onInputChange() {
...
module.exports = BaseMultiple.extend({
  onReady: function onReady() {
    var self = this,
        positionBase = gugurCommon.config.isRTL ? 'right' : 'left',
        last,
    ...
    this.ui.input.autocomplete({
      ...
      open: function open(event) {
        jQuery(event.target).data('uiAutocomplete').menu.activeMenu.addClass('gugur-autocomplete-menu');
      },
      minLength: 2,
      position: {
    ...
    .autocomplete('instance')._renderItem = function (ul, item) {
      ...
      return jQuery('<li role="option" id="mce-wp-autocomplete-' + item.ID + '">').append('<span>' + title + '</span>&nbsp;<span class="gugur-autocomplete-item-info">' + item.info + '</span>').appendTo(ul);
    };
  },
  onBeforeDestroy: function onBeforeDestroy() {
...
ControlWPWidgetItemView = ControlBaseDataView.extend({
  ...
  onFormChanged: function onFormChanged() {
    var idBase = 'widget-' + this.model.get('id_base'),
        settings = this.ui.form.gugurSerializeObject()[idBase].REPLACE_TO_ID;
    this.setValue(settings);
  },
  onReady: function onReady() {
    var self = this;
    gugurCommon.ajax.addRequest('editor_get_wp_widget_form', {
      data: {
        // Fake Widget ID
        id: self.model.cid,
      ...
      success: function success(data) {
        ...
        gugur.hooks.doAction('panel/widgets/' + self.model.get('widget') + '/controls/wp_widget/loaded', self);
      }
    });
  }
});
...
ControlWysiwygItemView = ControlBaseDataView.extend({
  ...
  ui: function ui() {
    var ui = ControlBaseDataView.prototype.ui.apply(this, arguments);
    jQuery.extend(ui, {
      inputWrapper: '.gugur-control-input-wrapper'
    });
    return ui;
  },
  events: function events() {
    return _.extend(ControlBaseDataView.prototype.events.apply(this, arguments), {
      'keyup textarea.gugur-wp-editor': 'onBaseInputChange'
    });
  },
  ...
  initialize: function initialize() {
    ControlBaseDataView.prototype.initialize.apply(this, arguments);
    var self = this;
    self.editorID = 'gugurwpeditor' + self.cid; // Wait a cycle before initializing the editors.
    ...
    _.defer(function () {
      ...
      if (gugur.config.rich_editing_enabled) {
    ...
    if (!gugur.config.rich_editing_enabled) {
      self.$el.addClass('gugur-rich-editing-disabled');
      return;
    }
    ...
    tinyMCEPreInit.mceInit[self.editorID] = _.extend(_.clone(tinyMCEPreInit.mceInit.gugurwpeditor), editorConfig);

    if (!gugur.config.tinymceHasCustomConfig) {
  ...
  onReady: function onReady() {
    ...
    var $editor = jQuery(gugur.config.wp_editor.replace(/gugurwpeditor/g, this.editorID).replace('%%EDITORCONTENT%%', this.getControlValue()));
...
module.exports = gugurModules.editor.elements.models.BaseSettings.extend({
  defaults: {
    _column_size: 100
  }
});
...
WidgetView = BaseElementView.extend({
  ...
  getTemplate: function getTemplate() {
    ...
    if ('remote' !== this.getTemplateType()) {
      return Marionette.TemplateCache.get('#tmpl-gugur-' + editModel.get('widgetType') + '-content');
    }
  ...
  className: function className() {
    ...
    return baseClasses + ' gugur-widget ' + gugur.getElementData(this.getEditModel()).html_wrapper_class;
  },
  events: function events() {
  ...
  behaviors: function behaviors() {
    ...
    _.extend(behaviors, {
      InlineEditing: {
        ...
        inlineEditingClass: 'gugur-inline-editing'
      },
      Draggable: {
    ...
    return gugur.hooks.applyFilters('elements/widget/behaviors', behaviors, this);
  },
  getEditButtons: function getEditButtons() {
    var elementData = gugur.getElementData(this.model),
        editTools = {};
  ...
  getContextMenuGroups: function getContextMenuGroups() {
    ...
    groups.splice(transferGroupIndex + 1, 0, {
      ...
      actions: [{
        name: 'save',
        title: gugur.translate('save_as_global'),
        shortcut: jQuery('<i>', {
  ...
  render: function render() {
    if (this.model.isRemoteRequestActive()) {
      ...
      this.$el.addClass('gugur-element');
      return;
    }
  ...
  handleEmptyWidget: function handleEmptyWidget() {
    // TODO: REMOVE THIS !!
    // TEMP CODING !!
    this.$el.addClass('gugur-widget-empty').append('<i class="gugur-widget-empty-icon ' + this.getEditModel().getIcon() + '"></i>');
  },
  getTemplateType: function getTemplateType() {
    if (null === this._templateType) {
      var editModel = this.getEditModel(),
          $template = jQuery('#tmpl-gugur-' + editModel.get('widgetType') + '-content');
  ...
  attachElContent: function attachElContent(html) {
    ...
    _.defer(function () {
      gugurFrontend.elements.window.jQuery(_this.el).empty().append(_this.getHandlesOverlay(), _this.getHTMLContent(html));
  ...
  addInlineEditingAttributes: function addInlineEditingAttributes(key, toolbar) {
    this.addRenderAttribute(key, {
      class: 'gugur-inline-editing',
      'data-gugur-setting-key': key
    });

    if (toolbar) {
      this.addRenderAttribute(key, {
        'data-gugur-inline-editing-toolbar': toolbar
      });
    }
  },
  ...
  onModelBeforeRemoteRender: function onModelBeforeRemoteRender() {
    this.$el.addClass('gugur-loading');
  },
  onBeforeDestroy: function onBeforeDestroy() {
    // Remove old style from the DOM.
    gugur.$previewContents.find('#gugur-style-' + this.model.cid).remove();
  },
  onModelRemoteRender: function onModelRemoteRender() {
    ...
    this.$el.removeClass('gugur-loading');
    this.render();
  },
  onRender: function onRender() {
    var self = this;
    ...
    self.$el.attr('data-widget_type', editModel.get('widgetType') + '.' + skinType).removeClass('gugur-widget-empty').children('.gugur-widget-empty-icon').remove(); // TODO: Find a better way to detect if all the images have been loaded

    self.$el.imagesLoaded().always(function () {
      setTimeout(function () {
        if (1 > self.$el.children('.gugur-widget-container').outerHeight()) {
...
function (_Marionette$Behavior) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function initialize() {
      ...
      this.listenTo(gugur.channels.dataEditMode, 'switch', this.toggle);
      var view = this.view,
    ...
    value: function toggle() {
      var isEditMode = 'edit' === gugur.channels.dataEditMode.request('activeMode'),
          isAbsolute = this.view.getEditModel().getSetting('_position');
      this.deactivate();

      if (isEditMode && isAbsolute && gugur.userCan('design')) {
        this.activate();
      }
    }
  }, {
    ...
    value: function onDragStop(event, ui) {
      ...
      event.stopPropagation();
      var currentDeviceMode = gugurFrontend.getCurrentDeviceMode(),
          ...
          settingToChange = {},
          isRTL = gugurFrontend.config.is_rtl;
      ...
      xPos = gugur.helpers.elementSizeToUnit(this.$el, xPos, offsetXUnit);
      ...
      yPos = gugur.helpers.elementSizeToUnit(this.$el, yPos, offsetYUnit);
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function toggle() {
      var editModel = this.view.getEditModel(),
          isEditMode = 'edit' === gugur.channels.dataEditMode.request('activeMode'),
          isAbsolute = editModel.getSetting('_position'),
          isInline = 'initial' === editModel.getSetting('_element_width');
      this.deactivate();

      if (isEditMode && (isAbsolute || isInline) && gugur.userCan('design')) {
        this.activate();
      }
    }
  }, {
    ...
    value: function onResizeStop(event, ui) {
      ...
      var currentDeviceMode = gugurFrontend.getCurrentDeviceMode(),
          ...
          width = gugur.helpers.elementSizeToUnit(this.$el, ui.size.width, unit),
          settingToChange = {};
...
InlineEditingBehavior = Marionette.Behavior.extend({
  ...
  getEditingSettingKey: function getEditingSettingKey() {
    return this.$currentEditingArea.data().gugurSettingKey;
  },
  startEditing: function startEditing($element) {
    if (this.editing || 'edit' !== gugur.channels.dataEditMode.request('activeMode') || this.view.model.isRemoteRequestActive()) {
      return;
    }

    var gugurSettingKey = $element.data().gugurSettingKey,
        settingKey = gugurSettingKey,
        keyParts = gugurSettingKey.split('.'),
        isRepeaterKey = 3 === keyParts.length,
    ...
    var elementData = this.$currentEditingArea.data(),
        elementDataToolbar = elementData.gugurInlineEditingToolbar,
        mode = 'advanced' === elementDataToolbar ? 'advanced' : 'basic',
        editModel = this.view.getEditModel(),
        inlineEditingConfig = gugur.config.inlineEditing,
    ...
    this.editor = new gugurInlineEditor({
      ...
      placeholder: gugur.translate('type_here') + '...',
    ...
    gugurCommon.elements.$body.on('mousedown', this.onInlineEditingBlur);
  },
  stopEditing: function stopEditing() {
    this.editing = false;
    this.$currentEditingArea.off('blur', this.onInlineEditingBlur);
    gugurCommon.elements.$body.off('mousedown', this.onInlineEditingBlur);
    this.editor.destroy();
    ...
    if ('advanced' === this.$currentEditingArea.data().gugurInlineEditingToolbar) {
  ...
  onInlineEditingBlur: function onInlineEditingBlur(event) {
    ...
    setTimeout(function () {
      var selection = gugurFrontend.elements.window.getSelection(),
...
PanelElementsElementsView = Marionette.CollectionView.extend({
  ...
  id: 'gugur-panel-elements',
  initialize: function initialize() {
    this.listenTo(gugur.channels.panelElements, 'filter:change', this.onFilterChanged);
  },
  filter: function filter(childModel) {
    var filterValue = gugur.channels.panelElements.request('filter:value');
...
PanelMenuPageView = Marionette.CompositeView.extend({
  id: 'gugur-panel-page-menu',
  template: '#tmpl-gugur-panel-menu',
  childView: PanelMenuGroupView,
  childViewContainer: '#gugur-panel-page-menu-content',
  initialize: function initialize() {
    this.collection = PanelMenuPageView.getGroups();
  },
  getArrowClass: function getArrowClass() {
    return 'eicon-arrow-' + (gugurCommon.config.isRTL ? 'right' : 'left');
  },
  onRender: function onRender() {
    gugur.getPanelView().getHeaderView().ui.menuIcon.removeClass('eicon-menu-bar').addClass(this.getArrowClass());
  },
  onDestroy: function onDestroy() {
    gugur.getPanelView().getHeaderView().ui.menuIcon.removeClass(this.getArrowClass()).addClass('eicon-menu-bar');
  }
}, {
  groups: null,
  initGroups: function initGroups() {
    ...
    this.groups.add({
      name: 'go_to',
      title: gugur.translate('go_to'),
      items: []
    });
    this.addItem({
      name: 'view-page',
      icon: 'eicon-preview',
      title: gugur.translate('view_page'),
      type: 'link',
      link: gugur.config.document.urls.permalink
    }, 'go_to');
    this.addItem({
      name: 'exit-to-dashboard',
      icon: 'eicon-wordpress',
      title: gugur.translate('exit_to_dashboard'),
      type: 'link',
      link: gugur.config.document.urls.exit_to_dashboard
    }, 'go_to');

    if (gugur.config.user.is_administrator) {
      this.addAdminMenu();
    }
  },
  addAdminMenu: function addAdminMenu() {
    this.groups.add({
      name: 'style',
      title: gugur.translate('global_style'),
      items: []
    }, {
      at: 0
    });
    this.groups.add({
      name: 'settings',
      title: gugur.translate('settings'),
      items: []
    }, {
      at: 1
    });
    this.addItem({
      ...
      title: gugurCommon.translate('finder', 'finder'),
      callback: function callback() {
    ...
    this.addItem({
      ...
      title: gugur.translate('global_colors'),
      type: 'page',
      callback: function callback() {
    ...
    this.addItem({
      name: 'global-fonts',
      icon: 'eicon-font',
      title: gugur.translate('global_fonts'),
      type: 'page',
      callback: function callback() {
    ...
    this.addItem({
      ...
      title: gugur.translate('color_picker'),
      type: 'page',
      callback: function callback() {
    ...
    this.addItem({
      ...
      icon: 'eicon-cogs',
      title: gugur.translate('global_settings'),
      type: 'page',
      callback: function callback() {
        return $e.route('panel/general-settings/style');
      }
    }, 'settings', 'gugur-settings');
    this.addItem({
      name: 'gugur-settings',
      icon: 'eicon-editor-external-link',
      title: gugur.translate('gugur_settings'),
      type: 'link',
      link: gugur.config.settings_page_link,
      newTab: true
    }, 'settings');
    this.addItem({
      name: 'about-gugur',
      icon: 'eicon-info-circle',
      title: gugur.translate('about_gugur'),
      type: 'link',
      link: gugur.config.gugur_site,
      newTab: true
    }, 'settings');
  },
...
module.exports = Marionette.CompositeView.extend({
  template: '#tmpl-gugur-panel-menu-group',
  className: 'gugur-panel-menu-group',
  childView: PanelMenuItemView,
  childViewContainer: '.gugur-panel-menu-items',
  initialize: function initialize() {
...
module.exports = Marionette.ItemView.extend({
  template: '#tmpl-gugur-panel-menu-item',
  className: function className() {
    return 'gugur-panel-menu-item gugur-panel-menu-item-' + this.model.get('name');
  },
  triggers: {
    click: {
      event: 'click',
...
module.exports = gugurModules.Module.extend({
  ...
  loadCacheRequests: function loadCacheRequests() {
    ...
    this.cacheRequests = {};
    this.cacheCallbacks = [];
    gugurCommon.ajax.addRequest('render_tags', {
      data: {
        post_id: gugur.config.document.id,
  ...
  getConfig: function getConfig(key) {
    return this.getItems(gugur.config.dynamicTags, key);
  },
  parseTagsText: function parseTagsText(text, settings, parseCallback) {
    ...
    return text.replace(/\[gugur-tag[^\]]+]/g, function (tagText) {
  ...
  createTag: function createTag(tagID, tagName, tagSettings) {
    ...
    var TagClass = this.tags[tagName] || this.tags.Base,
        model = new gugurModules.editor.elements.models.BaseSettings(tagSettings, {
  ...
  tagDataToTagText: function tagDataToTagText(tagID, tagName, tagSettings) {
    ...
    return '[gugur-tag id="' + tagID + '" name="' + tagName + '" settings="' + tagSettings + '"]';
  },
...
module.exports = Marionette.ItemView.extend({
  ...
  getTemplate: function getTemplate() {
    ...
    return Marionette.TemplateCache.get('#tmpl-gugur-tag-' + this.getOption('name') + '-content');
  },
  initialize: function initialize() {
    try {
      this.getTemplate();
  ...
  getConfig: function getConfig(key) {
    var config = gugur.dynamicTags.getConfig('tags.' + this.getOption('name'));
  ...
  getContent: function getContent() {
    ...
    if (!this.hasTemplate) {
      data = gugur.dynamicTags.loadTagDataFromCache(this);

      if (undefined === data) {
        throw new Error(gugur.dynamicTags.CACHE_KEY_NOT_FOUND_ERROR);
      }
    }
  ...
  onRender: function onRender() {
    this.el.id = 'gugur-tag-' + this.getOption('id');
  }
});
...
module.exports = gugurModules.Module.extend({
  ...
  initSettings: function initSettings() {
    ...
    _.each(gugur.config.settings, function (config, name) {
...
module.exports = BaseSettings.extend({
  ...
  changeCallbacks: {
    gugur_page_title_selector: function gugur_page_title_selector(newValue) {
      var newSelector = newValue || 'h1.entry-title',
          titleSelectors = gugur.settings.page.model.controls.hide_title.selectors = {};
      titleSelectors[newSelector] = 'display: none';
      gugur.settings.page.updateStylesheet();
    }
  }
});
...
function (_gugurModules$com) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function defaultTabs() {
      return {
        style: {
          title: gugur.translate('style')
        },
        lightbox: {
          title: gugur.translate('lightbox')
        }
      };
    }
  }, {
    key: "renderTab",
    value: function renderTab(tab) {
      gugur.getPanelView().setPage('general_settings').activateTab(tab);
    }
  }]);
  return _default;
}(gugurModules.common.Component);
...
module.exports = BaseSettings.extend({
  ...
  changeCallbacks: {
    post_title: function post_title(newValue) {
      var $title = gugurFrontend.elements.$document.find(gugur.config.page_title_selector);
      $title.text(newValue);
    },
  ...
  getDataToSave: function getDataToSave(data) {
    data.id = gugur.config.document.id;
    return data;
  }
});
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function defaultTabs() {
      return {
        ...
        advanced: {
          title: gugur.translate('advanced')
        }
      };
    }
  }, {
    key: "renderTab",
    value: function renderTab(tab) {
      gugur.getPanelView().setPage('page_settings').activateTab(tab);
    }
  }, {
    key: "getTabsWrapperSelector",
    value: function getTabsWrapperSelector() {
      return '.gugur-panel-navigation';
    }
  }]);
  return _default;
}(gugurModules.common.Component);
...
module.exports = gugurModules.editor.views.ControlsStack.extend({
  id: function id() {
    return 'gugur-panel-' + this.getOption('name') + '-settings';
  },
  getTemplate: function getTemplate() {
    return '#tmpl-gugur-panel-' + this.getOption('name') + '-settings';
  },
  childViewContainer: function childViewContainer() {
    return '#gugur-panel-' + this.getOption('name') + '-settings-controls';
  },
  childViewOptions: function childViewOptions() {
    return {
      elementSettingsModel: this.model
...
module.exports = gugurModules.Module.extend({
  autoSaveTimer: null,
  autosaveInterval: gugur.config.autosave_interval * 1000,
  isSaving: false,
  ...
  saveDraft: function saveDraft() {
    var postStatus = gugur.settings.page.model.get('post_status');

    if (!gugur.saver.isEditorChanged() && 'draft' !== postStatus) {
      return;
    }
  ...
  doAutoSave: function doAutoSave() {
    var editorMode = gugur.channels.dataEditMode.request('activeMode'); // Avoid auto save for Revisions Preview changes.
  ...
  discard: function discard() {
    var self = this;
    gugurCommon.ajax.addRequest('discard_changes', {
      success: function success() {
        self.setFlagEditorChange(false);
        location.href = gugur.config.document.urls.exit_to_dashboard;
      }
    });
  },
  update: function update(options) {
    options = _.extend({
      status: gugur.settings.page.model.get('post_status')
    }, options);
    this.saveEditor(options);
  },
  ...
  setFlagEditorChange: function setFlagEditorChange(status) {
    ...
    gugur.channels.editor.reply('status', status).trigger('status:change', status);
  },
  isEditorChanged: function isEditorChanged() {
    return true === gugur.channels.editor.request('status');
  },
  setWorkSaver: function setWorkSaver() {
    var self = this;
    gugurCommon.elements.$window.on('beforeunload', function () {
      if (self.isEditorChanged()) {
        return gugur.translate('before_unload_alert');
      }
    });
  },
  ...
  saveEditor: function saveEditor(options) {
    ...
    var self = this,
        elements = gugur.elements.toJSON({
      remove: ['default', 'editSettings', 'defaultEditSettings']
    }),
        settings = gugur.settings.page.model.toJSON({
      remove: ['default']
    }),
        oldStatus = gugur.settings.page.model.get('post_status'),
    ...
    gugurCommon.ajax.addRequest('save_builder', {
      data: {
      ...
      success: function success(data) {
        ...
        if ('autosave' !== options.status) {
          if (statusChanged) {
            gugur.settings.page.model.set('post_status', options.status);
        ...
        if (data.config) {
          jQuery.extend(true, gugur.config, data.config);
        }

        gugur.config.data = elements;
        gugur.channels.editor.trigger('saved', data);
      ...
      error: function error(data) {
        ...
        if (_.isString(data)) {
          ...
          message = gugur.createAjaxErrorMessage(data);

          if (0 === data.readyState) {
            message += ' ' + gugur.translate('saving_disabled');
          }
        } else if (data[0] && data[0].code) {
          message = gugur.translate('server_error') + ' ' + data[0].code;
        }
...
module.exports = gugurModules.Module.extend({
  initToast: function initToast() {
    var toast = gugurCommon.dialogsManager.createWidget('buttons', {
      id: 'gugur-toast',
      position: {
        my: 'center bottom',
        at: 'center bottom-10',
        of: '#gugur-panel-content-wrapper',
        autoRefresh: true
      },
      hide: {
        onClick: true,
...
module.exports = BaseRegion.extend({
  ...
  setSize: function setSize() {
    var width = this.storage.size.width,
        side = gugurCommon.config.isRTL ? 'right' : 'left';
    this.$el.css('width', width);
    gugur.$previewWrapper.css(side, width);
  },
...
PanelLayoutView = Marionette.LayoutView.extend({
  template: '#tmpl-gugur-panel',
  id: 'gugur-panel-inner',
  regions: {
    content: '#gugur-panel-content-wrapper',
    header: '#gugur-panel-header-wrapper',
    footer: '#gugur-panel-footer',
    modeSwitcher: '#gugur-mode-switcher'
  },
  pages: {},
  childEvents: {
  ...
  buildPages: function buildPages() {
    var pages = {
      elements: {
        ...
        title: '<img src="' + gugurCommon.config.urls.assets + 'images/logo-panel.svg">'
      },
      editor: {
        view: __webpack_require__(440)
      },
      menu: {
        view: gugur.modules.layouts.panel.pages.menu.Menu,
        title: '<img src="' + gugurCommon.config.urls.assets + 'images/logo-panel.svg">'
      },
      colorScheme: {
    ...
    var schemesTypes = (0, _keys.default)(gugur.schemes.getSchemes()),
        disabledSchemes = _.difference(schemesTypes, gugur.schemes.getEnabledSchemesTypes());

    _.each(disabledSchemes, function (schemeType) {
      var scheme = gugur.schemes.getScheme(schemeType);
  ...
  setPage: function setPage(page, title, viewOptions) {
    ...
    if ('elements' === page && !gugur.userCan('design')) {
    ...
    if (!pageData) {
      throw new ReferenceError('gugur panel doesn\'t have page named \'' + page + '\'');
    }
...
function (_gugurModules$com) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function defaultCommands() {
      return {
        open: function open() {
          return gugur.getPanelView().modeSwitcher.currentView.setMode('edit');
        },
        close: function close() {
          return gugur.getPanelView().modeSwitcher.currentView.setMode('preview');
        },
        toggle: function toggle() {
          return gugur.getPanelView().modeSwitcher.currentView.toggleMode();
        },
        save: function save() {
          return gugur.saver.saveDraft();
        },
        publish: function publish() {
          return gugur.saver.publish();
        },
        exit: function exit() {
        ...
        'change-device-mode': function changeDeviceMode(args) {
          ...
          if (!args.device) {
            var currentDeviceMode = gugur.channels.deviceMode.request('currentMode');
          ...
          gugur.changeDeviceMode(args.device);
        }
      };
    }
  }, {
...
function (_gugurModules$com) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function defaultTabs() {
      return {
        categories: {
          title: gugur.translate('elements')
        },
        global: {
          title: gugur.translate('global')
        }
      };
    }
  }, {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function defaultCommands() {
      ...
      return {
        open: function open(args) {
          ...
          gugur.hooks.doAction(action, _this.manager, args.model, args.view); // Example: panel/open_editor/widget/heading

          gugur.hooks.doAction(action + '/' + args.model.get('widgetType'), _this.manager, args.model, args.view);
        }
      };
    }
  }, {
    ...
    value: function setDefaultTab(args) {
      ...
      if (this.activeTabs[args.model.id]) {
        ...
        defaultTab = jQuery(this.getTabsWrapperSelector()).find('.gugur-component-tab').eq(0).data('tab');
      }
    ...
    value: function openEditor(model, view) {
      var title = gugur.translate('edit_element', [gugur.getElementData(model).title]),
          editor = gugur.getPanelView().setPage('editor', title, {
        model: model,
        controls: gugur.getElementControls(model),
...
EditModeItemView = Marionette.ItemView.extend({
  template: '#tmpl-gugur-mode-switcher-content',
  id: 'gugur-mode-switcher-inner',
  ui: {
    previewButton: '#gugur-mode-switcher-preview-input',
    previewLabel: '#gugur-mode-switcher-preview',
    previewLabelA11y: '#gugur-mode-switcher-preview .gugur-screen-only'
  },
  events: {
    'change @ui.previewButton': 'onPreviewButtonChange'
  },
  initialize: function initialize() {
    this.listenTo(gugur.channels.dataEditMode, 'switch', this.onEditModeChanged);
  },
  getCurrentMode: function getCurrentMode() {
  ...
  onPreviewButtonChange: function onPreviewButtonChange() {
    gugur.changeEditMode(this.getCurrentMode());
  },
  onEditModeChanged: function onEditModeChanged() {
    var activeMode = gugur.channels.dataEditMode.request('activeMode'),
        title = gugur.translate('preview' === activeMode ? 'back_to_editor' : 'preview');
...
var PanelElementsCategoriesCollection = __webpack_require__(436),
    ...
    PanelElementsElementsView = gugur.modules.layouts.panel.pages.elements.views.Elements,
    PanelElementsSearchView = __webpack_require__(439),
    PanelElementsGlobalView = __webpack_require__(257),
    PanelElementsLayoutView;

PanelElementsLayoutView = Marionette.LayoutView.extend({
  template: '#tmpl-gugur-panel-elements',
  id: 'gugur-panel-page-elements',
  options: {
    autoFocusSearch: true
  },
  regions: {
    elements: '#gugur-panel-elements-wrapper',
    search: '#gugur-panel-elements-search-area'
  },
  regionViews: {},
  elementsCollection: null,
  categoriesCollection: null,
  initialize: function initialize() {
    this.listenTo(gugur.channels.panelElements, 'element:selected', this.destroy);
  ...
  initRegionViews: function initRegionViews() {
    ...
    this.regionViews = gugur.hooks.applyFilters('panel/elements/regionViews', regionViews);
  },
  initElementsCollection: function initElementsCollection() {
    var elementsCollection = new PanelElementsElementsCollection(),
        sectionConfig = gugur.config.elements.section;
    elementsCollection.add({
      title: gugur.translate('inner_section'),
      elType: 'section',
      categories: ['basic'],
    ...
    _.each(gugur.config.widgets, function (widget) {
      if (gugur.config.document.panel.widgets_settings[widget.widget_type]) {
        widget = _.extend(widget, gugur.config.document.panel.widgets_settings[widget.widget_type]);
      }
  ...
  initCategoriesCollection: function initCategoriesCollection() {
    ...
    _.each(gugur.config.document.panel.elements_categories, function (categoryConfig, categoryName) {
  ...
  changeFilter: function changeFilter(filterValue) {
    gugur.channels.panelElements.reply('filter:value', filterValue).trigger('filter:change');
  },
  ...
  focusSearch: function focusSearch() {
    if (!gugur.userCan('design') || !this.search
  ...
  onDestroy: function onDestroy() {
    gugur.channels.panelElements.reply('filter:value', null);
  },
...
PanelElementsCategoriesView = Marionette.CompositeView.extend({
  template: '#tmpl-gugur-panel-categories',
  childView: PanelElementsCategoryView,
  childViewContainer: '#gugur-panel-categories',
  id: 'gugur-panel-elements-categories',
  initialize: function initialize() {
    this.listenTo(gugur.channels.panelElements, 'filter:change', this.onPanelElementsFilterChange);
  },
  onPanelElementsFilterChange: function onPanelElementsFilterChange() {
    if (gugur.channels.panelElements.request('filter:value')) {
      gugur.getPanelView().getCurrentPageView().showView('elements');
    }
  }
});
...
PanelElementsCategoryView = Marionette.CompositeView.extend({
  template: '#tmpl-gugur-panel-elements-category',
  className: 'gugur-panel-category',
  ui: {
    title: '.gugur-panel-category-title',
    items: '.gugur-panel-category-items'
  },
  events: {
    'click @ui.title': 'onTitleClick'
  },
  id: function id() {
    return 'gugur-panel-category-' + this.model.get('name');
  },
  childView: __webpack_require__(258),
  childViewContainer: '.gugur-panel-category-items',
  initialize: function initialize() {
    this.collection = new PanelElementsElementsCollection(this.model.get('items'));
  },
  onRender: function onRender() {
    var isActive = gugur.channels.panelElements.request('category:' + this.model.get('name') + ':active');
    ...
    if (isActive) {
      this.$el.addClass('gugur-active');
      this.ui.items.show();
    }
  },
  onTitleClick: function onTitleClick() {
    var $items = this.ui.items,
        activeClass = 'gugur-active',
        isActive = this.$el.hasClass(activeClass),
        slideFn = isActive ? 'slideUp' : 'slideDown';
    gugur.channels.panelElements.reply('category:' + this.model.get('name') + ':active', !isActive);
...
PanelElementsSearchView = Marionette.ItemView.extend({
  template: '#tmpl-gugur-panel-element-search',
  id: 'gugur-panel-elements-search-wrapper',
  ui: {
    input: 'input'
  },
  events: {
...
var ControlsStack = gugurModules.editor.views.ControlsStack,
    EditorView;
EditorView = ControlsStack.extend({
  template: Marionette.TemplateCache.get('#tmpl-editor-content'),
  id: 'gugur-panel-page-editor',
  childViewContainer: '#gugur-controls',
  childViewOptions: function childViewOptions() {
    return {
  ...
  getNamespaceArray: function getNamespaceArray() {
    var eventNamespace = gugurModules.editor.views.ControlsStack.prototype.getNamespaceArray();
  ...
  openActiveSection: function openActiveSection() {
    ...
    gugur.channels.editor.trigger('section:activated', this.activeSection, this);
  },
  isVisibleSectionControl: function isVisibleSectionControl(sectionControlModel) {
    return ControlsStack.prototype.isVisibleSectionControl.apply(this, arguments) && gugur.helpers.isActiveControl(sectionControlModel, this.model.get('settings').attributes);
  },
  scrollToEditedElement: function scrollToEditedElement() {
    gugur.helpers.scrollToView(this.getOption('editedElementView').$el);
  },
  onDestroy: function onDestroy() {
    var editedElementView = this.getOption('editedElementView');

    if (editedElementView) {
      editedElementView.$el.removeClass('gugur-element-editable');
    }
  ...
  onRender: function onRender() {
    ...
    if (editedElementView) {
      editedElementView.$el.addClass('gugur-element-editable');
    }
  },
  ...
  onChildviewSettingsChange: function onChildviewSettingsChange(childView) {
    ...
    gugur.channels.editor.trigger('change', childView, editedElementView).trigger('change:' + editedElementType, childView, editedElementView).trigger('change:' + editedElementType + ':' + childView.model.get('name'), childView, editedElementView);
  }
});
...
PanelSchemeColorView = PanelSchemeItemView.extend({
  ...
  ui: {
    input: '.gugur-panel-scheme-color-value'
  },
...
PanelSchemeTypographyView = PanelSchemeItemView.extend({
  ...
  className: function className() {
    ...
    return classes + ' gugur-panel-box';
  },
  ui: {
    heading: '.gugur-panel-heading',
    allFields: '.gugur-panel-scheme-typography-item-field',
    inputFields: 'input.gugur-panel-scheme-typography-item-field',
    selectFields: 'select.gugur-panel-scheme-typography-item-field',
    selectFamilyFields: 'select.gugur-panel-scheme-typography-item-field[name="font_family"]'
  },
  events: {
  ...
  toggleVisibility: function toggleVisibility() {
    this.$el.toggleClass('gugur-open');
  },
  changeUIValue: function changeUIValue(newValue) {
  ...
  onFieldChange: function onFieldChange(event) {
    var $select = this.$(event.currentTarget),
        currentValue = gugur.schemes.getSchemeValue('typography', this.model.get('key')).value,
        fieldKey = $select.attr('name');
    currentValue[fieldKey] = $select.val();

    if ('font_family' === fieldKey && !_.isEmpty(currentValue[fieldKey])) {
      gugur.helpers.enqueueFont(currentValue[fieldKey]);
    }
...
PanelSchemeColorPickerView = PanelSchemeColorsView.extend({
  ...
  orderView: function orderView(model) {
    return gugur.helpers.getColorPickerPaletteIndex(model.get('key'));
  }
});
...
PanelSchemeDisabledView = Marionette.ItemView.extend({
  template: '#tmpl-gugur-panel-schemes-disabled',
  id: 'gugur-panel-schemes-disabled',
  className: 'gugur-nerd-box',
  disabledTitle: '',
  templateHelpers: function templateHelpers() {
    return {
...
module.exports = Marionette.ItemView.extend({
  template: '#tmpl-gugur-panel-footer-content',
  tagName: 'nav',
  id: 'gugur-panel-footer-tools',
  possibleRotateModes: ['portrait', 'landscape'],
  ui: {
    menuButtons: '.gugur-panel-footer-tool',
    settings: '#gugur-panel-footer-settings',
    deviceModeIcon: '#gugur-panel-footer-responsive > i',
    deviceModeButtons: '#gugur-panel-footer-responsive .gugur-panel-footer-sub-menu-item',
    saveTemplate: '#gugur-panel-footer-sub-menu-item-save-template',
    history: '#gugur-panel-footer-history',
    navigator: '#gugur-panel-footer-navigator'
  },
  events: {
  ...
  behaviors: function behaviors() {
    var behaviors = {
      saver: {
        behaviorClass: gugur.modules.components.saver.behaviors.FooterSaver
      }
    };
    return gugur.hooks.applyFilters('panel/footer/behaviors', behaviors, this);
  },
  initialize: function initialize() {
    this.listenTo(gugur.channels.deviceMode, 'change', this.onDeviceModeChange);
  },
  ...
  addSubMenuItem: function addSubMenuItem(subMenuName, itemData) {
    var $newItem = jQuery('<div>', {
      id: 'gugur-panel-footer-sub-menu-item-' + itemData.name,
      class: 'gugur-panel-footer-sub-menu-item'
    }),
        $itemIcon = jQuery('<i>', {
      class: 'gugur-icon ' + itemData.icon,
      'aria-hidden': true
    }),
        $itemTitle = jQuery('<div>', {
      class: 'gugur-title'
    ...
    var $menuTool = this.ui.menuButtons.filter('#gugur-panel-footer-' + subMenuName);

    if (itemData.before) {
      var $beforeItem = $menuTool.find('#gugur-panel-footer-sub-menu-item-' + itemData.before);
    ...
    var $subMenu = $menuTool.find('.gugur-panel-footer-sub-menu');
  ...
  onMenuButtonsClick: function onMenuButtonsClick(event) {
    ...
    if (!$tool.hasClass('gugur-toggle-state') || jQuery(event.target).closest('.gugur-panel-footer-sub-menu-item').length) {
      return;
    }

    var isOpen = $tool.hasClass('gugur-open');
    this.ui.menuButtons.not('.gugur-leave-open').removeClass('gugur-open');

    if (!isOpen) {
      $tool.addClass('gugur-open');
    }
  },
  ...
  onDeviceModeChange: function onDeviceModeChange() {
    var previousDeviceMode = gugur.channels.deviceMode.request('previousMode'),
        currentDeviceMode = gugur.channels.deviceMode.request('currentMode');
  ...
  onResponsiveButtonsClick: function onResponsiveButtonsClick(event) {
    ...
    gugur.changeDeviceMode(newDeviceMode);
  },
...
PanelHeaderItemView = Marionette.ItemView.extend({
  template: '#tmpl-gugur-panel-header',
  id: 'gugur-panel-header',
  ui: {
    menuButton: '#gugur-panel-header-menu-button',
    menuIcon: '#gugur-panel-header-menu-button i',
    title: '#gugur-panel-header-title',
    addButton: '#gugur-panel-header-add-button'
  },
  events: {
...
Preview = BaseSectionsContainerView.extend({
  template: Marionette.TemplateCache.get('#tmpl-gugur-preview'),
  className: 'gugur-inner',
  childViewContainer: '.gugur-section-wrap',
  behaviors: function behaviors() {
    ...
    if (!gugur.config.user.introduction.rightClick && !gugur.config.user.introduction[2]) {
  ...
  paste: function paste(atIndex) {
    ...
    gugur.channels.data.trigger('element:before:add', transferData.elements[0]);
    ...
    gugur.channels.data.trigger('element:after:add', transferData.elements[0]);
  },
  isPasteEnabled: function isPasteEnabled() {
    return gugurCommon.storage.get('transfer');
  },
...
function (_Marionette$Behavior) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function ui() {
      return {
        editButton: '.gugur-editor-element-edit'
      };
    }
  }, {
    key: "events",
      ...
      this.getIntroduction = function () {
        if (!introduction) {
          introduction = new gugurModules.editor.utils.Introduction({
            ...
            dialogOptions: {
              className: 'gugur-right-click-introduction',
              headerMessage: gugur.translate('meet_right_click_header'),
              message: gugur.translate('meet_right_click_message'),
              iframe: gugur.$preview,
              position: {
                my: 'center top+5',
                at: 'center bottom',
            ...
            onDialogInitCallback: function onDialogInitCallback(dialog) {
              dialog.addButton({
                name: 'learn-more',
                text: gugur.translate('learn_more'),
                tag: 'div',
                callback: function callback() {
                  open(gugur.config.help_right_click_url, '_blank');
                }
              });
              dialog.addButton({
                name: 'ok',
                text: gugur.translate('got_it'),
                callback: function callback() {
                  return introduction.setViewed();
                }
              });
              dialog.getElements('ok').addClass('gugur-button gugur-button-success');
            }
          });
        }
...
BaseSectionsContainerView = BaseContainer.extend({
  ...
  behaviors: function behaviors() {
    ...
    return gugur.hooks.applyFilters('elements/base-section-container/behaviors', behaviors, this);
  },
  getSortableOptions: function getSortableOptions() {
    return {
      handle: '> .gugur-element-overlay .gugur-editor-element-edit',
  ...
  initialize: function initialize() {
    BaseContainer.prototype.initialize.apply(this, arguments);
    this.listenTo(this.collection, 'add remove reset', this.onCollectionChanged).listenTo(gugur.channels.panelElements, 'element:drag:start', this.onPanelElementDragStart).listenTo(gugur.channels.panelElements, 'element:drag:end', this.onPanelElementDragEnd);
  },
  onCollectionChanged: function onCollectionChanged() {
    gugur.saver.setFlagEditorChange(true);
  },
  onPanelElementDragStart: function onPanelElementDragStart() {
    ...
    this.$el.find('.gugur-background-video-embed').hide();
    gugur.helpers.disableElementEvents(this.$el.find('iframe'));
  },
  onPanelElementDragEnd: function onPanelElementDragEnd() {
    this.$el.find('.gugur-background-video-embed').show();
    gugur.helpers.enableElementEvents(this.$el.find('iframe'));
  }
});
