...
/* global jQuery, gugurGutenbergSettings */
(function ($) {
  'use strict';

  var gugurGutenbergApp = {
    cacheElements: function cacheElements() {
      this.isgugurMode = gugurGutenbergSettings.isgugurMode;
      this.cache = {};
      this.cache.$gutenberg = $('#editor');
      this.cache.$switchMode = $($('#gugur-gutenberg-button-switch-mode').html());
      this.cache.$gutenberg.find('.edit-post-header-toolbar').append(this.cache.$switchMode);
      this.cache.$switchModeButton = this.cache.$switchMode.find('#gugur-switch-mode-button');
      this.toggleStatus();
      this.buildPanel();
      var self = this;
    ...
    buildPanel: function buildPanel() {
      ...
      if (!$('#gugur-editor').length) {
        self.cache.$editorPanel = $($('#gugur-gutenberg-panel').html());
        ...
        self.cache.$editorPanelButton = self.cache.$editorPanel.find('#gugur-go-to-edit-page-link');
      ...
      self.cache.$switchModeButton.on('click', function () {
        if (self.isgugurMode) {
          gugurCommon.dialogsManager.createWidget('confirm', {
            message: gugurAdmin.translate('back_to_wordpress_editor_message'),
            headerMessage: gugurAdmin.translate('back_to_wordpress_editor_header'),
            strings: {
              confirm: gugurAdmin.translate('yes'),
              cancel: gugurAdmin.translate('cancel')
            },
            defaultOption: 'confirm',
            onConfirm: function onConfirm() {
              var wpEditor = wp.data.dispatch('core/editor');
              wpEditor.editPost({
                gutenberg_gugur_mode: false
              });
              wpEditor.savePost();
              self.isgugurMode = !self.isgugurMode;
              self.toggleStatus();
            }
          }).show();
        } else {
    ...
    redirectWhenSave: function redirectWhenSave() {
      ...
      setTimeout(function () {
        if (wp.data.select('core/editor').isSavingPost()) {
          ...
          location.href = gugurGutenbergSettings.editLink;
        }
      }, 300);
    },
    animateLoader: function animateLoader() {
      this.cache.$editorPanelButton.addClass('gugur-animate');
    },
    toggleStatus: function toggleStatus() {
      jQuery('body').toggleClass('gugur-editor-active', this.isgugurMode).toggleClass('gugur-editor-inactive', !this.isgugurMode);
    },
    init: function init() {
      var self = this;
      setTimeout(function () {
