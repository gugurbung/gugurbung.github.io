...
use gugurPro\Modules\ThemeBuilder\Documents\Archive;
use gugurPro\Plugin;
...
class Product_Archive extends Archive {
	...
	public static function get_title() {
		return __( 'Product Archive', 'gugur-pro' );
	}

	public function enqueue_scripts() {
		// In preview mode it's not a real Woocommerce page - enqueue manually.
		if ( Plugin::gugur()->preview->is_preview_mode( $this->get_main_id() ) ) {
	...
	public function filter_body_classes( $body_classes ) {
		...
		if ( get_the_ID() === $this->get_main_id() || Plugin::gugur()->preview->is_preview_mode( $this->get_main_id() ) ) {
	...
	public static function get_preview_as_options() {
		...
		$post_type_archives[ 'post_type_archive/' . $post_type ] = $post_type_object->label . ' ' . __( 'Archive', 'gugur-pro' );
		...
		foreach ( $post_type_taxonomies as $slug => $object ) {
			$taxonomies[ 'taxonomy/' . $slug ] = $object->label . ' ' . __( 'Archive', 'gugur-pro' );
		}

		$options = [
			'search' => __( 'Search Results', 'gugur-pro' ),
		];
	...
	protected static function get_editor_panel_categories() {
		$categories = [
			'woocommerce-elements-archive' => [
				'title' => __( 'Product Archive', 'gugur-pro' ),
			],
