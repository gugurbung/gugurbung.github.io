...
class Ajax_Handler {
	...
	public static function is_form_submitted() {
		return Utils::is_ajax() && isset( $_POST['action'] ) && 'gugur_pro_forms_send_form' === $_POST['action'];
	}

	public static function get_default_messages() {
		return [
			self::SUCCESS => __( 'The form was sent successfully.', 'gugur-pro' ),
			self::ERROR => __( 'An error occured.', 'gugur-pro' ),
			self::FIELD_REQUIRED => __( 'This field is required.', 'gugur-pro' ),
			self::INVALID_FORM => __( 'There\'s something wrong. The form is invalid.', 'gugur-pro' ),
			self::SERVER_ERROR => __( 'Server error. Form not sent.', 'gugur-pro' ),
			self::SUBSCRIBER_ALREADY_EXISTS => __( 'Subscriber already exists.', 'gugur-pro' ),
		];
	}
	...
	public static function get_default_message( $id, $settings ) {
		...
		return isset( $default_messages[ $id ] ) ? $default_messages[ $id ] : __( 'Unknown', 'gugur-pro' );
	}
	...
	public function ajax_send_form() {
		...
		$meta = $gugur->db->get_plain_editor( $post_id );
		...
		if ( ! empty( $form['templateID'] ) ) {
			$global_meta = $gugur->db->get_plain_editor( $form['templateID'] );
			$form = $global_meta[0];
		}

		// restore default values
		$widget = $gugur->elements_manager->create_element_instance( $form );
		 ...
		 */
		do_action( 'gugur_pro/forms/new_record', $record, $this );
	...
	public function send() {
		...
		if ( current_user_can( 'edit_post', $_POST['post_id'] ) && ! empty( $this->messages['admin_error'] ) ) {
			$this->add_admin_error_message( __( 'This Message is not visible for site visitors.', 'gugur-pro' ) );
			$error_msg .= '<div class="gugur-forms-admin-errors">' . implode( '<br>', $this->messages['admin_error'] ) . '</div>';
		}
	...
	public function __construct() {
		add_action( 'wp_ajax_gugur_pro_forms_send_form', [ $this, 'ajax_send_form' ] );
		add_action( 'wp_ajax_nopriv_gugur_pro_forms_send_form', [ $this, 'ajax_send_form' ] );
	}
}
