...
class Locations_Manager {
	...
	public function __construct() {
		...
		add_filter( 'gugur/admin/create_new_post/meta', [ $this, 'filter_add_location_meta_on_create_new_post' ] );
	...
	public function register_locations() {
		// Run Once.
		if ( ! did_action( 'gugur/theme/register_locations' ) ) {
			/**
			 ...
			 *
			 * This is where gugur theme locations are registered by
			 * external themes.
			 *
			 * @since 2.0.0
			 *
			 ...
			 */
			do_action( 'gugur/theme/register_locations', $this );
		}
	}
	...
	public function enqueue_styles() {
		...
		if ( ! empty( $css_files ) ) {
			Plugin::gugur()->frontend->enqueue_styles();
	...
	public function template_include( $template ) {
		...
		if ( is_singular() ) {
			$document = Plugin::gugur()->documents->get_doc_for_frontend( get_the_ID() );
		...
		/**
		 * @var \gugur\Modules\PageTemplates\Module $page_templates_module
		 */
		$page_templates_module = Plugin::gugur()->modules_manager->get_modules( 'page-templates' );
		...
		$need_override_location = apply_filters( 'gugur/theme/need_override_location', $need_override_location, $location, $this );
	...
	public function do_location( $location ) {
		 ...
		 *
		 * Fires before gugur location was printed.
		 *
		 ...
		 * @param Locations_Manager $this An instance of locations manager.
		 */
		do_action( "gugur/theme/before_do_{$location}", $this );
		 ...
		 *
		 * Fires after gugur location was printed.
		 *
		 * The dynamic portion of the hook name, `$location`, refers to the location name.
		 *
		 * @since 2.0.0
		 ...
		 * @param Locations_Manager $this An instance of locations manager.
		 */
		do_action( "gugur/theme/after_do_{$location}", $this );
		...
		if ( $post_id ) {
			...
			if ( $document ) {
				...
				if ( $location_settings && ! $location_settings['edit_in_content'] ) {
					$content = '<div class="gugur-theme-builder-content-area">' . __( 'Content Area', 'gugur-pro' ) . '</div>';
				}
			}
		}
	...
	public function register_core_location( $location, $args = [] ) {
		if ( ! isset( $this->core_locations[ $location ] ) ) {
			...
			wp_die( esc_html( sprintf( __( 'Location \'%s\' is not a core location.', 'gugur-pro' ), $location ) ) );
		}
	...
	private function set_core_locations() {
		$this->core_locations = [
			...
			'archive' => [
				...
				'public' => false,
				'overwrite' => true,
				'label' => __( 'Archive', 'gugur-pro' ),
				'edit_in_content' => true,
			],
			'single' => [
				'is_core' => true,
				'public' => false,
				'label' => __( 'Single', 'gugur-pro' ),
				'edit_in_content' => true,
			],
		];
	}
	...
	public function inspector_log( $args ) {
		$inspector_enabled = method_exists( Plugin::gugur()->inspector, 'is_enabled' ) && Plugin::gugur()->inspector->is_enabled();
		...
		if ( isset( $args['template'] ) ) {
			$title[] = Plugin::gugur()->inspector->parse_template_path( $args['template'] );
		}

		$title = implode( ' > ', $title );

		Plugin::gugur()->inspector->add_log( 'Theme', $title, $url );
	}
}
