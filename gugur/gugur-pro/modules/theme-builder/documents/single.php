...
class Single extends Theme_Page_Document {
	 ...
	 */
	const REMOTE_CATEGORY_META_KEY = '_gugur_template_sub_type';
	...
	public function print_content() {
		...
		if ( $requested_post_id !== $this->post->ID ) {
			...
			if ( $requested_document && ! $requested_document instanceof Section && $requested_document->get_location() !== $this->get_location() ) {
				echo '<div class="gugur-theme-builder-content-area">' . __( 'Content Area', 'gugur-pro' ) . '</div>';
	...
	public static function get_preview_as_options() {
		...
		return [
			...
			'page/404' => __( '404', 'gugur-pro' ),
		];
	}

	public function get_depended_widget() {
		return Plugin::gugur()->widgets_manager->get_widget_types( 'theme-post-content' );
	}

	public function get_elements_data( $status = DB::STATUS_PUBLISH ) {
		$data = parent::get_elements_data();

		if ( Plugin::gugur()->preview->is_preview_mode() && self::get_property( 'location' ) === Module::instance()->get_locations_manager()->get_current_location() ) {
			$has_the_content = false;

			$depended_widget = $this->get_depended_widget();

			Plugin::gugur()->db->iterate_data( $data, function( $element ) use ( &$has_the_content, $depended_widget ) {
	...
	public function preview_error_handler() {
		...
		wp_localize_script( 'gugur-frontend', 'gugurPreviewErrorArgs', [
			/* translators: %s: is the widget name. */
			'headerMessage' => sprintf( __( 'The %s Widget was not found in your template.', 'gugur-pro' ), $depended_widget_title ),
			/* translators: %1$s: is the widget name. %2$s: is the template name.  */
			'message' => sprintf( __( 'You must include the %1$s Widget in your template (<strong>%2$s</strong>), in order for gugur to work on this page.', 'gugur-pro' ), $depended_widget_title, static::get_title() ),
			'strings' => [
				'confirm' => __( 'Edit Template', 'gugur-pro' ),
			],
