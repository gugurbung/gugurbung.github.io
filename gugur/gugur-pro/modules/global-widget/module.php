<?php
namespace gugurPro\Modules\GlobalWidget;
...
class Module extends Module_Base {
	...
	const WIDGET_TYPE_META_KEY = '_gugur_template_widget_type';

	const INCLUDED_POSTS_LIST_META_KEY = '_gugur_global_widget_included_posts';
	...
	public function add_templates_localize_data( $settings ) {
		...
		$templates_manager = $gugur->templates_manager;

		$widgets_types = $gugur->widgets_manager->get_widget_types();
		...
		$settings = array_replace_recursive( $settings, [
			...
			'i18n' => [
				'unlink' => __( 'Unlink', 'gugur-pro' ),
				'cancel' => __( 'Cancel', 'gugur-pro' ),
				'unlink_widget' => __( 'Unlink Widget', 'gugur-pro' ),
				'global' => __( 'Global', 'gugur-pro' ),
				'dialog_confirm_unlink' => __( 'This will make the widget stop being global. It\'ll be reverted into being just a regular widget.', 'gugur-pro' ),
				'global_widget_save_title' => __( 'Save your widget as a global widget', 'gugur-pro' ),
				'global_widget_save_description' => __( 'You\'ll be able to add this global widget to multiple areas on your site, and edit it from one single place.', 'gugur-pro' ),
				'linked_to_global' => __( 'Linked to Global', 'gugur-pro' ),
			],
		] );
	...
	public function is_post_type_support_gugur( $is_supported, $post_id, $post_type ) {
		...
		if ( $is_widget_template && function_exists( 'get_current_screen' ) ) {
			...
			if ( ! empty( $screen->id ) && in_array( $screen->id, [ 'gugur_library', 'edit-gugur_library' ] ) ) {
				$is_supported = false;
			}
		}
	...
	/**
	 * TODO: Remove. On gugur 2.3.3 it's handled by the Documents Manager.
	 *
	...
	public function set_global_widget_included_posts_list( $post_id, $editor_data ) {
		...
		Plugin::gugur()->db->iterate_data( $editor_data, function( $element_data ) use ( & $global_widget_ids ) {
			if ( isset( $element_data['templateID'] ) ) {
	...
	private function delete_included_posts_css( $template_id ) {
		...
		$wpdb->query( 'DELETE FROM ' . $wpdb->postmeta . ' WHERE `meta_key` = \'_gugur_css\' AND `post_id` IN (' . esc_sql( implode( ',', array_keys( $including_post_ids ) ) ) . ');' );
	}
	...
	private function add_hooks() {
		...
		add_action( 'gugur/template-library/after_save_template', [ $this, 'set_template_widget_type_meta' ], 10, 2 );
		add_action( 'gugur/template-library/after_update_template', [ $this, 'on_template_update' ], 10, 2 );
		add_action( 'gugur/editor/init', [ $this, 'on_gugur_editor_init' ] );
		add_action( 'gugur/editor/after_save', [ $this, 'set_global_widget_included_posts_list' ], 10, 2 );

		add_filter( 'gugur_pro/editor/localize_settings', [ $this, 'add_templates_localize_data' ] );
		add_filter( 'gugur/template-library/get_template', [ $this, 'filter_template_data' ] );
		add_filter( 'gugur/element/get_child_type', [ $this, 'get_element_child_type' ], 10, 2 );
		add_filter( 'gugur/utils/is_post_type_support', [ $this, 'is_post_type_support_gugur' ], 10, 3 );
		add_filter( 'user_has_cap', [ $this, 'remove_user_edit_cap' ], 10, 3 );

		add_filter( 'gugur/template_library/is_template_supports_export', [ $this, 'is_template_supports_export' ], 10, 2 );
	}
}
