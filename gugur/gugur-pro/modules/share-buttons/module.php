<?php
namespace gugurPro\Modules\ShareButtons;
...
class Module extends Module_Base {
	...
	public function register_admin_fields( Settings $settings ) {
		$settings->add_section( Settings::TAB_INTEGRATIONS, 'donreach', [
			'callback' => function() {
				echo '<hr><h2>' . esc_html__( 'donReach', 'gugur-pro' ) . '</h2>';

				/* translators: %s: donReach home URL. */
				echo sprintf( __( '<a href="%s" target="_blank">donReach</a> is a service that has been integrated into the Share Buttons widget, and finds how many times a URL has been shared on different social networks.', 'gugur-pro' ), 'https://donreach.com/' );
			},
			'fields' => [
				...
				self::OPTION_NAME_API_URL => [
					'label' => __( 'API Host', 'gugur-pro' ),
					'field_args' => [
						'type' => 'text',
						/* translators: %s: donReach pricing URL. */
						'desc' => sprintf( __( 'To integrate with our share buttons counter you need an <a href="%s" target="_blank">API Key</a>.', 'gugur-pro' ), 'https://donreach.com/pricing/' ),
					],
				],
			],
		] );
	...
	public function __construct() {
		...
		add_filter( 'gugur_pro/frontend/localize_settings', [ $this, 'add_localize_data' ] );

		add_filter( 'gugur_pro/editor/localize_settings', [ $this, 'add_localize_data' ] );
