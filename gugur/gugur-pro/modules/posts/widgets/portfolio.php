...
class Portfolio extends Base_Widget {
	...
	private function register_query_section_controls() {
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'.gugur-msie {{WRAPPER}} .gugur-portfolio-item' => 'width: calc( 100% / {{SIZE}} )',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'exclude' => [ 'custom' ],
				'default' => 'medium',
				'prefix_class' => 'gugur-portfolio--thumbnail-size-',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .gugur-post__thumbnail__link' => 'padding-bottom: calc( {{SIZE}} * 100% )',
		...
		$this->add_control(
			'item_gap',
			[
				'label' => __( 'Item Gap', 'gugur-pro' ),
				'type' => Controls_Manager::SLIDER,
				'selectors' => [
					'{{WRAPPER}} .gugur-grid' => 'grid-row-gap: {{SIZE}}{{UNIT}}; grid-column-gap: {{SIZE}}{{UNIT}}',
					'.gugur-msie {{WRAPPER}} .gugur-portfolio' => 'margin: 0 -{{SIZE}}px',
					'(desktop).gugur-msie {{WRAPPER}} .gugur-portfolio-item' => 'width: calc( 100% / {{columns.SIZE}} ); border: {{SIZE}}px solid transparent',
					'(tablet).gugur-msie {{WRAPPER}} .gugur-portfolio-item' => 'width: calc( 100% / {{columns_tablet.SIZE}} ); border: {{SIZE}}px solid transparent',
					'(mobile).gugur-msie {{WRAPPER}} .gugur-portfolio-item' => 'width: calc( 100% / {{columns_mobile.SIZE}} ); border: {{SIZE}}px solid transparent',
				],
				'frontend_available' => true,
				'classes' => 'gugur-hidden',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					...
					'.gugur-msie {{WRAPPER}} .gugur-portfolio' => 'margin: 0 -{{SIZE}}px',
					'.gugur-msie {{WRAPPER}} .gugur-portfolio-item' => 'border-style: solid; border-color: transparent; border-right-width: calc({{SIZE}}px / 2); border-left-width: calc({{SIZE}}px / 2)',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					...
					'.gugur-msie {{WRAPPER}} .gugur-portfolio-item' => 'border-bottom-width: {{SIZE}}px',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'size_units' => [ 'px', '%' ],
				'selectors' => [
					'{{WRAPPER}} .gugur-portfolio-item__img, {{WRAPPER}} .gugur-portfolio-item__overlay' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->start_controls_section(
			...
			[
				'label' => __( 'Item Overlay', 'gugur-pro' ),
				'tab' => Controls_Manager::TAB_STYLE,
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} a .gugur-portfolio-item__overlay' => 'background-color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} a .gugur-portfolio-item__title' => 'color: {{VALUE}};',
				],
				'condition' => [
					'show_title' => 'yes',
				],
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .gugur-portfolio-item__title',
				'condition' => [
					'show_title' => 'yes',
				],
			]
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .gugur-portfolio__filter' => 'color: {{VALUE}}',
				],
			]
		);
		...
		$this->add_control(
			'color_filter_active',
			[
				'label' => __( 'Active Color', 'gugur-pro' ),
				'type' => Controls_Manager::COLOR,
				'scheme' => [
					'type' => Scheme_Color::get_type(),
				...
				'selectors' => [
					'{{WRAPPER}} .gugur-portfolio__filter.gugur-active' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'scheme' => Scheme_Typography::TYPOGRAPHY_1,
				'selector' => '{{WRAPPER}} .gugur-portfolio__filter',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .gugur-portfolio__filter:not(:last-child)' => 'margin-right: calc({{SIZE}}{{UNIT}}/2)',
					'{{WRAPPER}} .gugur-portfolio__filter:not(:first-child)' => 'margin-left: calc({{SIZE}}{{UNIT}}/2)',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .gugur-portfolio__filters' => 'margin-bottom: {{SIZE}}{{UNIT}}',
				],
			]
		);
	...
	protected function render_thumbnail() {
		...
		?>
		<div class="gugur-portfolio-item__img gugur-post__thumbnail">
	...
	protected function render_filter_menu() {
		...
		?>
		<ul class="gugur-portfolio__filters">
			<li class="gugur-portfolio__filter gugur-active" data-filter="__all"><?php echo __( 'All', 'gugur-pro' ); ?></li>
			<?php foreach ( $terms as $term ) { ?>
				<li class="gugur-portfolio__filter" data-filter="<?php echo esc_attr( $term->term_id ); ?>"><?php echo $term->name; ?></li>
			<?php } ?>
		</ul>
		<?php
	}
	...
	protected function render_title() {
		...
		?>
		<<?php echo $tag; ?> class="gugur-portfolio-item__title">
		<?php the_title(); ?>
		</<?php echo $tag; ?>>
		<?php
	}
	...
	protected function render_categories_names() {
		...
		$separator = '<span class="gugur-portfolio-item__tags__separator"></span>';

		$tags_array = [];

		foreach ( $post->tags as $tag ) {
			$tags_array[] = '<span class="gugur-portfolio-item__tags__tag">' . $tag->name . '</span>';
		}

		?>
		<div class="gugur-portfolio-item__tags">
	...
	protected function render_post_header() {
		...
		$tags_classes = array_map( function( $tag ) {
			return 'gugur-filter-' . $tag->term_id;
		}, $post->tags );

		$classes = [
			'gugur-portfolio-item',
			'gugur-post',
	...
	protected function render_overlay_header() {
		?>
		<div class="gugur-portfolio-item__overlay">
		<?php
	}
	...
	protected function render_loop_header() {
		...
		?>
		<div class="gugur-portfolio gugur-grid gugur-posts-container">
		<?php
	}
	...
	public function pre_get_posts_filter( $wp_query ) {
		...
		/**
		 * gugur Pro portfolio widget Query args.
		 *
		 ...
		 * @param Posts     $this
		 */
		do_action( "gugur_pro/portfolio/query/{$query_id}", $wp_query, $this );
	}
}
