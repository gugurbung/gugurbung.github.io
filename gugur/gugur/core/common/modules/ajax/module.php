<?php
namespace gugur\Core\Common\Modules\Ajax;
 ...
 * gugur ajax manager.
 *
 * gugur ajax manager handler class is responsible for handling gugur
...
class Module extends BaseModule {
	 ...
	 *
	 * Initializing gugur ajax manager.
	 *
	 * @since 2.0.0
	 * @access public
	 */
	public function __construct() {
		add_action( 'wp_ajax_gugur_ajax', [ $this, 'handle_ajax_request' ] );
	}
	...
	public function register_ajax_action( $tag, $callback ) {
		if ( ! did_action( 'gugur/ajax/register_actions' ) ) {
			_doing_it_wrong( __METHOD__, esc_html( sprintf( 'Use `%s` hook to register ajax action.', 'gugur/ajax/register_actions' ) ), '2.0.0' );
		}
	 ...
	 *
	 * Fired by `wp_ajax_gugur_ajax` action.
	 *
	 * @since 2.0.0
	 * @access public
	 */
	public function handle_ajax_request() {
		if ( ! $this->verify_request_nonce() ) {
			$this->add_response_data( false, __( 'Token Expired.', 'gugur' ) )
		 ...
		 */
		do_action( 'gugur/ajax/register_actions', $this );
		...
		foreach ( $this->requests as $id => $action_data ) {
			...
			if ( ! isset( $this->ajax_actions[ $action_data['action'] ] ) ) {
				$this->add_response_data( false, __( 'Action not found.', 'gugur' ), Exceptions::BAD_REQUEST );
	 ...
	 * give the user an access to gugur ajax actions.
	 *
	 * @since 2.3.0
	 * @access public
	 *
