...
use gugur\Core\Common\Modules\Ajax\Module;
use gugur\Core\Logger\Loggers\Logger_Interface;
use gugur\Core\Logger\Items\PHP;
use gugur\Core\Logger\Items\JS;
use gugur\Plugin;
...
class Manager extends BaseModule {
	...
	public function shutdown() {
		...
		// `untrailingslashit` in order to include other plugins prefixed with gugur.
		$gugur_path = untrailingslashit( wp_normalize_path( gugur_PATH ) );

		if ( false === strpos( $error_path, $gugur_path ) ) {
			return;
		}
	 ...
	 * Javascript log.
	 *
	 * Log gugur errors and save them in the database.
	 *
	 * Fired by `wp_ajax_gugur_js_log` action.
	 *
	 */
	...
	public function register_default_loggers() {
		$this->register_logger( 'db', 'gugur\Core\Logger\Loggers\Db' );
	...
	private function register_loggers() {
		if ( ! did_action( 'gugur/loggers/register' ) ) {
			do_action( 'gugur/loggers/register', $this );
		}
	}
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_gugur_js_log', [ $this, 'js_log' ] );

		add_action( 'gugur/loggers/register', [ $this, 'register_default_loggers' ] );
	}
}
