...
abstract class DB_Upgrades_Manager extends Background_Task_Manager {
	...
	public function get_task_runner_class() {
		return 'gugur\Core\Upgrade\Updater';
	}
	...
	public function admin_notice_start_upgrade() {
		...
		$message = '<p>' . sprintf( __( '%s Your site database needs to be updated to the latest version.', 'gugur' ), $this->get_updater_label() ) . '</p>';
		$message .= '<p>' . sprintf( '<a href="%s" class="button-primary">%s</a>', $upgrade_link, __( 'Update Now', 'gugur' ) ) . '</p>';
	...
	public function admin_notice_upgrade_is_running() {
		...
		$message = '<p>' . sprintf( __( '%s Database update process is running in the background.', 'gugur' ), $this->get_updater_label() ) . '</p>';
	...
	public function admin_notice_upgrade_is_completed() {
		...
		$message = '<p>' . sprintf( __( '%s The database update process is now complete. Thank you for updating to the latest version!', 'gugur' ), $this->get_updater_label() ) . '</p>';
	...
	protected function start_run() {
		...
		Plugin::$instance->logger->get_logger()->info( 'gugur data updater process has been queued.', [
			'meta' => [
				'plugin' => $this->get_plugin_label(),
				'from' => $this->current_version,
				'to' => $this->get_new_version(),
