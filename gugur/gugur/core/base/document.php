...
abstract class Document extends Controls_Stack {
	...
	const PAGE_META_KEY = '_gugur_page_settings';
	...
	public static function get_editor_panel_config() {
		return [
			...
			'messages' => [
				...
				'publish_notification' => sprintf( __( 'Hurray! Your %s is live.', 'gugur' ), static::get_title() ),
			],
		];
	}
	...
	public function get_container_attributes() {
		...
		$attributes = [
			'data-gugur-type' => $this->get_name(),
			'data-gugur-id' => $id,
			'class' => 'gugur gugur-' . $id,
		];

		$version_meta = $this->get_main_meta( '_gugur_version' );

		if ( version_compare( $version_meta, '2.5.0', '<' ) ) {
			$attributes['class'] .= ' gugur-bc-flex-widget';
		}

		if ( ! Plugin::$instance->preview->is_preview_mode( $id ) ) {
			$attributes['data-gugur-settings'] = wp_json_encode( $this->get_frontend_settings() );
		}
	...
	public function get_wp_preview_url() {
		 ...
		 */
		$url = apply_filters( 'gugur/document/urls/wp_preview', $url, $this );
	...
	public function get_exit_to_dashboard_url() {
		 ...
		 * @param Document $this The document instance.
		 */
		$url = apply_filters( 'gugur/document/urls/exit_to_dashboard', $url, $this );
	...
	public function get_autosave( $user_id = 0, $create = false ) {
		...
		if ( $autosave_id ) {
			...
			$autosave_id = wp_create_post_autosave( [
				...
				'post_content' => '<!-- Created With gugur -->',
				'post_modified' => current_time( 'mysql' ),
			] );

			Plugin::$instance->db->copy_gugur_meta( $this->post->ID, $autosave_id );
	...
	public function filter_admin_row_actions( $actions ) {
		if ( $this->is_built_with_gugur() && $this->is_editable_by_current_user() ) {
			$actions['edit_with_gugur'] = sprintf(
				'<a href="%1$s">%2$s</a>',
				$this->get_edit_url(),
				__( 'Edit with gugur', 'gugur' )
			);
		}
	...
	protected function _get_initial_config() {
		return [
			'id' => $this->get_main_id(),
			'type' => $this->get_name(),
			'version' => $this->get_main_meta( '_gugur_version' ),
	...
	protected function _register_controls() {
		$this->start_controls_section(
			'document_settings',
			[
				'label' => __( 'General Settings', 'gugur' ),
		...
		if ( $is_published || $can_publish || ! Plugin::$instance->editor->is_edit_mode() ) {
			...
			if ( 'future' === $this->get_main_post()->post_status ) {
				$statuses['future'] = __( 'Future', 'gugur' );
			}

			$this->add_control(
				'post_status',
				[
					'label' => __( 'Status', 'gugur' ),
		 ...
		 *
		 * Fires after gugur registers the document controls.
		 *
		 * @since 2.0.0
		 *
		 * @param Document $this The document instance.
		 */
		do_action( 'gugur/documents/register_controls', $this );
	}
	...
	public function save( $data ) {
		 ...
		 *
		 * Fires when document save starts on gugur.
		 *
		 * @since 2.5.12
		 *
		 * @param \gugur\Core\Base\Document $this The current document.
		 * @param $data.
		 */
		do_action( 'gugur/document/before_save', $this, $data );
		 ...
		 * @param $data.
		 */
		do_action( 'gugur/document/after_save', $this, $data );
	 ...
	 * Is built with gugur.
	 *
	 * Check whether the post was built with gugur.
	 *
	 * @since 2.0.0
	 * @access public
	 *
	 * @return bool Whether the post was built with gugur.
	 */
	public function is_built_with_gugur() {
		return ! ! get_post_meta( $this->post->ID, '_gugur_edit_mode', true );
	}
	...
	public function get_edit_url() {
		 ...
		 * @param Document $this The document instance.
		 */
		$url = apply_filters( 'gugur/document/urls/edit', $url, $this );
	...
	public function get_preview_url() {
		...
		if ( empty( $url ) ) {
			...
			$url = set_url_scheme( add_query_arg( [
				'gugur-preview' => $this->get_main_id(),
				'ver' => time(),
			], $this->get_permalink() ) );
			 ...
			 * @param Document $this The document instance.
			 */
			$url = apply_filters( 'gugur/document/urls/preview', $url, $this );
		}
	...
	public function get_elements_data( $status = DB::STATUS_PUBLISH ) {
		$elements = $this->get_json_meta( '_gugur_data' );
		...
		if ( Plugin::$instance->editor->is_edit_mode() ) {
			if ( empty( $elements ) && empty( $autosave_elements ) ) {
				// Convert to gugur.
				$elements = $this->convert_to_gugur();
				if ( $this->is_autosave() ) {
					Plugin::$instance->db->copy_gugur_meta( $this->post->post_parent, $this->post->ID );
				}
			}
		}
	 ...
	 * @access public
	 */
	public function convert_to_gugur() {
		$this->save( [] );
	...
	public function print_elements_with_wrapper( $elements_data = null ) {
		...
		<div <?php echo Utils::render_html_attributes( $this->get_container_attributes() ); ?>>
			<div class="gugur-inner">
				<div class="gugur-section-wrap">
	...
	public function get_panel_page_settings() {
		return [
			...
			'title' => sprintf( __( '%s Settings', 'gugur' ), static::get_title() ),
		];
	}
	...
	protected function save_elements( $elements ) {
		...
		$is_meta_updated = update_metadata( 'post', $this->post->ID, '_gugur_data', $json_value );

		/**
		 * Before saving data.
		 *
		 * Fires before gugur saves data to the database.
		 *
		 * @since 1.0.0
		 *
		 ...
		 */
		do_action( 'gugur/db/before_save', $this->post->post_status, $is_meta_updated );
		 ...
		 * After saving data.
		 *
		 * Fires after gugur saves data to the database.
		 *
		 * @since 1.0.0
		 *
		 ...
		 */
		do_action( 'gugur/editor/after_save', $this->post->ID, $editor_data );
	}
	...
	public function save_version() {
		if ( ! defined( 'IS_gugur_UPGRADE' ) ) {
			// Save per revision.
			$this->update_meta( '_gugur_version', gugur_VERSION );

			/**
			 * Document version save.
			 *
			 * Fires when document version is saved on gugur.
			 * Will not fire during gugur Upgrade.
			 *
			 * @since 2.5.12
			 *
			 * @param \gugur\Core\Base\Document $this The current document.
			 *
			 */
			do_action( 'gugur/document/save_version', $this );
		}
	}
	...
	public function get_last_edited() {
		...
		$date = date_i18n( _x( 'M j, H:i', 'revision date format', 'gugur' ), strtotime( $post->post_modified ) );
		$display_name = get_the_author_meta( 'display_name', $post->post_author );

		if ( $autosave_post || 'revision' === $post->post_type ) {
			/* translators: 1: Saving date, 2: Author display name */
			$last_edited = sprintf( __( 'Draft saved on %1$s by %2$s', 'gugur' ), '<time>' . $date . '</time>', $display_name );
		} else {
			/* translators: 1: Editing date, 2: Author display name */
			$last_edited = sprintf( __( 'Last edited on %1$s by %2$s', 'gugur' ), '<time>' . $date . '</time>', $display_name );
		}
	...
	public function __construct( array $data = [] ) {
		if ( $data ) {
			...
			$saved_settings = get_post_meta( $this->post->ID, '_gugur_page_settings', true );
