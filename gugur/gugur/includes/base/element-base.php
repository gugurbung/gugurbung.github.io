 ...
 * gugur element base.
 *
 * An abstract class to register new gugur elements. It extended the
...
abstract class Element_Base extends Controls_Stack {
	...
	final public static function is_edit_buttons_enabled() {
		return get_option( 'gugur_edit_buttons' );
	}
	...
	public function get_help_url() {
		return 'https://go.gugur.com/widget-' . $this->get_name();
	}
	...
	public function print_element() {
		 ...
		 *
		 * Fires before gugur element is rendered in the frontend.
		 *
		 * @since 2.2.0
		 *
		 * @param Element_Base $this The element.
		 */
		do_action( 'gugur/frontend/before_render', $this );
		 ...
		 * @param Element_Base $this The element.
		 */
		do_action( "gugur/frontend/{$element_type}/before_render", $this );
		 ...
		 * @param Element_Base $this The element.
		 */
		$should_render = apply_filters( "gugur/frontend/{$element_type}/should_render", $should_render, $this );
		 ...
		 *
		 * Fires after gugur element is rendered in the frontend.
		 *
		 * The dynamic portion of the hook name, `$element_type`, refers to the element type.
		 *
		 * @since 1.0.0
		 ...
		 * @param Element_Base $this The element.
		 */
		do_action( "gugur/frontend/{$element_type}/after_render", $this );
		 ...
		 * @param Element_Base $this The element.
		 */
		do_action( 'gugur/frontend/after_render', $this );
	}
	 ...
	 *
	 * The data with the HTML used always to display the data, but the gugur
	 ...
	 * class for each HTML element. This way gugur can set custom styles for
	 * each element.
	 *
	 * @since 1.0.0
	 * @access public
	...
	public function get_unique_selector() {
		return '.gugur-element-' . $this->get_id();
	}
	...
	protected function _add_render_attributes() {
		...
		if ( ! empty( $settings['animation'] ) || ! empty( $settings['_animation'] ) ) {
			...
			$this->add_render_attribute( '_wrapper', 'class', 'gugur-invisible' );
		}
		 ...
		 * @param Element_Base $this The element.
		 */
		do_action( 'gugur/element/after_add_attributes', $this );
	}
	...
	private function get_child_type( $element_data ) {
		 ...
		 */
		$child_type = apply_filters( 'gugur/element/get_child_type', $child_type, $element_data, $this );
	 ...
	 * way gugur renders data when initializing elements.
	 *
	 * @since 1.0.0
	 * @access public
	 *
