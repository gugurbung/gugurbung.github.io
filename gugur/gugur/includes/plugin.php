...
use gugur\Core\Admin\Admin;
use gugur\Core\Common\Modules\Ajax\Module as Ajax;
use gugur\Core\Common\App as CommonApp;
use gugur\Core\Debug\Inspector;
use gugur\Core\Documents_Manager;
use gugur\Core\Editor\Editor;
use gugur\Core\Files\Manager as Files_Manager;
use gugur\Core\Files\Assets\Manager as Assets_Manager;
use gugur\Core\Modules_Manager;
use gugur\Core\Settings\Manager as Settings_Manager;
use gugur\Core\Settings\Page\Manager as Page_Settings_Manager;
use gugur\Modules\History\Revisions_Manager;
use gugur\Core\DynamicTags\Manager as Dynamic_Tags_Manager;
use gugur\Core\Logger\Manager as Log_Manager;
 ...
 * gugur plugin.
 *
 * The main plugin handler class is responsible for initializing gugur. The
...
class Plugin {
	 ...
	 * @access public
	 *
	 * @var \gugur\Core\RoleManager\Role_Manager
	 */
	public $role_manager;
	...
	public function __clone() {
		...
		_doing_it_wrong( __FUNCTION__, esc_html__( 'Something went wrong.', 'gugur' ), '1.0.0' );
	}
	...
	public static function instance() {
		if ( is_null( self::$instance ) ) {
			 ...
			 * gugur loaded.
			 *
			 * Fires when gugur was fully loaded and instantiated.
			 *
			 * @since 1.0.0
			 */
			do_action( 'gugur/loaded' );
		}
	 ...
	 * Init.
	 *
	 * Initialize gugur Plugin. Register gugur support for all the
	 * supported post types and initialize gugur components.
	 *
	 * @since 1.0.0
	 * @access public
	 */
	public function init() {
		 ...
		 * gugur init.
		 *
		 * Fires on gugur init, after gugur has finished loading but
		 * before any headers are sent.
		 *
		 * @since 1.0.0
		 */
		do_action( 'gugur/init' );
	}

	/**
	 * Get install time.
	 *
	 * Retrieve the time when gugur was installed.
	 *
	 * @since 2.6.0
	 * @access public
	 * @static
	 *
	 * @return int Unix timestamp when gugur was installed.
	 */
	public function get_install_time() {
		$installed_time = get_option( '_gugur_installed_time' );

		if ( ! $installed_time ) {
			$installed_time = time();

			update_option( '_gugur_installed_time', $installed_time );
		}
	 ...
	 * Init components.
	 *
	 * Initialize gugur components. Register actions, run setting manager,
	 * initialize all the components that run gugur, and if in admin page
	 ...
	 *
	 * Register gugur support for all the supported post types defined by
	 * the user in the admin screen and saved as `gugur_cpt_support` option
	...
	private function add_cpt_support() {
		$cpt_support = get_option( 'gugur_cpt_support', [ 'page', 'post' ] );

		foreach ( $cpt_support as $cpt_slug ) {
			add_post_type_support( $cpt_slug, 'gugur' );
		}
	}
	 ...
	 * Register autoloader.
	 *
	 * gugur autoloader loads all the classes needed to run the plugin.
	 *
	 * @since 1.6.0
	 * @access private
	 */
	private function register_autoloader() {
		require gugur_PATH . '/includes/autoloader.php';
