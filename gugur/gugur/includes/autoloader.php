 ...
 * gugur autoloader.
 *
 * gugur autoloader handler class is responsible for loading the different
...
class Autoloader {
	 ...
	 * Classes map.
	 *
	 * Maps gugur classes to file names.
	 *
	 * @since 1.6.0
	 * @access private
	 * @static
	 *
	 * @var array Classes used by gugur.
	 */
	 ...
	 * reference : https://github.com/gugur/gugur/issues/7310#issuecomment-469593385
	 *
	 * @param $string
	...
	private static function load_class( $relative_class_name ) {
		...
		if ( isset( $classes_map[ $relative_class_name ] ) ) {
			$filename = gugur_PATH . '/' . $classes_map[ $relative_class_name ];
		} else {
			$filename = strtolower(
				preg_replace(
					[ '/([a-z])([A-Z])/', '/_/', '/\\\/' ],
			...
			$filename = gugur_PATH . $filename . '.php';
		}
