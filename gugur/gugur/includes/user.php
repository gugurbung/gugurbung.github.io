 ...
 * gugur user.
 *
 * gugur user handler class is responsible for checking if the user can edit
 * with gugur and displaying different admin notices.
 *
 * @since 1.0.0
 */
class User {
	 ...
	 */
	const ADMIN_NOTICES_KEY = 'gugur_admin_notices';

	const INTRODUCTION_KEY = 'gugur_introduction';

	const BETA_TESTER_META_KEY = 'gugur_beta_tester';
	 ...
	 * @var string API URL.
	 */
	const BETA_TESTER_API_URL = 'https://my.gugur.com/api/v1/beta_tester/';

	/**
	 * Init.
	 *
	 * Initialize gugur user.
	 *
	 * @since 1.0.0
	 * @access public
	 * @static
	...
	public static function init() {
		add_action( 'wp_ajax_gugur_set_admin_notice_viewed', [ __CLASS__, 'ajax_set_admin_notice_viewed' ] );
		add_action( 'admin_post_gugur_set_admin_notice_viewed', [ __CLASS__, 'ajax_set_admin_notice_viewed' ] );
	...
	/**
	 * Is current user can access gugur.
	 *
	 * Whether the current user role is not excluded by gugur Settings.
	 *
	 * @since 2.1.7
	 * @access public
	 * @static
	...
	public static function is_current_user_in_editing_black_list() {
		...
		$exclude_roles = get_option( 'gugur_exclude_user_roles', [] );
	 ...
	 *
	 * Fired by `wp_ajax_gugur_set_admin_notice_viewed` action.
	 *
	 * @since 1.0.0
	 * @access public
	 * @static
