 ...
 * gugur database.
 *
 * gugur database handler class is responsible for communicating with the
 * DB, save and retrieve gugur data and meta data.
 *
 * @since 1.0.0
 */
class DB {
	 ...
	 *
	 * When editing the with gugur the first time, the current page content
	 * is parsed into Text Editor Widget that contains the original data.
	 *
	 * @since 2.1.0
	 * @deprecated 2.3.0 Use `Plugin::$instance->documents->get( $post_id )->convert_to_gugur()` instead
	 * @access public
	 *
	 * @param int $post_id Post ID.
	 *
	 * @return array Content in gugur format.
	 */
	public function get_new_editor_from_wp_editor( $post_id ) {
		 _deprecated_function( __METHOD__, '2.3.0', 'Plugin::$instance->documents->get( $post_id )->convert_to_gugur()' );

		$document = Plugin::$instance->documents->get( $post_id );

		if ( $document ) {
			return $document->convert_to_gugur();
		}
	 ...
	 *
	 * @since 1.0.0
	 * @deprecated 2.1.0 Use `Plugin::$instance->documents->get( $post_id )->convert_to_gugur()` instead
	 * @access public
	 *
	 * @param int $post_id Post ID.
	 *
	...
	public function _get_new_editor_from_wp_editor( $post_id ) {
		_deprecated_function( __METHOD__, '2.1.0', 'Plugin::$instance->documents->get( $post_id )->convert_to_gugur()' );
	 ...
	 * Is using gugur.
	 *
	 * Set whether the page is using gugur or not.
	 *
	 * @since 1.5.0
	 * @access public
	 *
	 * @param int  $post_id      Post ID.
	 * @param bool $is_gugur Optional. Whether the page is gugur page.
	 *                           Default is true.
	 */
	public function set_is_gugur_page( $post_id, $is_gugur = true ) {
		if ( $is_gugur ) {
			// Use the string `builder` and not a boolean for rollback compatibility
			update_post_meta( $post_id, '_gugur_edit_mode', 'builder' );
		} else {
			delete_post_meta( $post_id, '_gugur_edit_mode' );
		}
	}
	 ...
	 * Iterate data.
	 *
	 * Accept any type of gugur data and a callback function. The callback
	 * function runs recursively for each element and his child elements.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	 * @param array    $data_container Any type of gugur data.
	...
	/**
	 * Safely copy gugur meta.
	 *
	 * Make sure the original page was built with gugur and the post is not
	 * auto-save. Only then copy gugur meta from one post to another using
	 * `copy_gugur_meta()`.
	 *
	 * @since 1.9.2
	 * @access public
	 ...
	 */
	public function safe_copy_gugur_meta( $from_post_id, $to_post_id ) {
		// It's from  WP-Admin & not from gugur.
		if ( ! did_action( 'gugur/db/before_save' ) ) {

			if ( ! Plugin::$instance->db->is_built_with_gugur( $from_post_id ) ) {
				return;
			}

			// It's an exited gugur auto-save
			if ( get_post_meta( $to_post_id, '_gugur_data', true ) ) {
				return;
			}
		}

		$this->copy_gugur_meta( $from_post_id, $to_post_id );
	}
	 ...
	 *
	 * Consider using `safe_copy_gugur_meta()` method instead.
	 *
	 * @since 1.1.0
	 * @access public
	 *
	 ...
	 * @param int $to_post_id   Target post ID.
	 */
	public function copy_gugur_meta( $from_post_id, $to_post_id ) {
		...
		foreach ( $from_post_meta as $meta_key => $values ) {
			// Copy only meta with the `_gugur` prefix
			if ( 0 === strpos( $meta_key, '_gugur' ) || in_array( $meta_key, $core_meta, true ) ) {
				$value = $values[0];

				// The gugur JSON needs slashes before saving
				if ( '_gugur_data' === $meta_key ) {
	 ...
	 */
	public function is_built_with_gugur( $post_id ) {
		return ! ! get_post_meta( $post_id, '_gugur_edit_mode', true );
	}
	 ...
	 *
	 * Retrieve the post plain text from any given gugur data.
	 *
	 * @since 1.9.2
	 * @access public
	 *
