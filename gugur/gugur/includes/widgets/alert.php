...
class Widget_Alert extends Widget_Base {
	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'default' => __( 'This is an Alert', 'gugur' ),
				'label_block' => true,
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default' => __( 'I am a description. Click the edit button to change this text.', 'gugur' ),
				'separator' => 'none',
				'show_label' => false,
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .gugur-alert' => 'background-color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .gugur-alert' => 'border-color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			'border_left-width',
			[
				'label' => __( 'Left Border Width', 'gugur' ),
				...
				'selectors' => [
					'{{WRAPPER}} .gugur-alert' => 'border-left-width: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .gugur-alert-title' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				'name' => 'alert_title',
				'selector' => '{{WRAPPER}} .gugur-alert-title',
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .gugur-alert-description' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .gugur-alert-description',
	...
	protected function render() {
		...
		if ( ! empty( $settings['alert_type'] ) ) {
			$this->add_render_attribute( 'wrapper', 'class', 'gugur-alert gugur-alert-' . $settings['alert_type'] );
		}

		$this->add_render_attribute( 'wrapper', 'role', 'alert' );

		$this->add_render_attribute( 'alert_title', 'class', 'gugur-alert-title' );
		...
		<div <?php echo $this->get_render_attribute_string( 'wrapper' ); ?>>
			...
			if ( ! empty( $settings['alert_description'] ) ) :
				$this->add_render_attribute( 'alert_description', 'class', 'gugur-alert-description' );
			...
			<?php if ( 'show' === $settings['show_dismiss'] ) : ?>
				<button type="button" class="gugur-alert-dismiss">
					<span aria-hidden="true">&times;</span>
					<span class="gugur-screen-only"><?php echo __( 'Dismiss alert', 'gugur' ); ?></span>
				</button>
			<?php endif; ?>
		</div>
		<?php
	...
	protected function _content_template() {
		...
		<# if ( settings.alert_title ) {
			view.addRenderAttribute( {
				alert_title: { class: 'gugur-alert-title' },
				alert_description: { class: 'gugur-alert-description' }
			} );

			view.addInlineEditingAttributes( 'alert_title', 'none' );
			view.addInlineEditingAttributes( 'alert_description' );
			#>
			<div class="gugur-alert gugur-alert-{{ settings.alert_type }}" role="alert">
