...
/**
 * gugur star rating widget.
 *
 * gugur widget that displays star rating.
 *
 * @since 2.3.0
 */
class Widget_Star_Rating extends Widget_Base {
	...
	protected function _register_controls() {
		...
		$this->add_control(
			'unmarked_star_style',
			[
				'label' => __( 'Unmarked Style', 'gugur' ),
				'type' => Controls_Manager::CHOOSE,
				'label_block' => false,
				'options' => [
					'solid' => [
						'title' => __( 'Solid', 'gugur' ),
						'icon' => 'eicon-star',
					],
					'outline' => [
						'title' => __( 'Outline', 'gugur' ),
						'icon' => 'eicon-star-o',
					],
				],
				'default' => 'solid',
		...
		$this->add_responsive_control(
			...
			[
				...
				'prefix_class' => 'gugur-star-rating%s--align-',
				'selectors' => [
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .gugur-star-rating__title' => 'color: {{VALUE}}',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .gugur-star-rating__title',
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'body:not(.rtl) {{WRAPPER}}:not(.gugur-star-rating--align-justify) .gugur-star-rating__title' => 'margin-right: {{SIZE}}{{UNIT}}',
					'body.rtl {{WRAPPER}}:not(.gugur-star-rating--align-justify) .gugur-star-rating__title' => 'margin-left: {{SIZE}}{{UNIT}}',
				],
			]
		);
	...
	protected function render() {
		...
		$schema_rating = '<span itemprop="ratingValue" class="gugur-screen-only">' . $textual_rating . '</span>';
		$stars_element = '<div ' . $this->get_render_attribute_string( 'icon_wrapper' ) . '>' . $this->render_stars( $icon ) . ' ' . $schema_rating . '</div>';
		?>

		<div class="gugur-star-rating__wrapper">
			<?php if ( ! empty( $settings['title'] ) ) : ?>
				<div class="gugur-star-rating__title"><?php echo $settings['title']; ?></div>
			<?php endif; ?>
	...
	protected function _content_template() {
		...
		<#
			...
			renderStars = function( icon ) {
				...
				for ( var stars = 1; stars <= ratingData[1]; stars++ ) {
					if ( stars <= flooredRating  ) {
						starsHtml += '<i class="gugur-star-full">' + icon + '</i>';
					} else if ( flooredRating + 1 === stars && rating !== flooredRating ) {
						starsHtml += '<i class="gugur-star-' + ( rating - flooredRating ).toFixed( 1 ) * 10 + '">' + icon + '</i>';
					} else {
						starsHtml += '<i class="gugur-star-empty">' + icon + '</i>';
					}
				}
			...
			view.addRenderAttribute( 'iconWrapper', 'class', 'gugur-star-rating' );
		...
		<div class="gugur-star-rating__wrapper">
			<# if ( ! _.isEmpty( settings.title ) ) { #>
				<div class="gugur-star-rating__title">{{ settings.title }}</div>
			<# } #>
			<div {{{ view.getRenderAttributeString( 'iconWrapper' ) }}} >
				{{{ stars }}}
				<span itemprop="ratingValue" class="gugur-screen-only">{{ textualRating }}</span>
			</div>
		</div>
