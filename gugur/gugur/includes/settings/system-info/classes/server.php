<?php
namespace gugur\System_Info\Classes;
...
/**
 * gugur server environment report.
 *
 * gugur system report handler class responsible for generating a report for
...
class Server_Reporter extends Base_Reporter {
	...
	public function get_php_version() {
		...
		if ( version_compare( $result['value'], '5.4', '<' ) ) {
			$result['recommendation'] = _x( 'We recommend to use php 5.4 or higher', 'System Info', 'gugur' );
	...
	public function get_write_permissions() {
		...
		$gugur_uploads_path = $wp_upload_dir['basedir'] . '/gugur';

		if ( is_dir( $gugur_uploads_path ) ) {
			$paths_to_check[ $gugur_uploads_path ] = 'gugur uploads directory';
		}
	...
	/**
	 * Check for gugur library connectivity.
	 *
	 * Check whether the remote gugur library is reachable.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	 ...
	 *    Report data.
	 *
	 *    @type string $value   The status of gugur library connectivity.
	 *    @type bool   $warning Whether to display a warning. True if gugur
	 * *                        library is not reachable, False otherwise.
	 * }
	 */
	public function get_gugur_library() {
