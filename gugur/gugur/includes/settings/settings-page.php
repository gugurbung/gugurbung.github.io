...
abstract class Settings_Page {
	...
	final public function register_settings_fields() {
		...
		foreach ( $tabs as $tab_id => $tab ) {
			...
			foreach ( $tab['sections'] as $section_id => $section ) {
				$full_section_id = 'gugur_' . $section_id . '_section';
				...
				foreach ( $section['fields'] as $field_id => $field ) {
					$full_field_id = ! empty( $field['full_field_id'] ) ? $field['full_field_id'] : 'gugur_' . $field_id;
	...
	public function display_settings_page() {
		...
		<div class="wrap">
			...
			<div id="gugur-settings-tabs-wrapper" class="nav-tab-wrapper">
				<?php
				foreach ( $tabs as $tab_id => $tab ) {
					...
					echo "<a id='gugur-settings-tab-{$tab_id}' class='nav-tab{$active_class}' href='#tab-{$tab_id}'>{$tab['label']}</a>";
				}
				?>
			</div>
			<form id="gugur-settings-form" method="post" action="options.php">
				...
				foreach ( $tabs as $tab_id => $tab ) {
					...
					if ( 'general' === $tab_id ) {
						$active_class = ' gugur-active';
					}

					echo "<div id='tab-{$tab_id}' class='gugur-settings-form-page{$active_class}'>";
	...
	private function ensure_tabs() {
		if ( null === $this->tabs ) {
			 ...
			 *
			 * Fires after the settings are created in gugur admin page.
			 *
			 ...
			 */
			do_action( "gugur/admin/after_create_settings/{$page_id}", $this );
		}
	}
}
